...
\subsubsection{ACT Already Shows the Pattern}

\textbf{The key finding is that ACT DR6 residuals already exhibit the exact oscillatory structure predicted by the Ridder field.} When we compare ACT high-$\ell$ data against $\Lambda$CDM best fits, the residuals show:
\begin{itemize}
...
\begin{table}[ht]
\centering
\caption{Planck-only $\chi^2$ breakdown compared to Planck+SH0ES. In Planck-only fits, low-$\ell$ EE shows no preference for EDE. When SH0ES is added, the joint best-fit shifts to a parameter region where low-$\ell$ EE is substantially better fit.}
\label{tab:planck_only}
\begin{tabular}{lcc}
\toprule
\textbf{Component} & \textbf{Planck-only $\Delta\chi^2$} & \textbf{+SH0ES $\Delta\chi^2$} \\
\midrule
Planck high-$\ell$ TTTEEE & $+15$ & $+19$ \\
Planck low-$\ell$ EE & $\mathbf{-1}$ & $\mathbf{-15}$ \\
Planck low-$\ell$ TT & $-4$ & $-3$ \\
Planck lensing & $0$ & $0$ \\
SH0ES & --- & $-4$ \\
\midrule
\textbf{Total} & $\mathbf{+10}$ & $\mathbf{-5}$ \\
\bottomrule
\end{tabular}
\end{table}

In Planck-only fits, the Ridder field pays an $\mathcal{O}(15)$ $\chi^2$ cost in the high-$\ell$ damping tail and recovers only a small improvement in low-$\ell$ polarization ($\Delta\chi^2 \approx -1$). The Planck-only best-fit EDE solution sits in a broad region of parameter space that Planck alone does not strongly constrain. When we add the SH0ES $H_0$ prior, the joint best-fit shifts into a \emph{different} region of parameter space---one where $H_0 \approx 70$ km s$^{-1}$ Mpc$^{-1}$ with correspondingly different values of $\Lambda_{\rm EDE}$, $\tau$, and $n_s$. Evaluated at this new joint best-fit, Planck low-$\ell$ EE is substantially better fit ($\Delta\chi^2 = -15$), while the high-$\ell$ penalty remains of the same order.
