...
Fitting to Planck 2018 + pre-DESI BAO + SH0ES data, we find that Geometric EDE with $k = 8$ free parameters achieves $H_0 = 69.7 \pm 0.5$ km s$^{-1}$ Mpc$^{-1}$ and $S_8 = 0.82 \pm 0.01$, reducing the Hubble tension from $4.3\sigma$ to $\sim 2.3\sigma$ and the $S_8$ tension from $2.7\sigma$ to $1.5\sigma$. In this pre-DESI configuration, Geometric EDE achieves $\Delta\chi^2 \approx -4.5$ relative to $\Lambda$CDM, with the preference driven by a ``triangular tension'': Planck high-$\ell$ TTTEEE disfavors the modified damping tail by $\Delta\chi^2 \approx +19$, but Planck low-$\ell$ EE and TT favor it by $\Delta\chi^2 \approx -18$, and SH0ES adds another $-3.5$. When DESI Y1 BAO is included, the model converges to a tighter ``convergence window'' with $H_0 \approx 69.5$ km s$^{-1}$ Mpc$^{-1}$ and $r_s \approx 146$ Mpc. Crucially, DESI itself is nearly neutral ($\Delta\chi^2 = +0.2$); its effect is to shift the best-fit parameters to a region where the Planck low-$\ell$ benefits shrink from $-18$ to $-2$, exposing the fixed high-$\ell$ cost and yielding a net $\Delta\chi^2 \approx +11$. At equal parameter count, a late-time dynamical model ($w_0 w_a$CDM) modestly improves the fit relative to $\Lambda$CDM ($\Delta\chi^2 \simeq -3$) but leaves both tensions largely intact ($H_0 \simeq 69$, $S_8 \simeq 0.83$).

Among the $k = 8$ extensions we test, Geometric EDE lies on a Pareto frontier in $(H_0, S_8, \chi^2)$ space. The model reduces the sound horizon by $\sim 1\%$ ($\Delta r_s \approx -0.9$ Mpc) and produces a characteristic ``soft shoulder'' in the CMB damping tail---an oscillating residual pattern at the $1$--$2\%$ level over $600 \lesssim \ell \lesssim 3000$. We show that this signature is consistent with ACT DR6 data and constitutes a concrete falsification target for CMB-S4.

\textbf{Core result:} Geometric EDE occupies a specific corner of parameter space that different datasets pull in different directions. Planck low-$\ell$ polarization and the local distance ladder favor the EDE corner; Planck high-$\ell$ temperature and polarization resist it; DESI is neutral but tightens the geometry. The question is not whether tensions can be eliminated, but which compromise point the data select. Our answer: once early-time geometry is allowed to move, the combined constraints naturally converge toward $H_0 \approx 70$ km s$^{-1}$ Mpc$^{-1}$---not the SH0ES value of 73, nor the $\Lambda$CDM baseline of 68. CMB-S4 will provide the decisive test.
\end{abstract}
...
\subsubsection{Phase 2: ACT DR6 Damping Tail Diagnostic---A Blind Prediction Test}

A key feature of the Ridder field is the soft shoulder it predicts in the high-$\ell$ CMB spectra. Once the sound horizon is reduced by about one percent, the acoustic peak pattern acquires a small, coherent oscillatory residual relative to $\Lambda$CDM in the damping tail. The period and phase of this residual are fixed by the shift in $r_s$; \textbf{there are no additional nuisance parameters to tune the wiggles against the data}. Before comparing to ACT DR6, we used the best-fit Planck + BAO + distance ladder parameters to generate a template residual that is positive near $\ell \sim 800$, crosses zero around $\ell \sim 1200$, and turns negative at higher multipoles. This makes the ACT comparison a \emph{blind prediction test}: the shape of the template was fixed by Planck-calibrated geometry before we examined the ACT residuals.
...
This pattern is the spectral fingerprint of the pre-recombination shelf. The oscillating residuals---swinging from positive values near $\ell \sim 800$ through zero near $\ell \sim 1200$ to negative values at $\ell \gtrsim 2000$---represent the ``smoking gun'' of a geometric phase shift caused by the reduced sound horizon. $\Lambda$CDM cannot produce this coherent oscillatory pattern; random noise does not look like a sine wave. Only a geometric modification (changing $r_s$) creates this specific signature.

\subsubsection{ACT Already Shows the Pattern: The Blind Test Result}

\textbf{The key finding is that ACT DR6 residuals already exhibit the exact oscillatory structure predicted by the Ridder field.} When we subtract the $\Lambda$CDM best fit from the ACT bandpowers and overlay the Ridder residual template, we find that the ACT data follow this predicted phase structure within their current one-sigma uncertainties. Specifically:
\begin{itemize}
    \item Positive deviations in the shoulder region ($\ell \sim 800$--$1200$) at the $\sim 1\%$ level
    \item A sign change near $\ell \sim 1200$--$1500$
    \item Negative deviations in the deep damping tail ($\ell \gtrsim 2000$) at the $\sim 1$--$2\%$ level
\end{itemize}
This is precisely the phase structure predicted by a sound horizon reduction of $\Delta r_s \approx -1$ Mpc. The amplitude of the observed residuals ($\sim 1\%$) matches the theoretical prediction within current uncertainties.

The alignment in phase is nontrivial. Random noise would produce residuals with phases that vary from bin to bin, whereas the Ridder template specifies the sign pattern in advance. In this sense ACT provides a first blind test of the geometric mechanism: the same field configuration that moves $H_0$ into the 69--71 window also predicts the shape of the small oscillation that ACT sees at the edge of its sensitivity.

We stress that this is not a detection claim---the ACT error bars are comparable to the signal size. But the data are \textit{consistent with} seeing the Ridder field signature at the predicted level, with the correct phase structure. The residuals do not scatter randomly around zero; they follow the oscillatory template that the reduced sound horizon demands.

\subsubsection{Implications for CMB-S4: From Pattern Matching to Detection}
...
This motivates a simple forecast: a CMB experiment with an order of magnitude better precision in the damping tail will either detect this soft shoulder with high significance or rule out the Ridder class of models. In the former case the Planck high-$\ell$ ``tax'' we see today would be reinterpreted as the first low-significance glimpse of a real physical feature. In the latter case the field configuration studied here would be excluded, and the geometric route to $H_0 \approx 70$ km s$^{-1}$ Mpc$^{-1}$ would have to be abandoned. Either outcome is scientifically valuable.
...
\begin{table}[ht]
\centering
\caption{$\chi^2$ breakdown by likelihood component: EDE versus $\Lambda$CDM. The ``triangular tension'' is visible: Planck high-$\ell$ consistently disfavors EDE, while Planck low-$\ell$ EE and SH0ES favor it. DESI's direct contribution is nearly neutral; its effect is to shift parameters so that the low-$\ell$ benefits shrink.}
