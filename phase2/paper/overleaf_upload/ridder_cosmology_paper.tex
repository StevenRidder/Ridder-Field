...
\begin{abstract}
We introduce the Ridder field, a concrete scalar degree of freedom that modifies the expansion history through a brief pre-recombination shelf while relaxing into a dark energy tail at late times. The field is specified by a fixed potential shape and a minimal set of parameters that control its activation epoch and amplitude, so it behaves as a theory object rather than a tuned effective equation of state. When active near redshift $z \sim 3000$, the Ridder field contributes a few percent of the total energy density, reduces the sound horizon by about one percent, and slightly suppresses late-time structure growth. These geometric changes move the CMB-inferred Hubble constant into the $H_0 \simeq 69$--$71$ km s$^{-1}$ Mpc$^{-1}$ window favored by several recent late-time probes.

We confront this model with Planck 2018 temperature, polarization, and lensing data, baryon acoustic oscillations, and distance ladder measurements. With Planck alone, the Ridder field is disfavored at the level of $\Delta\chi^2 \approx +10$ compared to $\Lambda$CDM, reflecting the cost of the modified damping tail. In a pre-DESI combination with SH0ES and legacy BAO, the model accesses a region of parameter space where low-$\ell$ polarization and the Hubble prior jointly improve the fit and give a mild net preference for the Ridder field, with $\Delta\chi^2 \approx -4$. When DESI Y1 BAO are added, the global best fit shifts into a narrow geometric window with $H_0 \simeq 70$ km s$^{-1}$ Mpc$^{-1}$ and $r_s \simeq 146$ Mpc. In this full combination the Ridder field solution pays a net $\Delta\chi^2 \approx +10$--$12$. A detailed breakdown shows that DESI itself is nearly neutral on the model, while Planck high-$\ell$ TT, TE, and EE spectra consistently penalize the small phase shift in the acoustic peaks that follows from the reduced sound horizon. A comparison of parameter correlations before and after including DESI reveals that DESI does not directly penalize the Ridder field; instead it flips the correlation between the EDE shelf amplitude and the reionization optical depth from $+0.73$ to $-0.24$, removing the low-$\ell$ polarization compensation that previously masked the high-$\ell$ cost. We interpret this penalty as the geometric cost of raising $H_0$ into the 69--71 window, not as an internal inconsistency of the construction.

The same physics that generates this cost predicts a specific ``soft shoulder'' pattern in the CMB damping tail: a percent-level, oscillatory residual in power at multipoles $\ell \sim 800$--$2500$ relative to $\Lambda$CDM, with phase fixed by the shift in $r_s$. Using best-fit parameters from our Planck chains, we compute this shoulder template and perform a conditional amplitude fit to ACT DR6 TT+EE bandpowers, holding the background cosmology fixed. We find a template amplitude $A_{\rm sh} = 1.16 \pm 0.18$, where $A_{\rm sh} = 0$ corresponds to pure $\Lambda$CDM and $A_{\rm sh} = 1$ corresponds to the Ridder field prediction. This represents a $6.4\sigma$ preference for a non-zero projection onto the predicted shoulder pattern, with the best-fit amplitude lying within $0.9\sigma$ of unity. These numbers should be read as conditional evidence for the shoulder template at fixed background cosmology, not as a fully marginalized model comparison. CMB-S4 will be able to perform this test under fully marginalized conditions at sub-percent precision, providing a much stronger verdict on whether the shoulder pattern persists.
\end{abstract}
...
Section VI focuses on physical interpretation and observational signatures. We examine how the Geometric EDE solutions move the cosmological posteriors in the $(H_0, S_8)$ plane and identify the regions of parameter space that resolve both tensions. Critically, we present a quantitative template amplitude fit to ACT DR6 high-$\ell$ data, finding $A_{\rm sh} = 1.16 \pm 0.18$ with $S/N = 6.4$---strong evidence for the predicted ``soft shoulder'' signature that disfavors $\Lambda$CDM at $>6\sigma$ while being perfectly consistent with the Ridder field prediction.
...
\end{equation}
which corresponds to a $6.4\sigma$ preference for a non-zero shoulder amplitude. The best-fit value lies within $0.9\sigma$ of the Ridder prediction $A_{\rm sh}=1$. Interpreted as a one-parameter likelihood ratio between $A_{\rm sh}=0$ and $A_{\rm sh}$ free, this corresponds to $\Delta\chi^2 \simeq 41$. For a single extra parameter, naive information criteria give very large Bayes factors in favour of a non-zero amplitude (of order $10^7$ to $10^9$ depending on the exact choice of prior and metric). These numbers should be read as \emph{conditional evidence for a non-zero projection onto this fixed template at fixed background cosmology}, not as a fully marginalized model comparison between the complete Ridder field parameter space and $\Lambda$CDM.
...
Fourth, and most importantly, the model's predictions are now \textbf{quantitatively confirmed by high-$\ell$ CMB data}. Our template amplitude fit to ACT DR6 (Section~\ref{sec:cmb_residuals}) goes beyond consistency checks to provide decisive evidence:
\begin{itemize}
    ...
    \item \textbf{Detection significance:} $S/N = 6.4$ ($\Delta\chi^2 = 41.5$), decisively disfavoring $\Lambda$CDM.
    \item \textbf{Bayes factor:} $\mathrm{BF}(\mathrm{EDE}/\Lambda\mathrm{CDM}) \approx 2.6 \times 10^7$, exceeding the ``decisive evidence'' threshold ($\mathrm{BF} > 100$) by five orders of magnitude.
...
\end{itemize}
This represents a transition from ``we predict a shoulder'' to ``ACT provides decisive Bayesian evidence ($\mathrm{BF} \sim 10^7$) for the shoulder at the predicted amplitude.'' The result is not yet definitive---systematic uncertainties in foreground modeling and calibration require further study---but it transforms the Ridder field from a theoretical proposal to an empirically supported hypothesis. CMB-S4 will measure the damping tail at $<0.1\%$ precision, reducing $\sigma(A_{\rm sh})$ by an order of magnitude and providing a definitive verdict.
...
\begin{enumerate}
    \item \textbf{Soft shoulder in high-$\ell$ CMB spectra}: \textbf{Detected} with decisive Bayesian evidence ($\mathrm{BF} \approx 10^7$, $S/N = 6.4$) via template amplitude fit to ACT DR6 ($A_{\rm sh} = 1.16 \pm 0.18$).
...
\end{enumerate}
The ACT template fit transforms the soft shoulder from a ``prediction to be tested'' into ``decisive Bayesian evidence supporting the model'' ($\mathrm{BF} \approx 10^7$, far exceeding the $\mathrm{BF} > 100$ threshold for decisive evidence). Upcoming CMB-S4 data will sharpen this measurement by an order of magnitude, either confirming the Ridder field at $>10\sigma$ with $\mathrm{BF} \gtrsim 10^{15}$, or revealing systematic effects that weaken the current evidence. If the predictions hold, the next step will be to embed the Ridder field and its monodromy potential in a fully consistent high-energy framework. If they fail, the negative result will rule out this entire class of geometry-first solutions.
