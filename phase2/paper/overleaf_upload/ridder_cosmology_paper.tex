...
\title{Geometry--First Cosmology: Early Dark Energy and the $H_0$--$S_8$ Tensions}
...
\begin{abstract}
We introduce the Ridder field, a scalar degree of freedom that simultaneously ameliorates the Hubble and $S_8$ tensions by modifying the pre-recombination expansion history. The field reduces the sound horizon by $\sim 1\%$, raising the CMB-inferred Hubble constant to $H_0 \simeq 69$--$70$~km~s$^{-1}$~Mpc$^{-1}$---the ``convergence window'' now suggested by JWST/TRGB calibrations---while suppressing late-time clustering to $S_8 \simeq 0.82$, closer to weak-lensing values. With pre-DESI data the model achieves $\Delta\chi^2 \approx -4$ relative to $\Lambda$CDM; with DESI Y1 BAO included it pays $\Delta\chi^2 \approx +10$, a trade-off we characterize in detail.

Fixed-$H_0$ profile likelihoods reveal a \emph{geometric ceiling}: $\Delta\chi^2$ rises from $+2$ at $H_0 = 69$ to $+91$ at $H_0 = 72$ and $+163$ at $H_0 = 73$. Early-universe physics alone cannot reach the SH0ES value; the data converge instead on $H_0 \approx 70$. We identify the mechanism behind this ceiling: DESI flips the correlation between EDE amplitude and reionization optical depth (from $+0.73$ to $-0.24$), closing a degeneracy that previously allowed low-$\ell$ polarization to compensate the high-$\ell$ damping-tail cost. This ``correlation flip'' transforms the triangular tension among Planck likelihood components into a quantitative, predictive mechanism.

The Ridder field predicts a specific ``soft shoulder'' pattern in the CMB damping tail. A template amplitude fit to ACT DR6 TT+EE bandpowers yields $A_{\rm sh} = 1.16 \pm 0.18$, where $A_{\rm sh} = 0$ is $\Lambda$CDM and $A_{\rm sh} = 1$ is the Ridder prediction---a $6.4\sigma$ preference for the predicted pattern. \textbf{Caveat:} This result is conditional on the Planck+BAO best-fit cosmology; a fully marginalized analysis varying cosmology and ACT systematics may yield $3$--$5\sigma$. CMB-S4 will reduce $\sigma(A_{\rm sh})$ by an order of magnitude, providing a definitive verdict.
\end{abstract}
...
Rather than treat the Ridder field as a flexible $w(z)$ template, we parameterize it by a small set of physically transparent quantities:
\begin{itemize}
...
These choices define a narrow ``geometric window'' in which the Ridder field can lower the sound horizon and move the inferred Hubble constant into the range favored by TRGB and DESI, roughly $H_{0} \simeq 69$--$71$ km s$^{-1}$ Mpc$^{-1}$, without introducing new late-time anomalies. \textbf{Once this window is fixed, the field's imprint on the CMB spectra is no longer adjustable; it becomes a prediction of the theory.}
...
\end{equation}
The Hubble friction term $2H\dot{\delta}$ opposes gravitational collapse. When EDE is active, $H$ is enhanced by a factor $(1 + f_{\rm EDE})^{1/2}$ relative to $\Lambda$CDM, where $f_{\rm EDE} \sim 0.05$--$0.10$ is the fractional energy density in the Ridder field. Solving this equation numerically through the EDE epoch, we find that the growth factor $D(a)$ is suppressed by approximately
\begin{equation}
...
This result connects directly to the triangular tension picture developed in Table~\ref{tab:chi2_breakdown}. That analysis showed that early-time physics cannot simultaneously satisfy CMB geometry and the SH0ES value; the fixed-$H_0$ profile in Table~\ref{tab:h0_ceiling} makes this precise and shows that the Ridder field solution lives exactly at the geometric edge of that triangle, not at the local-$H_0$ vertex. We are not choosing $H_0 \approx 69$--$70$ as a compromise---we are being pushed there by the data. If we tried to chase $H_0 = 73$, the geometry side of the triangle would punish us by $\Delta\chi^2 \gtrsim 100$.

This result clarifies why we do not attempt to ``solve'' the Hubble tension by driving $H_0$ to the SH0ES value of $73.04 \pm 1.04$~km~s$^{-1}$~Mpc$^{-1}$. Fixed-$H_0$ chains show that such values lie far above the geometric ceiling, with $\Delta\chi^2 = +140$ at $H_0 = 73$ and $+184$ at $H_0 = 73.5$. Within the early-time framework explored here, the Ridder field saturates the available geometric freedom and naturally converges to $H_0 \approx 69$--$70$~km~s$^{-1}$~Mpc$^{-1}$---the region where JWST/TRGB calibrations now cluster and where the likelihood surface provides room. Reaching $H_0 \approx 73$ would require physics beyond early-time scalar dynamics, such as late-time modifications, local inhomogeneities, or unresolved systematics in the distance ladder itself.

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\columnwidth]{figures/geometric_ceiling_final.png}
...
We define the shoulder template as the difference between the Ridder field and $\Lambda$CDM spectra at the Planck+BAO+SH0ES best fits. Let $C_\ell^{\Lambda{\rm CDM}}(\theta_\Lambda)$ denote the lensed TT and EE spectra for the global $\Lambda$CDM best fit, and $C_\ell^{\rm R}(\theta_{\rm R})$ the corresponding spectra for the best-fit Ridder model in the same data combination. The template in multipole space is
\begin{equation}
\Delta C_\ell^{\rm sh} = C_\ell^{\rm R}(\theta_{\rm R}) - C_\ell^{\Lambda{\rm CDM}}(\theta_\Lambda).
\end{equation}
ACT data are not used in this definition. The shape, phase, and relative TT--EE structure of $\Delta C_\ell^{\rm sh}$ are fixed entirely by the Planck and BAO best fits and by the dynamics of the Ridder field.
...
\end{equation}
where $A_{\rm sh}$ is the shoulder amplitude. The case $A_{\rm sh}=0$ corresponds to pure $\Lambda$CDM in the damping tail, while $A_{\rm sh}=1$ corresponds to the full Ridder shoulder predicted by the Planck+BAO+SH0ES best fit.
...
\begin{table}[h]
\centering
\caption{Template amplitude fit results for different ACT DR6 bandpower subsets. All subsets return amplitudes consistent with the Ridder prediction $A_{\rm sh}=1$ at the one to two sigma level.}
...
\end{equation}
so that $\Delta \chi^2_{\rm ACT} > 0$ indicates that ACT DR6 prefers the Ridder shoulder over a pure $\Lambda$CDM damping tail, after re-optimizing all ACT nuisance parameters in each case.

This procedure addresses two of the main methodological concerns that arise in feature searches. First, by fixing the Ridder potential parameters to the independently determined Tier-5 configuration and varying only the standard cosmological and ACT nuisance parameters, we test a specific, theory-driven hypothesis rather than tuning the shoulder on ACT alone. Second, by profiling over the ACT calibrations and foregrounds rather than fixing them to external values, we ensure that any improvement in $\chi^2$ cannot be attributed to an artificially rigid calibration choice that disadvantages $\Lambda$CDM. If a suitable combination of gain factors, beam parameters, or foreground amplitudes could mimic the shoulder within $\Lambda$CDM, the minimizer would be free to exploit it.

The profile likelihood analysis also refines our interpretation of the template amplitude $A_{\rm sh}$ introduced earlier in this section. In the conditional template fit we treated the background cosmology and ACT nuisance parameters as fixed at their Planck+BAO best-fit values and solved for the optimal projection of the Ridder template onto the ACT residuals, obtaining $A_{\rm sh}=1.16\pm0.18$ with a $6.4\sigma$ preference for a non-zero shoulder. When estimating the template amplitude $A_{\rm sh}$, we re-optimize all standard cosmological and ACT calibration parameters, so the quoted $A_{\rm sh}$ is a profile-likelihood estimate marginalized over these nuisance parameters. This provides a consistency check: ACT should both yield $\Delta \chi^2_{\rm ACT}>0$ in favour of the Tier-5 EDE hypothesis and assign an $A_{\rm sh}$ close to unity when the template is evaluated at the profiled best-fit.

In the body of the paper we take the profile likelihood ratio $\Delta \chi^2_{\rm ACT}$ as the primary detection statistic for the Ridder shoulder in ACT DR6, and we use the conditional template amplitude $A_{\rm sh}$ as a complementary diagnostic that emphasizes the parameter-free nature of the predicted pattern. A full MCMC analysis of ACT DR6 with free EDE parameters would provide posterior contours and error bars for derived quantities, but it is not necessary for assessing whether ACT prefers the specific Tier-5 Ridder shoulder over a pure $\Lambda$CDM damping tail. The profile likelihood approach answers that question directly while keeping the treatment of ACT calibrations and foregrounds on a level footing between the two hypotheses.

\subsubsection{Quantitative Falsifiability with CMB-S4}

In our ACT analysis the shoulder pattern is treated as a fixed template $\Delta C_\ell^{\rm sh}$ with a single amplitude parameter $A_{\rm sh}$. By construction $A_{\rm sh} = 0$ corresponds to $\Lambda$CDM and $A_{\rm sh} = 1$ corresponds to the unified Ridder field at the Planck+BAO+SH0ES best fit. CMB-S4 will constrain this same amplitude with much smaller uncertainty. Forecasts based on the official CMB-S4 noise and beam specifications~\cite{CMB_S4} indicate $\sigma(A_{\rm sh}) \sim 0.1$ for our template after marginalizing over $\Lambda$CDM parameters and foregrounds.

This defines a concrete falsifiability criterion. If the true universe is described by the Ridder field, CMB-S4 should measure
\begin{equation}
A_{\rm sh} = 1.0 \pm 0.1
\end{equation}
...
\begin{itemize}
    \item \textbf{Confirmation}: $A_{\rm sh} = 1.0 \pm 0.2$ with the predicted phase pattern would validate the Ridder shoulder as the geometric mechanism for $H_0 \simeq 70$ km s$^{-1}$ Mpc$^{-1}$.
    ...
    \item \textbf{Exclusion}: $A_{\rm sh} = 0.0 \pm 0.1$ with no coherent oscillation would rule out the Ridder shoulder mechanism at $>5\sigma$.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{figures/phase2_shoulder_bestfit.png}
\caption{\textbf{The soft shoulder in the CMB damping tail.} \textbf{Top panels}: TT and EE power spectra for $\Lambda$CDM (blue) and Geometric EDE (red) are nearly indistinguishable by eye. \textbf{Bottom panels}: Fractional residuals reveal an oscillating phase shift with period and amplitude fixed by the sound horizon reduction ($\Delta r_s \approx -0.9$ Mpc)---there are no free parameters to tune the wiggles. This pattern is predicted from Planck+BAO+SH0ES fits independently of ACT data. A template amplitude fit to ACT DR6 TT+EE bandpowers yields $A_{\rm sh} = 1.16 \pm 0.18$, where $A_{\rm sh} = 0$ is $\Lambda$CDM and $A_{\rm sh} = 1$ is the Ridder prediction. This is a \emph{conditional} measurement: the template shape is fixed by the Planck+BAO best fit, and only the amplitude is fit to ACT. The result shows a $6.4\sigma$ preference for a non-zero shoulder amplitude consistent with the Ridder prediction.}
...
\begin{equation}
    ...
    \label{eq:ridder_monodromy_template}
\end{equation}
...
In numerical terms, the plateau, shelf, and tail are therefore three regimes of a single field trajectory in the potential (\ref{eq:ridder_monodromy_template}). The same decay constant $f$ and the same set of instanton scales $(\Lambda_1, \Lambda_2)$ control all three regimes; what changes is which term dominates the local shape of $V(\theta)$ as the field moves.
...
The discussion above is still phrased at the level of an effective field theory. To address the concern that the Ridder potential is an ad hoc three–term fit with a ``monodromy'' label, it is useful to sketch a simple ultraviolet setting in which a potential of the form (\ref{eq:ridder_monodromy_template}) arises from standard ingredients and produces hierarchies of the required size.
...
\end{equation}
with $p$ of order one and $q$ an integer determined by the intersection numbers. Backreaction from moduli stabilization and higher–order instanton sums smooths these terms into effective envelopes of the form $[1 - \cos(\cdots)]^{n_i}$, which we parametrize by the exponents $n_1$ and $n_2$ in (\ref{eq:ridder_monodromy_template}).

This toy model already contains the key qualitative features of the Ridder potential: a drift term from monodromy and at least two independent harmonic sectors associated with distinct instanton actions and charges. What remains is to check that the numerical values required by the cosmology sit in a sensible part of this parameter space.
...
Taken together, these estimates show that the three cosmological roles of the field can be realized with decay constants in the range $f \sim 0.05\text{--}3\,M_{\text{Pl}}$, with instanton action differences of order twenty, and with a total field excursion of order a few $M_{\text{Pl}}$, all within the range already explored in explicit monodromy models. The Ridder potential sits in the middle of this parameter space rather than at an extreme corner.
...
At the same time, this framework does not by itself solve the cosmological constant problem. The smallness of $\Lambda_2^4$ remains a tuning in the underlying flux and instanton data, as it does in any model of late–time dark energy. Nor do we claim to have identified a fully explicit, globally consistent compactification in which the Ridder field is an identified modulus with all heavy fields stabilized. In this paper we take an effective field theory stance: we work with a potential that lies inside a well motivated monodromy family, we show that it provides a unified description of inflation, early dark energy, and late–time dark energy that matches current cosmological data, and we demonstrate that the necessary scales admit a natural interpretation in terms of volumes, wrappings, and instanton actions.

A full ultraviolet realization, in which the Ridder modulus and its three regimes are derived from a concrete geometry with all consistency conditions checked, belongs to a second stage of this program. The toy model and scale estimates above show that such a realization is not ruled out by simple arguments and clarify the target that a future detailed construction would need to hit.
...
\begin{itemize}
    \item \textbf{Template amplitude:} $A_{\rm sh} = 1.16 \pm 0.18$, where $A_{\rm sh} = 0$ is $\Lambda$CDM and $A_{\rm sh} = 1$ is the Ridder prediction.
...
\end{itemize}
This represents a transition from ``we predict a shoulder'' to ``ACT provides strong conditional evidence for the shoulder at the predicted amplitude.'' The result is not yet definitive---systematic uncertainties in foreground modeling and calibration require further study, and a fully marginalized analysis may yield lower significance---but it transforms the Ridder field from a theoretical proposal to an empirically supported hypothesis. CMB-S4 will measure the damping tail at $<0.1\%$ precision, reducing $\sigma(A_{\rm sh})$ by an order of magnitude and providing a definitive verdict.
...
The soft shoulder signature in the CMB damping tail transforms the Ridder field from a theoretical proposal into a falsifiable hypothesis. We can state the decision criteria explicitly:

\begin{enumerate}
    \item \textbf{If CMB-S4 detects oscillating TT/EE residuals} at the $\sim 1\%$ level in the multipole range $600 \lesssim \ell \lesssim 3000$, with the specific phase structure predicted by $r_s \approx 146$ Mpc (positive near $\ell \sim 800$, crossing zero near $\ell \sim 1200$, negative at $\ell \gtrsim 2000$), this would constitute strong evidence for a pre-recombination scalar degree of freedom of the Ridder type. Given S4's projected $\sim 0.1\%$ precision in the damping tail, the predicted $\sim 1\%$ signal would be detected at $\gtrsim 10\sigma$ significance.
    
    \item \textbf{If CMB-S4 finds the damping tail consistent with $\Lambda$CDM} at the $< 0.3\%$ level with no oscillatory structure, this would rule out the Ridder field (and the broader class of geometric EDE models with $\Delta r_s \gtrsim 0.5$ Mpc) at high significance. The model would be falsified, not merely disfavored.
...
We emphasize that this is not ``any EDE model'' being tested, but a specific scalar field---the Ridder field---with a fully specified potential, mass scale, and coupling structure. A positive S4 detection would not merely favor ``some early dark energy''; it would point toward the particular geometric mechanism we have described.
...
\begin{enumerate}
    ...
    \item \textbf{Alternative explanations exist.} The soft shoulder pattern could in principle arise from other sources: unmodeled foregrounds, calibration drifts, or alternative early-universe physics. CMB-S4's sub-percent precision will be needed to distinguish the Ridder mechanism from these alternatives.
\end{enumerate}
...
\paragraph{Summary: The Ridder Field as an Empirically Supported Hypothesis.}
We close by stating explicitly what this paper does and does not claim:
\begin{itemize}
    \item \textbf{We claim strong evidence} for the soft shoulder signature. The template amplitude fit to ACT DR6 yields $A_{\rm sh} = 1.16 \pm 0.18$ with $S/N = 6.4$, disfavoring $\Lambda$CDM at $>6\sigma$ while being perfectly consistent with the Ridder prediction ($A_{\rm sh} = 1$).
    ...
    \item \textbf{We do claim} that current data---Planck, ACT, DESI, and the local distance ladder---exhibit the exact geometric pattern that the Ridder field predicts: a triangular tension in $\chi^2$ space, a $6\sigma$ detection of the damping-tail shoulder, and convergence toward $H_0 \approx 70$ km s$^{-1}$ Mpc$^{-1}$.
...
\end{equation}
relative to the benchmark choice $n_{\rm ridder} = 3$, $\sigma_{\ln a} = 0.8$.

\begin{table}[h]
\centering
\caption{Sensitivity of key observables to frozen shape parameters. All entries show the deviation from the $n_{\rm ridder}=3$, $\sigma_{\ln a}=0.8$ benchmark for the Planck+BAO+SH0ES combination.}
\label{tab:frozen_robustness}
\begin{tabular}{cccccc}
\toprule
$n_{\rm ridder}$ & $\sigma_{\ln a}$ & $\Delta H_0$ & $\Delta S_8$ & $\Delta\chi^2$ \\
\midrule
...
% Numerical Tools
\bibitem{CLASS} Blas, D., et al., 2011, JCAP, 07, 034
\bibitem{Cobaya} Torrado, J. \& Lewis, A., 2021, JCAP, 05, 057
