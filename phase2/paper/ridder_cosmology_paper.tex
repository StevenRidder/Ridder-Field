...
\label{eq:cs2}
\end{equation}
which is manifestly positive. Combined with the standard kinetic term in the action, this ensures:
\begin{itemize}
\item No ghost instabilities ($\dot{\phi}^2/2$ has the correct sign).
\item No gradient instabilities ($k^2/a^2$ coefficient is positive).
\item Subluminal propagation ($c_s \leq 1$).
\end{itemize}
These properties hold throughout the evolution, confirming that the model is free of pathological instabilities at the linear level.
...
The following likelihood modules are used throughout this analysis:

\begin{itemize}
\item \textbf{Planck 2018 CMB:}
    \begin{itemize}
    \item High-$\ell$ TT/TE/EE: \texttt{planck\_2018\_highl\_plik.TTTEEE\_lite} ($\ell = 30$--$2508$)
    \item Low-$\ell$ TT: \texttt{planck\_2018\_lowl.TT} ($\ell = 2$--$29$)
    \item Low-$\ell$ EE: \texttt{planck\_2018\_lowl.EE} ($\ell = 2$--$29$)
    \item Lensing: \texttt{planck\_2018\_lensing.clik} ($L = 8$--$400$)
    \end{itemize}

\item \textbf{ACT DR6:}
    \begin{itemize}
    \item TT/TE/EE: \texttt{act\_dr6\_mflike} ($\ell = 600$--$4000$)
    \item Foreground model: thermal SZ, kinetic SZ, CIB, radio point sources
    \item Nuisance parameters: $\sim$20 (calibration, foregrounds per frequency)
    \end{itemize}

\item \textbf{BAO (pre-DESI):}
    \begin{itemize}
    \item 6dFGS: $D_V(z=0.106)/r_d$~\cite{6dFGS}
    \item SDSS MGS: $D_V(z=0.15)/r_d$~\cite{MGS}
    \item BOSS DR12: $D_M(z)/r_d$, $H(z)r_d$ at $z = 0.38, 0.51, 0.61$~\cite{BOSS_DR12}
    \item eBOSS LRG: $D_M(z=0.70)/r_d$, $H(z=0.70)r_d$~\cite{eBOSS}
    \end{itemize}

\item \textbf{BAO (DESI Y1):}
    \begin{itemize}
    \item BGS: $z_{\rm eff} = 0.30$
    \item LRG: $z_{\rm eff} = 0.51, 0.71$
    \item ELG: $z_{\rm eff} = 1.32$
    \item QSO: $z_{\rm eff} = 1.49$
    \item Ly$\alpha$: $z_{\rm eff} = 2.33$
    \end{itemize}

\item \textbf{Local $H_0$ priors:}
    \begin{itemize}
    \item SH0ES: $H_0 = 73.04 \pm 1.04$~km~s$^{-1}$~Mpc$^{-1}$~\cite{SH0ES}
    \item TRGB-CCHP: $H_0 = 69.8 \pm 1.7$~km~s$^{-1}$~Mpc$^{-1}$~\cite{Freedman2024}
    \end{itemize}

\item \textbf{Supernovae:} Pantheon+ compilation~\cite{Pantheon}
\item \textbf{Weak lensing (extended):} DES Y1 3$\times$2pt~\cite{DES_Y1}
\end{itemize}
...
We adopt stricter tolerances than CLASS defaults to ensure sub-percent accuracy in the damping tail:
\begin{itemize}
\item Background precision: \texttt{tol\_background\_integration = 1e-4}
\item Perturbation precision: \texttt{tol\_perturb\_integration = 1e-6}
\item Multipole sampling: $\ell_{\rm max} = 5000$, $\Delta\ell = 1$ for $\ell > 2000$
\item $k$-sampling: increased density at $k > 0.1$~Mpc$^{-1}$
\end{itemize}
The $\Lambda$CDM spectra from our modified CLASS agree with unmodified CLASS at the 0.1\% level across all multipoles.
