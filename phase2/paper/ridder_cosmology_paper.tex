...
\subsection*{The Sound Horizon Puzzle: ACT vs DESI}

The geometric EDE shelf has two tightly linked consequences. It reduces the sound horizon by order one percent, from the $\Lambda$CDM value $r_s \simeq 147.5$ Mpc to $r_s \simeq 146$ Mpc, and it induces a coherent ``soft shoulder'' pattern in the CMB damping tail at $\ell \gtrsim 600$. In the pre-DESI analysis, these two facts pointed in the same direction: a smaller $r_s$ eased the Hubble tension, and the corresponding shoulder was small enough to lie inside Planck's small-scale error bars.

Once DESI Y1 BAO is added, the situation becomes more subtle. DESI constrains the combination of distances and the sound horizon that enters the BAO scale, and in our fits it clearly prefers a sound horizon close to the $\Lambda$CDM value. For the minimal Geometric EDE model, this preference shows up as a geometry penalty of $\Delta\chi^2 \sim +15$ when DESI is added to the SH0ES-anchored world, even though the model continues to sit at $H_0 \simeq 70$ km s$^{-1}$ Mpc$^{-1}$ with a modest reduction in $S_8$.

ACT DR6 probes the same physical scale in a very different way. It is sensitive to the detailed phase and amplitude of the acoustic peaks in the damping tail, rather than to the BAO standard ruler at low redshift. When we fit Geometric EDE and $\Lambda$CDM to Planck+ACT and construct residuals, the predicted shoulder pattern \textbf{appears in the data}: oscillating residuals at the one-to-two percent level, with sign changes near $\ell \sim 1200$ and the characteristic ``soft shoulder'' morphology that a reduced sound horizon implies. The amplitude and phase of this pattern match the theoretical prediction. Current ACT precision cannot yet distinguish this signal from statistical fluctuation at high significance---the residuals lie within the error envelope---but the pattern is present, coherent, and in the right place. This defines both a consistency check (the model is not excluded) and a sharp target for CMB-S4, which will either confirm the shoulder as a detection or rule it out definitively.

From this point of view, the apparent conflict between ``ACT sees the shoulder'' and ``DESI prefers a large $r_s$'' is better described as a genuine multi-probe tension in the $(H_0, r_s)$ plane. DESI currently favors a calibration close to $\Lambda$CDM, while Planck, the local distance ladder and weak-lensing surveys point toward a solution with slightly higher $H_0$, slightly lower $S_8$, and a modestly reduced sound horizon. Geometric EDE chooses the latter corner and pays a well-quantified $\chi^2$ cost on DESI to do so. The decisive test will come from the next generation of CMB small-scale measurements: if CMB-S4 detects a damping-tail shoulder at the level predicted here, the reduced sound horizon will be strongly favored and the DESI preference will have to be revisited. If instead the damping tail is measured to be featureless at the sub-percent level, the geometric EDE shelf will be ruled out and the DESI-anchored $\Lambda$CDM calibration will win.
