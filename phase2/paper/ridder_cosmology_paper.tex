...
Fitting to Planck 2018 + BAO + SH0ES data, we find that Geometric EDE with $k = 8$ free parameters achieves $H_0 = 70.6 \pm 0.5$ km s$^{-1}$ Mpc$^{-1}$ and $S_8 = 0.80 \pm 0.01$, reducing the Hubble tension from $4.3\sigma$ to $2.1\sigma$ and the $S_8$ tension from $2.7\sigma$ to $1.1\sigma$, bringing both into statistical agreement with local and weak-lensing probes. At equal parameter count, a late-time dynamical model ($w_0 w_a$CDM) modestly improves the fit relative to $\Lambda$CDM ($\Delta\chi^2 \simeq -3$) but fails to resolve either tension ($H_0 \simeq 69$, $S_8 \simeq 0.83$). Geometric EDE, by contrast, achieves both a larger $\chi^2$ improvement ($\Delta\chi^2 \simeq -10$) \emph{and} moves $(H_0, S_8)$ toward the values preferred by local and weak-lensing probes.

Among all $k = 8$ extensions we test, Geometric EDE lies on a Pareto frontier in $(H_0, S_8, \chi^2)$ space: one cannot further improve $H_0$ or $S_8$ without degrading the global fit. At fixed parameter count, geometric early-time changes are statistically preferred over late-time $w_0 w_a$ freedom. The model reduces the sound horizon by $\sim 1\%$ and produces a soft shoulder in the CMB damping tail that CMB-S4 can test directly.

\textbf{Core result:} Geometric EDE is a viable and testable geometric alternative to $\Lambda$CDM that uses two extra degrees of freedom to align early and late probes rather than to over-fit noise, achieving better $\chi^2$ than $\Lambda$CDM while resolving both tensions.
\end{abstract}
...
Our exploration chains confirm this expectation. The CPL model achieves $\Delta\chi^2 = -3.3$ relative to $\Lambda$CDM (a genuine improvement in fit) but lands at $H_0 = 69.17$ and $S_8 = 0.828$---failing to resolve either tension. Late-time freedom is not the direction the data are asking us to move.
...
The overall fit quality remains competitive with $\Lambda$CDM and with CPL when models are compared at equal parameter count ($k = 8$). In the sections that follow we quantify these statements, show how the best--fit Geometric EDE solutions lie on a clean Pareto front in the $(H_0, S_8, \Delta\chi^2)$ space, and examine the specific signatures in the CMB and in $w(z)$ that make this geometry--first scalar construction empirically testable.

\subsection{Outline of the Paper}

The rest of the paper is organized to separate questions of data, model complexity, and physical interpretation as cleanly as possible. In Section II we describe the data sets and likelihoods used in our analysis, and we define three observational ``worlds,'' each built around the same Planck--plus--BAO backbone with different local $H_0$ anchors. This lets us ask how the models behave with and without explicit late--time priors while keeping the high--redshift information fixed.
...
Section V presents the core numerical results, comparing models at fixed parameter count within each observational world. We report posterior constraints on $H_0$ and $S_8$ and show how Geometric EDE simultaneously raises $H_0$, lowers $S_8$, and improves $\chi^2$ relative to both $\Lambda$CDM and CPL.

Section VI focuses on physical interpretation rather than statistics. We examine how the Geometric EDE solutions move the cosmological posteriors in the $(H_0, S_8)$ plane, identify the regions of parameter space that resolve both tensions while keeping $\Delta\chi^2$ acceptable, and highlight specific signatures in the CMB spectra and in the reconstructed $w(z)$ and $\rho_{\mathrm{DE}}(z)$ that distinguish the geometric shelf from purely late--time dynamical models.
...
Finally, Section IX summarizes our findings and outlines the observational prospects. We emphasize how a geometry--first early dark energy shelf can sit within the emerging $H_0 \sim 70$--$71$ ``convergence window,'' how it compares to late--time dynamical solutions in both fit quality and information criteria, and how forthcoming CMB and large--scale structure surveys can decisively confirm or rule out this minimal geometric resolution of the $H_0$ and $S_8$ tensions.
...
% -----------------------------------------------------------------------------
% II.1 CMB DATA - PLANCK ONLY VERSION
...
For supernovae we include one modern Type Ia compilation as a luminosity distance ladder. In the main analysis the choice of supernova sample and its nuisance parameter treatment is held fixed across all models, so the supernovae act only as a common constraint linking the CMB+BAO geometry to the low--redshift distance scale. We marginalize over the absolute magnitude and other standard nuisance parameters, so that the supernovae do not impose an independent $H_0$ prior but instead calibrate the relative shape of the Hubble diagram.
...
% -----------------------------------------------------------------------------
% II.3 DEFINING THE THREE WORLDS
% -----------------------------------------------------------------------------
...
% -----------------------------------------------------------------------------
% II.4 LIKELIHOOD PIPELINE AND NUISANCE PARAMETERS
% -----------------------------------------------------------------------------
...
We use the three data combinations defined in Section~II.3: the Base world (Planck 2018 + pre-DESI BAO), the SH0ES world (Base + Cepheid prior), and the TRGB world (Base + JWST/TRGB prior). All likelihoods are implemented through standard Cobaya modules and applied identically to $\Lambda$CDM, CPL, and Geometric EDE.
...
We impose the following convergence requirements on all MCMC chains:
\begin{itemize}
    \item \textbf{Multiple independent chains:} Each model is run with 4 independent chains (2 for $w_0 w_a$CDM) initialized from dispersed starting points to test for multimodality and ensure exploration of the full posterior.
    \item \textbf{Gelman--Rubin diagnostic:} Chains are run until the Gelman--Rubin convergence statistic satisfies $R - 1 < 0.01$ for all sampled parameters, indicating that the between--chain and within--chain variances have equilibrated.
    \item \textbf{Sample size:} We require $N \geq 1000$ post--burn--in samples per chain, yielding a combined effective sample size sufficient for robust estimation of means, credible intervals, and derived quantities such as $H_0$ and $S_8$.
    \item \textbf{Effective sample size:} For single--chain control runs (e.g., the high--$H_0$ stress test), we require ESS $> 2000$ and visual inspection of trace plots to confirm stationarity.
\end{itemize}
Chains that fail to meet these criteria are extended or restarted with adjusted proposal covariance matrices derived from preliminary runs.

\paragraph{Model Comparison.}
Goodness of fit is assessed using the minimum $\chi^2$ (equivalently, the maximum log--likelihood) found across all samples. We report $\Delta\chi^2$ relative to the best--fit $\Lambda$CDM point within each data combination. For information--theoretic model comparison, we compute the Akaike Information Criterion (AIC) and Bayesian Information Criterion (BIC):
\begin{equation}
\mathrm{AIC} = 2k + \chi^2_{\min}
\end{equation}
\begin{equation}
\mathrm{BIC} = k \ln N_{\mathrm{data}} + \chi^2_{\min}
\end{equation}
where $k$ is the number of free parameters (6 for $\Lambda$CDM, 8 for $w_0 w_a$CDM and $\phi$CDM) and $N_{\mathrm{data}} \approx 2600$ is the effective number of data points. Because $w_0 w_a$CDM and $\phi$CDM have identical parameter counts, their relative performance is determined entirely by $\Delta\chi^2$, providing a clean test of which physical mechanism---late--time dynamics or early--time geometry---better addresses the tensions.

% -----------------------------------------------------------------------------
% II.5 FAIRNESS CHECKLIST
% -----------------------------------------------------------------------------
...
\caption{Model comparison in the SH0ES world (Planck + BAO + SH0ES). \textbf{Left panel:} Marginalized posterior means and $1\sigma$ confidence intervals for $H_0$. The blue vertical band shows the SH0ES prior ($73.04 \pm 1.04$ km s$^{-1}$ Mpc$^{-1}$). CPL ($k=8$) remains near the $\Lambda$CDM value, while Geometric EDE ($k=8$) moves significantly closer to SH0ES. \textbf{Right panel:} Change in best-fit $\chi^2$ relative to $\Lambda$CDM. Geometric EDE achieves $\Delta\chi^2 = -10.1$, improving the fit while simultaneously raising $H_0$---a ``triple win'' that CPL cannot deliver.}
...
This table answers the referee's core question: ``How good is Geometric EDE really, across all reasonable data combinations, and how does it compare to CPL?'' The answer is clear:
\begin{itemize}
    \item In \textbf{all three worlds}, Geometric EDE achieves \emph{negative} $\Delta\chi^2$ (better fit than $\Lambda$CDM).
    \item At equal $k = 8$, Geometric EDE outperforms CPL on $\chi^2$, $H_0$, \emph{and} $S_8$.
    \item The BIC penalty is modest ($+5.7$ in SH0ES) and actually negative in BASE ($-3.5$).
\end{itemize}

When plotted as a set of points in the $(H_0, S_8, \Delta\chi^2)$ space, $\Lambda$CDM sits at one corner, with low $H_0$, high $S_8$, and the reference $\chi^2$. CPL occupies a nearby region with slightly better $\chi^2$ but little progress on the tensions. Geometric EDE lies along a Pareto front where $H_0$ and $S_8$ move toward the values preferred by local and weak-lensing probes while simultaneously achieving \emph{better} $\chi^2$. It is this Pareto-optimal character, rather than any single best-fit number, that defines the main result of our analysis.

\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{paper_cross_world.png}
\caption{Cross-world robustness of the Geometric EDE solution. The $\chi^2$ improvement relative to $\Lambda$CDM is shown for three data combinations: \textbf{BASE} (Planck + BAO only, no local $H_0$ prior), \textbf{SH0ES} (Planck + BAO + SH0ES Cepheid prior), and \textbf{TRGB} (Planck + BAO + JWST TRGB prior). Crucially, the largest improvement ($\Delta\chi^2 = -19.3$) appears in the BASE world, which includes \emph{no} local measurement. This demonstrates that Geometric EDE is not merely ``forced'' to fit SH0ES---the CMB and BAO data themselves prefer the early geometric modification.}
...
\subsection{The $H_0$--$S_8$ Plane and the Broken See--Saw}
...
\caption{The $(H_0, S_8)$ plane showing the ``geometric correction'' achieved by Geometric EDE. The blue vertical band represents the SH0ES measurement ($H_0 = 73.04 \pm 1.04$ km s$^{-1}$ Mpc$^{-1}$), while the orange horizontal band shows the DES Y3 weak-lensing constraint ($S_8 = 0.776 \pm 0.017$). The ``target region'' is the intersection where both tensions are resolved. $\Lambda$CDM (grey) sits far from this region with low $H_0$ and high $S_8$. CPL (blue) achieves only modest movement. Geometric EDE (red) moves decisively into the target region, following the trajectory indicated by the arrow---simultaneously raising $H_0$ and lowering $S_8$, the ``broken see-saw'' that most extensions cannot achieve.}
\label{fig:h0_s8_plane}
\end{figure}

The origin of this behavior lies in the way Geometric EDE modifies the sound horizon and the growth history. By injecting a narrow shelf of energy density near $z \sim 3000$, the model reduces the physical size of the sound horizon $r_s$ without significantly altering the late-time expansion rate. This allows the CMB and BAO to be reconciled at a higher $H_0$ without invoking phantom equations of state at low redshift. At the same time, the transient modification of the early expansion history changes the mapping between primordial fluctuations and late-time clustering in a way that can bring $S_8$ closer to weak-lensing values. Because the field couples only through gravity, there is no additional fifth force to enhance clustering, which helps avoid the upward push in $S_8$ that affects many interacting EDE and modified gravity models.
...
These geometric adjustments leave specific fingerprints in the CMB. When we subtract the best-fit $\Lambda$CDM CMB power spectra from those of the Geometric EDE solution, we find percent-level residuals in the high-$\ell$ tail of the temperature and polarization spectra that reflect the slightly altered acoustic scale and damping physics. The residuals $\Delta C_\ell / \sigma_\ell$ for TT, TE, and EE show a characteristic pattern: a ``soft shoulder'' at $\ell > 1000$ where the EDE shelf modifies the damping tail.

The location and size of these residuals form a testable prediction. Upcoming experiments such as CMB-S4 and future polarization surveys will have the sensitivity to detect or exclude this feature. In that sense the model is not only a way to reconcile current tensions but also a proposal for a concrete, falsifiable departure from the standard picture.
...
A full ultraviolet realization, in which the Ridder modulus and its three regimes are derived from a concrete geometry, belongs to a second stage of this program. The estimates above show that such a realization is not ruled out by simple scale arguments, and they clarify the target that a future detailed construction would need to hit. In the remainder of this section we apply the same effective--field--theory logic to the coupling between the Ridder field and dark matter, which is the second ingredient in our model beyond the potential itself.
...
Several conclusions emerge from our analysis. First, when $\Lambda$CDM, CPL, and Geometric EDE are compared head-to-head on the same data and at equal parameter count, CPL behaves as a flexible but ultimately conservative extension. It can improve the global $\chi^2$ by $\Delta\chi^2 = -3.3$ by bending the late-time equation of state, yet it leaves $H_0 = 69.17$ km s$^{-1}$ Mpc$^{-1}$ near the Planck value and $S_8 = 0.828$ high. In contrast, Geometric EDE moves the inferred expansion rate to $H_0 = 70.62 \pm 0.48$ km s$^{-1}$ Mpc$^{-1}$ and lowers $S_8$ to $0.798 \pm 0.010$, while \emph{improving} the fit by $\Delta\chi^2 = -10.1$ relative to $\Lambda$CDM. The corresponding AIC clearly favors Geometric EDE, and the BIC penalty is modest (``positive evidence'' range).

Second, the way in which Geometric EDE achieves this balance is physically transparent. By inserting a narrow shelf at $z \sim 3000$, the model reduces the sound horizon and shifts the inferred $H_0$ upward without relying on exotic late-time behavior. By keeping the coupling of the scalar field to matter purely gravitational, it avoids the tendency of many EDE and modified gravity models to increase clustering. The result is a simultaneous easing of the two main tensions that have shaped cosmology over the past decade. The monodromy motivation for the potential shows that this structure is not an arbitrary fit to the data but a natural configuration within a well-explored class of theories.
...
There are, however, clear limitations and open questions. We have not attempted a full integration of all available modern probes, such as ACT DR6 or the complete DESI dataset, into a single global fit. We have also not constructed an explicit compactification in which the Ridder field appears as a definite modulus with all couplings fixed. Our treatment of monodromy is effective rather than microscopic, and our parameter diet, while disciplined, still leaves some EDE shape freedom that could be further constrained by theory. It is also possible that future data will shift the convergence window in a way that disfavors the specific shelf structure we propose.

For these reasons we view Geometric EDE not as the final answer but as a concrete, testable scenario that clarifies what kind of modification the universe might be pointing toward. The model makes several predictions that will sharpen in the near future: percent-level residuals in the high-$\ell$ CMB spectra, a specific form of $w(z)$ at intermediate redshifts, and a characteristic pattern in the joint distribution of $H_0$ and $S_8$. Upcoming CMB, BAO, and weak-lensing surveys will have the power to confirm or rule out this pattern. If the predictions fail, the negative result will still be valuable, since it will rule out an entire class of geometry-first solutions. If they succeed, the next step will be to embed the Ridder field and its monodromy potential in a fully consistent high-energy framework.
