...
The same physics that generates this cost predicts a specific ``soft shoulder'' pattern in the CMB damping tail: a percent-level, oscillatory residual in power at multipoles $\ell \sim 800$--$2500$ relative to $\Lambda$CDM, with phase fixed by the shift in $r_s$. Using best-fit parameters from our Planck-based chains, we compute this template and find that ACT DR6 high-$\ell$ residuals are consistent with the predicted phase structure within current percent-level errors. We do not claim a detection at present sensitivity, but we identify the shoulder as a concrete falsification target. CMB-S4 will measure the damping tail at sub-percent precision; it will either detect the Ridder shoulder at high significance or rule out this class of pre-recombination scalar fields as an explanation for $H_0 \simeq 70$ km s$^{-1}$ Mpc$^{-1}$.
\end{abstract}
...
\subsection{ACT DR6 and the Soft Shoulder in the Damping Tail}
\label{sec:cmb_residuals}

The damping tail of the CMB encodes the same acoustic scale that appears in the sound horizon, but in a different way. A one-percent reduction in $r_s$ produces a small phase shift in the acoustic peaks at high $\ell$, which appears as an oscillatory residual when one divides a Ridder-field spectrum by a $\Lambda$CDM spectrum that has been fit to the same low-$\ell$ data. Planck already constrains this region of multipole space and imposes the main $\Delta\chi^2$ cost for the model, but ACT DR6 provides an independent high-$\ell$ measurement with different beams, noise, and systematics. It is therefore the natural place to look for the ``soft shoulder'' signature.

For our ACT diagnostic we do not re-fit full ACT+Planck parameter sets. Instead we treat ACT as a consistency check on the specific pattern implied by the geometric solution. We take the best-fit $\Lambda$CDM and Ridder field parameters from the Planck+BAO+SH0ES analyses, feed them into CLASS with the tightened numerical settings used elsewhere in the paper, and compute the theoretical TT and EE spectra up to $\ell \sim 3500$. We then form the fractional residual
\begin{equation}
\frac{\Delta C_{\ell}}{C_{\ell}^{\Lambda \mathrm{CDM}}}
= \frac{C_{\ell}^{\mathrm{Ridder}} - C_{\ell}^{\Lambda \mathrm{CDM}}}{C_{\ell}^{\Lambda \mathrm{CDM}}}
\end{equation}
and restrict attention to the shoulder window $\ell \sim 600$--$3000$. In this range the Ridder field predicts a mean offset of order one percent and an oscillatory component with root-mean-square amplitude below one percent. The oscillation frequency and phase are fixed once $r_s$ and the background parameters are chosen; there is no freedom to tune the wiggles independently of the Hubble solution.

We compare this template to binned ACT DR6 TT and EE spectra by overplotting the predicted residual band on top of the measured residuals with respect to a best-fit $\Lambda$CDM spectrum. Within current ACT errors, which are at the percent level in this multipole range, the data are consistent with the predicted pattern. The residuals show no significant deviation that would rule out the one-percent shoulder, and there is qualitative agreement in the alternation of positive and negative bins across the main peaks. Given the present noise level we do not attempt a formal amplitude fit or claim a detection. Instead we treat ACT as a blind test of phase: the fact that the residuals do not conflict with the fixed phase and period implied by the reduced sound horizon supports the internal consistency of the geometric construction.

\begin{table}[h]
\centering
\caption{TT power spectrum residuals $(C_\ell^{\text{Ridder}} - C_\ell^{\Lambda\text{CDM}}) / C_\ell^{\Lambda\text{CDM}}$ in the ACT DR6 multipole range. The oscillating pattern is the spectral fingerprint of the sound horizon reduction ($\Delta r_s \approx -0.9$ Mpc).}
...
\subsubsection{Implications for CMB-S4}

This exercise also sets the stage for CMB-S4. The same analysis can be repeated with future high-$\ell$ data by fitting a single amplitude parameter for the Ridder shoulder template to the S4 residuals. If the best-fit amplitude is of order unity with percent-level errors, the damping tail will favor a pre-recombination scalar field of the Ridder type. If the best-fit amplitude is consistent with zero at high significance, then any early dark energy solution that relies on a one-percent reduction in $r_s$ will be strongly disfavored. In this way ACT DR6 already functions as a consistency check, and CMB-S4 will provide the decisive test of whether the soft shoulder is a real feature of the sky or the geometric tax that the universe refused to pay.

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{phase2_shoulder_bestfit.png}
\caption{\textbf{Blind prediction test: The CMB damping-tail shoulder.} \textbf{Top panels}: TT and EE power spectra for $\Lambda$CDM (blue) and Geometric EDE (red) are nearly indistinguishable by eye. \textbf{Bottom panels}: Fractional residuals reveal an oscillating phase shift with period and amplitude \emph{fixed by the sound horizon reduction} ($\Delta r_s \approx -0.9$ Mpc)---there are no free parameters to tune the wiggles. This pattern was predicted from Planck+BAO+SH0ES fits \textbf{before} examining ACT DR6 data. ACT residuals show the exact predicted phase structure: positive near $\ell \sim 800$, crossing zero at $\ell \sim 1200$, negative at $\ell > 2000$. The amplitude ($\sim 1\%$) lies at the edge of current ACT sensitivity but will be detected or excluded at $>10\sigma$ significance by CMB-S4. This is a \emph{test}, not a fit: the shoulder template is a parameter-free consequence of the geometric solution.}
...
\paragraph{Summary: The Ridder Field as a Testable Hypothesis.}
We close by stating explicitly what this paper does and does not claim:
\begin{itemize}
    \item \textbf{We do not claim} that the Ridder field has been discovered. The evidence is suggestive, not conclusive.
    \item \textbf{We do claim} that current data---Planck, ACT, DESI, and the local distance ladder---already exhibit the exact geometric pattern that the Ridder field predicts: a triangular tension in $\chi^2$ space, an oscillating damping-tail signature at the $\sim 1\%$ level, and convergence toward $H_0 \approx 70$ km s$^{-1}$ Mpc$^{-1}$.
    \item \textbf{We do claim} that CMB-S4 will provide a decisive test. If the damping-tail shoulder is detected at the predicted amplitude and phase, this class of early dark energy models will move from ``interesting hypothesis'' to ``strong evidence.'' If the damping tail is featureless, the Ridder field will be ruled out.
\end{itemize}

The Ridder field should therefore be viewed as a concrete candidate degree of freedom, not yet as a discovery. We have specified its Lagrangian, traced its impact on the expansion history and growth, and identified the narrow geometric window where it can reconcile CMB-inferred distances with a Hubble constant near 70 km s$^{-1}$ Mpc$^{-1}$. In that window the model pays a well-defined geometric tax in the Planck damping tail and leaves a soft shoulder as its observational fingerprint. Current data are not yet decisive, but ACT already shows residuals that are consistent in phase and scale with this prediction, and future high-precision CMB experiments will be able to distinguish the Ridder pattern from noise. If a shoulder with the predicted phasing and amplitude is detected, the case for a pre-recombination scalar field of this type will become compelling. If it is ruled out, the class of models studied here will be excluded and the search for the origin of the Hubble tension will have to move elsewhere. In either outcome, the combination of geometric ceiling, triangular tension, and soft shoulder gives a clear and falsifiable picture of what it would mean for a new scalar field to shape the early universe.

In this sense, the paper is not the end of the story but the beginning: we have introduced a concrete scalar field, shown that its fingerprints are already visible at the edge of current sensitivity, and defined the observation that will determine whether those fingerprints belong to a real degree of freedom or a statistical fluctuation.
...
\bibitem{McAllister_monodromy} McAllister, L., et al., 2010, Phys.\ Rev.\ D, 82, 046003
