...
The same physics predicts a characteristic ``soft shoulder'' in the CMB damping tail at the one-percent level. We construct a fixed template for this ripple pattern and fit its amplitude to ACT DR6 TT+EE data, finding a $6.4\sigma$ conditional preference for a nonzero shoulder aligned with the Geometric EDE template, under fixed-cosmology assumptions; a fully marginalized analysis may reduce this significance to $3$--$5\sigma$. Taken together, the geometric ceiling, the $H_0$--$S_8$ convergence window, and the soft-shoulder correlation make early-time geometry a concrete and falsifiable alternative to purely late-time or systematic explanations of the current anomalies and provide a sharp target for upcoming CMB-S4 observations.
\end{abstract}
...
We consider a single scalar field $\phi$ minimally coupled to gravity, evolving in a potential with three characteristic regimes: a plateau at early times, a narrow shelf near recombination, and a slowly varying tail at late times. We refer to this scalar-field extension as ``Geometric EDE'' (or $\phi$-EDE for short). This structure is motivated by axion monodromy scenarios~\cite{Silverstein_Westphal,McAllister_monodromy}, though we treat the potential phenomenologically in the main analysis (see Section~VII for the theoretical motivation).
...
\begin{enumerate}
    ...
    \item \textbf{Comparative context:} To contextualize the $\Delta\chi^2 \approx +11$ penalty, we performed fixed-$H_0$ analyses quantifying the statistical cost for $\Lambda$CDM to reach the same $H_0 \approx 70$ region. We estimate that forcing $\Lambda$CDM to $H_0 = 70$ would incur $\Delta\chi^2 \gtrsim +30$--$40$ (Table~\ref{tab:price_of_admission}), approximately $3\times$ the EDE penalty, while degrading rather than improving the $S_8$ fit. This suggests that early-time geometric modifications provide a more economical path to the $H_0 \approx 70$ regime than parameter adjustments within $\Lambda$CDM.
...
\begin{enumerate}
    \item \textbf{The $\chi^2$ cost is real.} EDE pays $\Delta\chi^2 \approx +11$ relative to $\Lambda$CDM's global minimum. However, we estimate $\Lambda$CDM would pay $\gtrsim +30$--$40$ to reach the same $H_0 \approx 70$ region (Table~\ref{tab:price_of_admission}).
...
The modified CLASS implementation, Cobaya configuration files, analysis scripts, and MCMC chains supporting this work are available at \url{https://github.com/StevenRidder/Ridder-Field} and permanently archived at Zenodo~\cite{zenodo_data}.
...
\bibitem{6dFGS} Beutler, F., et al., 2011, MNRAS, 416, 3017
\bibitem{MGS} Ross, A.~J., et al., 2015, MNRAS, 449, 835
\bibitem{BOSS_DR12} Alam, S., et al., 2017, MNRAS, 470, 2617
\bibitem{eBOSS} Alam, S., et al., 2021, Phys.\ Rev.\ D, 103, 083533

% Supernovae
\bibitem{Pantheon} Scolnic, D., et al., 2022, ApJ, 938, 113

% Weak Lensing
\bibitem{DES_Y3} DES Collaboration, 2022, Phys.\ Rev.\ D, 105, 023520
\bibitem{DES_Y1} DES Collaboration, 2018, Phys.\ Rev.\ D, 98, 043526
...
\bibitem{Denef_Douglas} Denef, F. \& Douglas, M.~R., 2004, JHEP, 05, 072
...
% Data Archive
\bibitem{zenodo_data} Ridder, S., 2025, MCMC Chains for Geometric EDE Field, Zenodo, \url{https://doi.org/10.5281/zenodo.17822063}
