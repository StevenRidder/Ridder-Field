...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    /* Ridder field (fluid-only) */
    class_define_index(ppv->index_pt_phi_ridder,pba->has_ridder,index_pt,1); /* Ridder field density (delta) */
    class_define_index(ppv->index_pt_phi_prime_ridder,pba->has_ridder,index_pt,1); /* Ridder field velocity (theta) */
  ...
  if (_scalars_) {
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      /* Using Generalized Dark Matter variables: delta_rho and Theta_flux */
      /* BLOCKER 3 FIX: Frozen field before oscillation, adiabatic after */
      if (pba->has_ridder == _TRUE_) {
        ...
        /* CORRECTED LOGIC: ALL modes get adiabatic IC */
        /* Reasoning: The ghost is NOT from initial conditions */
        /* The ghost appears because the fluid approximation itself */
        /* generates a non-decaying mode on superhorizon scales */
        /* This is a fundamental limitation of the fluid treatment */
        
        /* Use standard adiabatic IC for all modes */
        ...
        if (coeff > 0.0) {
          ...
          /* If rho+p ~ 0 (slow-roll), set to zero */
        ...
        if (ic_count < 3) {
          printf("RIDDER IC: k=%.3e coeff=%.3e delta_g=%.3e\n", k, coeff, ppw->pv->y[ppw->pv->index_pt_delta_g]);
          ic_count++;
        }
      }
...
int perturbations_total_stress_energy(
  ...
  if (_scalars_) {
    ...
    */
    ...
    /* Ridder field (fluid-only) contribution */
    /* Using Generalized Dark Matter variables: delta_rho and Theta_flux */
    if (pba->has_ridder == _TRUE_) {
      ...
      if (rho_ridder > 1.e-15) { // Safety check
          double p_ridder = ppw->pvecback[pba->index_bg_p_ridder];
          
          double delta_rho = y[ppw->pv->index_pt_phi_ridder];
          double Theta_flux = y[ppw->pv->index_pt_phi_prime_ridder];
          
          /* Local variables for consistency */
          double k2 = k*k;
          double a2 = a*a; /* 'a' is available in scope */
          
          /* Calculate Sound Speeds (Duplicated from derivs for consistency) */
          /* (Assuming same logic as in derivs) */
          int is_fluid_regime = (a > pba->a_osc_ridder) ? _TRUE_ : _FALSE_;
          
          double cs2, ca2;
          if (is_fluid_regime == _FALSE_) {
             cs2 = 1.0;
             ca2 = 1.0;
          } else {
             /* FLUID REGIME (Late Universe) */
             double m2_eff = 0.0;
          ...
          ppw->delta_rho += delta_rho;
          ppw->delta_p += delta_p;
          ppw->rho_plus_p_theta += Theta_flux;
      }
    }
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    /** - ---> Ridder field (FLUID APPROXIMATION ONLY) */
    
    if (pba->has_ridder == _TRUE_) {
      
      /* FLUID-ONLY MODE: Treat Ridder as perfect fluid from t=0 */
      /* Variables: delta_ridder (density contrast), theta_ridder (velocity divergence) */
      /* This avoids the oscillation crash by never solving Klein-Gordon */
      
      double w_eff = pba->w_eff_ridder;  // Set in background.c: (n-1)/(n+1) = 0.5 for n=3
      
      /* STABILITY FIX: Cutoff perturbations when energy density becomes negligible */
      /* rho_ridder decays faster than matter, but perturbations can grow. */
      /* To prevent numerical garbage from dominating P(k), turn off when subdominant. */
      ...
      if (0) { // DISABLED DEBUG rho_ridder < 1.e-4 * rho_crit) {
         ...
         */
          ...
          /* Implements scale-dependent sound speed to fix P(k) ghost */
          
          /* 1. Calculate m_eff^2 (Mpc^-2) */
          double m2_eff = 0.0;
          if (rho_ridder > 0.0) {
             double M_Pl_eV = 2.435e27;
             double eV_to_Mpc_inv = 1.5637e29;
          ...
          /* Formula from Smith et al (2019): cs2 -> w at low k, 1 at high k */
          double cs2 = w_eff; /* Fallback */
          if (m2_eff > 0.0) {
              double a2m2 = a2 * m2_eff;
              cs2 = (2.0 * a2m2 * w_eff + k2) / (2.0 * a2m2 + k2);
          }
          
          /* 3. Set Adiabatic Sound Speed ca2 */
