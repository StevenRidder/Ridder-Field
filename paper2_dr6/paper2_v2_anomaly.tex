...
Crucially, a fixed-geometry test where background cosmological parameters are 
frozen shows ACT spectra alone prefer the shoulder by $\Delta\chi^2 \approx -1{,}500$, 
confirming the signal is internal to ACT, not manufactured by DESI.
...
\textbf{Thesis.} We show that ACT DR6 high-$\ell$ data, when combined with 
Planck, already impose a sharp binary choice on cosmology: either a concrete 
pre-recombination feature that reshapes the $H_0$ and $S_8$ tensions, or a 
specific class of ACT-like systematics. We derive the decisive tests that 
forthcoming CMB experiments must perform to resolve this fork.

The significance of this result lies not in a specific model, but in the 
binary choice it imposes. Small-scale CMB data already force a decision 
between new pre-recombination physics and a specific class of ACT 
systematics---and we show exactly what that choice looks like and how to test it.

\subsection{The core finding}

This paper makes one concrete claim:
...
This is not a vague ``tension.'' It is a sharp, well-posed null test that 
\lcdm\ fails in ACT but not in Planck. We identify the exact knob in early-time 
physics that ACT is pulling on, and we show that smooth late-time fixes cannot 
reproduce it.
...
\subsection{The binary choice}

ACT DR6 high-$\ell$ data, when combined with Planck, already impose a sharp 
binary choice on cosmology. Either:
\begin{enumerate}
\item \textbf{New physics:} A concrete pre-recombination feature exists that 
reshapes the $H_0$ and $S_8$ tensions, visible only to high-resolution CMB 
experiments, or
\item \textbf{Systematics:} ACT has a specific class of high-$\ell$ systematic 
that mimics cosmological structure with achromatic, phase-coherent properties.
\end{enumerate}

We have derived the decisive tests---SPT-3G by 2026, Simons Observatory by 
2027---that forthcoming CMB experiments must perform to resolve this fork. 
The significance of this work is not a specific model, but a roadmap: we 
specify the exact signature, the required precision, and the expected 
significance under each hypothesis. The test is falsifiable and imminent.
