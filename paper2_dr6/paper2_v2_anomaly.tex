...
\begin{abstract}
We perform a template test for pre-recombination physics in the CMB damping 
tail. The template $T_\ell^{\rm soft}$ encodes the high-$\ell$ signature of 
a percent-level reduction in the sound horizon $r_s$. Its shape and phase 
are fixed from early-time physics before examining the data. We introduce 
a single amplitude parameter $A_{\rm sh}$ that rescales this template on 
top of a standard \lcdm\ spectrum.

Applied to ACT DR6, the template is detected with high significance. ACT 
prefers $A_{\rm sh} = 1.54 \pm 0.19$ (8.1$\sigma$), improving the fit by 
$\Delta\chi^2 = -766$ relative to pure \lcdm. Ninety percent of this 
improvement comes from ACT high-$\ell$ temperature data ($\ell > 1500$); 
BAO and supernovae are neutral at the level of $|\Delta\chi^2| \lesssim 1$. 
A fixed-geometry test, where background cosmological parameters are held 
fixed, shows that ACT spectra alone prefer the shoulder pattern by 
$\Delta\chi^2 \approx -1{,}500$, confirming that the signal is internal 
to the damping tail and not manufactured by distance data.

Applied to Planck, the same template is not detected. Planck prefers 
$A_{\rm sh} \approx 0$ and penalizes the ACT-like amplitude with 
$\Delta\chi^2 = +121$, of which 89\% arises from high-$\ell$ TTTEEE 
data in the beam-suppressed regime. This asymmetry correlates with 
angular resolution: ACT retains high signal-to-noise at $\ell \sim 3000$, 
while Planck becomes noise-dominated once beam deconvolution is applied.

Monte Carlo simulations, phase-scrambled templates, and frequency-split 
fits show that the shoulder detection in ACT is phase coherent with the 
acoustic peaks, statistically robust, and achromatic at 90 and 150~GHz. 
\emph{If interpreted as a pre-recombination feature}, a concrete early 
dark energy model that produces this template maps to $H_0 \simeq 70.7$~\kms\ 
and $\sigma_8 \simeq 0.75$. We outline concrete tests for SPT-3G, Simons 
Observatory, and CMB-S4 to determine whether this resolution-dependent 
asymmetry arises from pre-recombination physics or ACT-specific systematics.
\end{abstract}
...
Section~\ref{sec:latetime} explains 
...
$H_0$ tension is ``solved.'' What the data show is:
\begin{enumerate}
