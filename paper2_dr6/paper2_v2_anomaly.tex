...
We construct a fixed, theory-motivated damping-tail template $T_\ell^{\rm soft}$ 
and fit a single amplitude $A_{\rm sh}$ to current CMB data. ACT DR6 prefers 
$A_{\rm sh} \neq 0$ with large $\Delta\chi^2$ improvement, while Planck prefers 
$A_{\rm sh} \approx 0$ and penalizes the same template in its high-$\ell$ 
likelihood. This defines a sharp, internal CMB inconsistency that depends on 
angular resolution.
...
Its shape and phase are fixed \emph{before} examining ACT DR6 data. We fit 
only the amplitude $A_{\rm sh}$, treating it as a single observable.

In the discussion we outline two broad interpretations of this result---pre-recombination 
physics versus ACT-specific systematics---and the observational tests that can 
distinguish them.
...
\subsection{The ACT/Planck asymmetry: two arms of a controlled experiment}

By folding in the Planck result, we turn what could be an embarrassment 
into an asset. Planck and ACT are now \emph{two arms of a controlled 
experiment on the early universe}: one experiment screams for the feature, 
the other resists it, and both operate in regimes where the theory is clean.

The asymmetry correlates with beam sensitivity:
\begin{itemize}
...
The significance is that the cosmological model is no longer only in tension 
with distance-ladder $H_0$ or weak lensing. The CMB community itself faces 
a hard internal fork: either there is previously unknown structure in the 
pre-recombination plasma that only shows up in high-resolution data, or ACT 
has an identifiable class of systematic that future experiments must rule 
in or out.
