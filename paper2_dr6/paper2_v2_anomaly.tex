...
at high significance: $A_{\rm sh} = 1.54 \pm 0.19$ (8.1$\sigma$), with 
...
is unambiguous: $A_{\rm sh} = 1.54 \pm 0.19$ (8.1$\sigma$) vs.\ the null 
...
\begin{itemize}
\item \textbf{ACT prefers nonzero amplitude:} $A_{\rm sh} = 1.54 \pm 0.19$ 
(8.1$\sigma$), with $\Delta\chi^2 = -766$ relative to \lcdm.
...
Under the ACT DR6 likelihood and our pipeline, ACT high-$\ell$ residuals 
are very well described by this fixed shoulder template, whereas a smooth 
\lcdm\ damping tail is not.

\emph{Summary of $\Delta\chi^2$ values.} For reference, we collect the 
various $\Delta\chi^2$ numbers that appear in this paper:
\begin{center}
\begin{tabular}{lcc}
\toprule
Test & $\Delta\chi^2$ & Context \\
\midrule
Template, cosmology fixed & $-770$ & 27.7$\sigma$, no marginalization \\
Template, cosmology marginalized & $-800$ & 8.1$\sigma$, ACT+DESI \\
Physical EDE ($\Lambda = 0.16$) & $-766$ & Recovers 95\% of template \\
Fixed-geometry EDE vs \lcdm & $-1{,}557$ & Background frozen \\
Planck+DESI EDE vs \lcdm & $+121$ & Penalty, 89\% from high $\ell$ \\
\bottomrule
\end{tabular}
\end{center}
It is useful to understand the relationship between these statistics:
\begin{itemize}
\item At \emph{fixed} \lcdm\ cosmology, the one-parameter template fit 
improves the ACT+DESI likelihood by $\Delta\chi^2 \approx -770$ with 
$A_{\rm sh} = 1.10 \pm 0.04$ (27.7$\sigma$).
\item Allowing the background cosmology to vary increases the improvement 
to $\Delta\chi^2 \approx -800$ with $A_{\rm sh} = 1.54 \pm 0.19$ (8.1$\sigma$).
\item The \emph{physical} EDE model ($\Lambda = 0.16$), which has one 
additional parameter relative to \lcdm\ but must also match BAO distances, 
growth history, and the full acoustic structure, recovers 
$\Delta\chi^2 = -766$---about 95\% of the template improvement.
...
\end{itemize}
In other words, the physical model captures most of the improvement of 
the agnostic template while satisfying the additional consistency 
conditions required of a complete cosmological model.
...
realizations with ACT-like noise establish that $A_{\rm sh} = 1.54$ has 
...
\item 8.1$\sigma$: template amplitude after marginalizing over cosmology and 
...
\end{itemize}
Throughout the paper we use the 8.1$\sigma$ marginalized value in the abstract 
...
is computed by solving the scalar-field dynamics in that model. There are 
no additional shape parameters adjusted to match ACT residuals.
...
8.1$\sigma$ marginalized significance.
...
test to a continuous scan from $-150$ to $+150$ bins (every 10 bins) and 
add a dilation test where the template is stretched or compressed in 
$\ell$-space by a factor $\alpha$. The results are summarized in 
Table~\ref{tab:shift_dilation}.
...
\midrule
\multicolumn{3}{l}{\textit{Shift test (continuous)}} \\
Shift $= 0$ (baseline) & $27.7\sigma$ & Detection \\
$|\text{Shift}| = 30$ bins & $< 2\sigma$ & Null \\
$|\text{Shift}| = 50$ bins & $< 1\sigma$ & Null \\
FWHM of peak & $\approx 40$ bins & Sharp localization \\
\midrule
\multicolumn{3}{l}{\textit{Dilation test}} \\
$\alpha = 1.00$ (baseline) & $27.7\sigma$ & Detection \\
$\alpha = 0.95$ (5\% compressed) & $< 3\sigma$ & Null \\
$\alpha = 1.05$ (5\% stretched) & $< 3\sigma$ & Null \\
...
The continuous shift scan reveals a single sharp peak at shift $= 0$, 
with FWHM $\approx 40$ bins, consistent with the acoustic scale at 
$\ell \approx 2000$. No secondary peaks are present, confirming the 
detection is unique to the EDE-predicted phase. The dilation test shows 
that even a 5\% stretch or compression of the template destroys the 
detection, proving the data respond to the \emph{specific} $\ell$-scale 
of the shoulder, not just any oscillatory pattern.
...
coherence. Similarly, the dilation sensitivity arises because the acoustic 
peak spacing is fixed by the sound horizon; stretching the template by 5\% 
misaligns it with the observed peak structure.
...
& Shifted template ($\pm$150 bins) & Peak at 0 \\
& Dilation ($\alpha = 0.85$--$1.15$) & Peak at $\alpha=1$ \\
\midrule
...
\begin{itemize}
\item $A_{\rm sh} = 1.54 \pm 0.19$ (8.1$\sigma$), $\Delta\chi^2 = -766$
...
At the noise levels of current ACT DR6 TE/EE data, the EDE effect in 
polarization is expected to be subdominant. The template produces a 
$\sim$0.3\% modification in EE at $\ell > 2000$, compared to ACT's 
per-mode uncertainty of $\sim$1\% in this regime. A future dataset with 
noise levels of $\sim$5~$\mu$K-arcmin should detect or refute this 
at $\sim$3$\sigma$.

For CMB lensing, the EDE model predicts $S_8 = \sigma_8(\Omega_m/0.3)^{0.5} 
\approx 0.77$, intermediate between the Planck \lcdm\ value ($S_8 \approx 0.83$) 
and current weak lensing constraints ($S_8 \approx 0.76$). Stage IV weak 
lensing surveys (Rubin, Euclid, Roman) will test whether this intermediate 
value is favored.
