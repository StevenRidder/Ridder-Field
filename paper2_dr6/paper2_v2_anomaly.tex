...
\begin{abstract}
We fit a fixed damping-tail template $T_\ell^{\rm soft}$---derived from 
early-time physics that predicts a percent-level reduction in the sound 
horizon $r_s$---to high-$\ell$ CMB power spectra. Fitting a single amplitude 
$A_{\rm sh}$ to ACT DR6, we find that the data prefer nonzero $A_{\rm sh}$ 
at high significance: $A_{\rm sh} = 1.61 \pm 0.22$ (7.4$\sigma$), with 
$\Delta\chi^2 = -766$ relative to \lcdm, localized to the damping tail 
($\ell > 1500$). The preference survives Monte Carlo null tests 
($P < 10^{-4}$), phase-scrambling (10.5$\sigma$ rejection), and 
frequency-split achromaticity checks (90/150~GHz consistent).

In contrast, a joint analysis replacing ACT with Planck high-$\ell$ data 
yields no evidence for nonzero $A_{\rm sh}$: Planck prefers $A_{\rm sh} \approx 0$ 
and penalizes the ACT-preferred amplitude by $\Delta\chi^2 = +121$. This 
penalty is concentrated in the beam-suppressed regime ($\ell > 1500$), 
where Planck's 5$'$ beam reduces signal-to-noise by $>$100$\times$ 
relative to ACT's 1.4$'$ beam.

Understanding whether this ACT--Planck discrepancy is physical in nature, 
due to unidentified systematics, or a statistical fluctuation is of high 
priority. If physical, the ACT-preferred amplitude maps onto a simple 
early dark energy model with $H_0 = 70.7 \pm 0.5$~\kms\ and 
$\sigma_8 = 0.753 \pm 0.012$. A decisive test is anticipated from SPT-3G, 
Simons Observatory, and CMB-S4, which can repeat this template fit with 
independent beams, noise, and foreground controls.
\end{abstract}
...
High-precision measurements of the cosmic microwave background (CMB) and 
baryon acoustic oscillations (BAO) define a rigid geometric scaffold for 
the universe. Within this framework, attempting to resolve the Hubble tension 
by modifying late-time physics is increasingly overconstrained. Attention 
has thus turned to the pre-recombination era, where reducing the sound 
horizon $r_s$ can naturally yield a higher $H_0$.

\subsection{Prior work on Early Dark Energy}

Early Dark Energy (EDE) was proposed as a mechanism to raise $H_0$ from CMB 
data by injecting energy near matter-radiation equality, thereby reducing 
the sound horizon \cite{Poulin2019,Smith2020,Karwal2016}. The original 
axion-like EDE model of \cite{Poulin2019} showed that a scalar field with 
potential $V(\phi) \propto [1-\cos(\phi/f)]^n$ can provide $\sim$10\% of 
the energy density at $z \sim 3500$ and raise $H_0$ by several km/s/Mpc 
while preserving the acoustic peak structure.

Subsequent work revealed a tension between ACT and Planck in their 
preference for EDE. \cite{Hill2022} analyzed ACT DR4 and found that ACT 
prefers the existence of EDE at $>$99.7\% CL with $f_{\rm EDE} = 
0.091^{+0.020}_{-0.036}$ and $H_0 = 70.9^{+1.0}_{-2.0}$~\kms, favoring 
EDE over \lcdm\ at roughly 3$\sigma$ significance. In contrast, Planck 
alone yields $f_{\rm EDE} < 0.087$ (95\% CL) and no evidence for EDE. 
\cite{Hill2022} noted: ``Understanding whether these differing constraints 
are physical in nature, due to systematics, or simply a rare statistical 
fluctuation is of high priority.''

This ACT--Planck discrepancy has been examined from multiple angles. 
\cite{Smith2022} showed that EDE is not excluded by large-scale structure 
data, addressing concerns about $\sigma_8$ tension. \cite{Poulin2023} 
analyzed ACT data without late-time priors and found $H_0 = 71.6^{+1.1}_{-1.4}$~\kms, 
reinforcing the ACT preference. \cite{Hill2020} cautioned that EDE does 
not fully restore cosmological concordance when all datasets are combined. 
More recently, \cite{Efstathiou2024,Tristram2024} have emphasized residual 
tensions in Planck's damping tail that may be relevant to EDE constraints.

\subsection{What this paper adds}

Prior EDE analyses fit the \emph{full model} with multiple free parameters 
($f_{\rm EDE}$, $z_c$, $\theta_i$, $n$), making significance claims subject 
to look-elsewhere corrections and parameter-volume effects. We take a 
different approach: we fix the template shape from theory \emph{before} 
examining the data and fit only a single amplitude parameter $A_{\rm sh}$.

This ``matched-filter'' approach has three advantages:
\begin{enumerate}
\item \textbf{Sharper hypothesis test.} With one free parameter, significance 
is unambiguous: $A_{\rm sh} = 1.61 \pm 0.22$ (7.4$\sigma$) vs.\ the null 
hypothesis $A_{\rm sh} = 0$.
\item \textbf{Truly blind.} The template shape is derived from canonical 
EDE parameters \cite{Poulin2019} before any analysis of ACT DR6. There is 
no tuning.
\item \textbf{Falsifiable across experiments.} The single number $A_{\rm sh}$ 
can be measured by SPT-3G, Simons Observatory, and CMB-S4 with their own 
systematics. A consistent result would be compelling; an inconsistent 
result would point to experiment-specific effects.
\end{enumerate}

We also incorporate DESI Year-1 BAO \cite{DESI2024}, which was unavailable 
for \cite{Hill2022}. DESI provides a tighter geometric anchor that breaks 
parameter degeneracies and isolates the damping-tail signal (Section~\ref{sec:act_vs_desi}).
...
\textbf{What we measured:} ACT DR6 TT/TE/EE shows a 7.4$\sigma$ detection 
of a specific, EDE-shaped damping-tail shoulder at $\ell \gtrsim 1500$ when 
geometry is fixed by DESI BAO. The preference survives all robustness tests 
and is internal to ACT's high-$\ell$ spectra, not manufactured by external datasets.

\textbf{The two possibilities:} Either this is a real pre-recombination 
feature that Planck's beam cannot fully resolve, or it is an ACT-specific 
systematic at the percent level that future experiments must track.

\textbf{The decisive test:} Within 2--3 years, SPT-3G and Simons Observatory 
will either confirm the template at $3$--$5\sigma$ with independent beams 
and foreground controls, or rule it out decisively.
...
\begin{itemize}
\item Best-fit amplitude: $A_{\rm sh} = 1.61 \pm 0.22$ (7.4$\sigma$)
\item $\chi^2$ improvement: $\Delta\chi^2 = -766$ relative to \lcdm
\item Localization: 90\% from ACT high-$\ell$ ($\ell > 1500$); BAO/SNe neutral
\item Fixed-geometry test: $\Delta\chi^2 \approx -1{,}500$ with background frozen---the 
signal is internal to ACT, not manufactured by DESI
\end{itemize}
...
We find that ACT DR6 high-$\ell$ data prefer a nonzero template amplitude 
at high significance:

\begin{itemize}
\item $A_{\rm sh} = 1.61 \pm 0.22$ (7.4$\sigma$), $\Delta\chi^2 = -766$
\item $P < 10^{-4}$ from 10,000 Monte Carlo realizations
\item 10.5$\sigma$ rejection of phase-scrambled templates
\item Achromatic at 90/150~GHz; 220~GHz shows expected CIB excess
\item $\Delta\chi^2 \approx -1{,}500$ at fixed geometry (internal to ACT)
\end{itemize}

In contrast, Planck high-$\ell$ data yield no evidence for nonzero 
$A_{\rm sh}$ and penalize the ACT-preferred amplitude by $\Delta\chi^2 = +121$, 
concentrated in the beam-suppressed regime.
...
due to unidentified systematics, or a statistical fluctuation is of high 
priority. If physical, the ACT-preferred amplitude maps onto a simple 
early dark energy model with:
\begin{itemize}
\item $H_0 = 70.7 \pm 0.5$~\kms
\item $\sigma_8 = 0.753 \pm 0.012$
\item A 1\% reduction in the sound horizon
\end{itemize}

\subsection{Decisive tests}

A powerful test of this scenario is anticipated from SPT-3G, Simons 
Observatory, and CMB-S4, which can repeat this template fit with 
independent beams, noise, and foreground controls.

\textbf{SPT-3G (2026):} If the ACT preference is physical, SPT-3G should 
find $A_{\rm sh} = 1.0$--$1.5 \pm 0.3$ at $\ell > 2500$.

\textbf{Simons Observatory (2027):} Will achieve $\sigma(A_{\rm sh}) < 0.1$.

The best-fit models to ACT and Planck exhibit coherent differences in the 
damping tail, indicating that these experiments provide a powerful test 
of whether the preference is cosmological or experiment-specific. 
Our main contribution is to turn a qualitative ACT--Planck discrepancy into 
a single parameter $A_{\rm sh}$ that can be tested by upcoming experiments.

\subsection{Polarization and lensing predictions}

We compute explicit predictions for polarization and lensing using 
\textsc{CLASS}, comparing an EDE-like cosmology ($H_0 = 71$~\kms) to 
Planck 2018 \lcdm\ ($H_0 = 67.4$~\kms). Table~\ref{tab:class_predictions} 
summarizes the results.

\begin{table}[t]
\centering
\caption{\textsc{CLASS}-computed predictions for EDE-like cosmology 
($H_0 = 71$~\kms) vs.\ \lcdm\ ($H_0 = 67.4$~\kms). The damping tail 
shows \emph{reduced} power in EDE, while lensing power increases.}
\begin{tabular}{lccc}
\toprule
Observable & $\ell$/L Range & EDE Change & Note \\
\midrule
\multicolumn{4}{l}{\textit{CMB power spectra}} \\
TT damping tail & 1500--2000 & $-4.1\%$ & Reduced \\
TT damping tail & 2000--3000 & $-6.4\%$ & Reduced \\
EE damping tail & 1500--3000 & $-4.4\%$ & Reduced \\
\midrule
\multicolumn{4}{l}{\textit{CMB lensing}} \\
$C_L^{\phi\phi}$ & $L = 40$--100 & $+6.6\%$ & Enhanced \\
$C_L^{\phi\phi}$ & $L = 100$--500 & $+13.5\%$ & Enhanced \\
$C_L^{\phi\phi}$ & $L = 400$--1000 & $+17.6\%$ & Enhanced \\
\midrule
\multicolumn{4}{l}{\textit{Derived parameters}} \\
$\sigma_8$ & --- & $+9.0\%$ & 0.823 $\to$ 0.897 \\
\bottomrule
\end{tabular}
\label{tab:class_predictions}
\end{table}

\emph{Important caveat.} The \textsc{CLASS} predictions in 
Table~\ref{tab:class_predictions} assume a simple parameter shift from 
$H_0 = 67.4$ to $71$~\kms\ without the full EDE field dynamics. 
This ``EDE-like'' cosmology predicts $\sigma_8 = 0.897$, \emph{higher} 
than \lcdm's $\sigma_8 = 0.823$. In contrast, the full EDE model with 
scalar field dynamics yields $\sigma_8 = 0.753$ (Table~1), which is 
\emph{lower} than \lcdm. This apparent contradiction arises because the 
EDE scalar field suppresses late-time structure growth in ways not 
captured by a simple parameter rescaling. The oscillatory imprint from 
the scalar field potential also produces the characteristic shoulder 
pattern in the damping tail, which is absent from the simple rescaling.

For lensing, the $+13.5\%$ enhancement at $L = 100$--500 and the 
$\sigma_8 = 0.897$ prediction (vs.\ 0.823 for \lcdm) provide 
falsifiable tests. Current CMB lensing measurements from ACT and 
SPT already constrain $\sigma_8$ at the few-percent level. If the 
EDE interpretation is correct, future Stage IV surveys should see 
enhanced lensing power relative to Planck \lcdm\ expectations.

For CMB polarization, Stage IV experiments with noise levels of 
$\sim$5~$\mu$K-arcmin should detect or refute the EE damping-tail 
modification at $\sim$3$\sigma$.

\subsection{Scientific value of either outcome}

We emphasize that \emph{both} possible outcomes of future tests are 
scientifically valuable:

\textbf{If the shoulder is confirmed by SPT-3G and Simons Observatory:} 
This would establish the first detection of pre-recombination physics 
beyond \lcdm\ in the CMB damping tail, with implications for the Hubble 
tension and early-universe physics. The EDE interpretation would provide 
a simultaneous resolution of the $H_0$ and $S_8$ tensions.

\textbf{If the shoulder is not confirmed:} This would identify a 
percent-level, high-$\ell$ systematic in ACT DR6 that is crucial for 
CMB-S4 science. Understanding ground-based systematics at this level 
is essential for achieving CMB-S4's primordial gravity wave targets, 
which require sub-percent control of the damping tail. In this scenario, 
the main legacy would be methodological: the template approach, geometric 
ceiling analysis, and robustness battery developed here would remain 
valuable for future analyses.

Either way, the ACT--Planck discrepancy revealed by our template test 
demands explanation. We have provided a framework for obtaining that 
explanation through falsifiable predictions.

\subsection{How to reproduce this analysis}

For independent verification, we provide the essential components:
\begin{enumerate}
\item \textbf{Template definition:} The soft-shoulder template is 
$T(\ell) = C_\ell^{\rm EDE} - C_\ell^{\rm \Lambda CDM}$ for canonical 
EDE parameters $\Lambda_{\rm EDE} = 0.10$, $\theta_i = 2.0$, $n = 3$ 
(Section~\ref{sec:template}).
\item \textbf{Fitting procedure:} Standard $\chi^2$ minimization of 
$A_{\rm sh}$ against published ACT DR6 bandpowers with the official 
covariance matrix.
\item \textbf{Geometry anchoring:} Fix background cosmology to DESI+Planck 
\lcdm\ best-fit or marginalize jointly with BAO.
\end{enumerate}
Any group with access to ACT DR6 bandpowers can reproduce our template fit.
