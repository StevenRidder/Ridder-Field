...
near the geometric ceiling for $H_0$ when combined with DESI BAO, but we do
not use this model as a primary fit function and we do not claim that it is
favored once Planck high-$\ell$ data are included. Instead, we present
$A_{\rm sh}$ as a single summary statistic that SPT-3G, Simons Observatory,
and CMB-S4 can test with independent high-resolution CMB data to decide
whether the ACT damping-tail preference reflects pre-recombination
structure or an experiment-specific systematic.
\end{abstract}
...
\subsection{Conditional implications for $H_0$ and $\sigma_8$}

If the nonzero $A_{\rm sh}$ inferred from ACT is interpreted in terms of
the specific early dark energy model we use to define the template, then
the ACT+DESI fit corresponds to $H_0 = 70.7 \pm 0.5$~\kms\ 
and $\sigma_8 = 0.753 \pm 0.012$. These values lie near the geometric
ceiling for early-time models defined in Section~\ref{sec:geometry}. 
This mapping relies on adopting that particular EDE model and on using 
ACT, not Planck, as the high-$\ell$ dataset.

The main result of this paper does not depend on that interpretation. The
measurement-level statement is simply that ACT DR6 prefers $A_{\rm sh} \neq
0$ at high significance, while Planck prefers $A_{\rm sh} \approx 0$, under
a fixed template derived from early-time physics. All parameter-level
numbers in this subsection should be read as conditional on the EDE model
choice and on treating ACT as the high-resolution anchor.

For reference, the conditional $H_0$ would be consistent with recent 
non-Cepheid measurements:
\begin{center}
\begin{tabular}{lcc}
\toprule
...
Continuing with the conditional interpretation, the physical origin of the 
$H_0$ shift is a 1\% reduction in the sound horizon: $r_s = 146.0$~Mpc (EDE) 
vs.\ $r_s = 147.4$~Mpc (\lcdm). Combined with DESI BAO, this yields the 
higher $H_0$. Relative to Planck \lcdm:
\begin{itemize}
...
$\sigma_8 = 0.753$, and $\Delta\chi^2 = -766$. Under this conditional 
interpretation, $H_0$ and $\sigma_8$ move toward late-time values.
...
The measurement-level conclusion is therefore an observational
asymmetry: one high-resolution CMB experiment prefers this specific
damping-tail pattern, another lower-resolution experiment does not. Mapping
$A_{\rm sh}$ onto a concrete early-time model inevitably brings in
additional assumptions and tension with Planck; we have shown how one such
model projects onto $H_0$ and $\sigma_8$, but we do not adopt it as a
global solution.

\subsection{The observational test}

The most direct way to resolve this asymmetry is observational. SPT-3G,
