...
\title{ACT DR6 Damping Tail Shows 8.1$\sigma$ Deviation from \lcdm\ at $\ell > 1500$:\\
Planck Shows Opposite Response in Beam-Suppressed Regime}
...
\begin{abstract}
ACT DR6 combined with DESI BAO shows $\Delta\chi^2 = -766$ preference for an 
early dark energy (EDE) template at $\ell > 1500$, detected at 8.1$\sigma$. 
The improvement is localized: 690 of 766 $\chi^2$ units (90\%) come from 
ACT's high-$\ell$ likelihood; BAO and supernovae contribute $\pm 1$.

The same model applied to Planck + DESI yields $\Delta\chi^2 = +121$, with 
108 of 121 $\chi^2$ units (89\%) from Planck high-$\ell$. This asymmetry 
correlates with beam sensitivity: ACT retains 87\% transmission at $\ell=3000$; 
Planck falls to 18\%.

The preferred EDE parameters ($\Lambda_{\rm EDE} = 0.145 \pm 0.006$) yield 
$H_0 = 70.7$~\kms\ and $\sigma_8 = 0.753$. Both values lie between CMB-only 
(67.4, 0.83) and late-time probes (JWST TRGB: $69.9 \pm 1.2$; weak lensing 
$S_8 \approx 0.76$).

Monte Carlo simulations (10,000 realizations) yield $P < 10^{-4}$. 
Phase-scrambling tests show 10.5$\sigma$ coherence. Frequency splits at 
90/150~GHz give consistent $\Lambda_{\rm EDE} = 0.141 \pm 0.006$; 220~GHz 
shows 1.6$\sigma$ excess (CIB). Fixed-geometry tests with all background 
parameters frozen show ACT prefers the shoulder by $\Delta\chi^2 = -1{,}557$.

SPT-3G provides the critical test: with comparable resolution but independent 
systematics, SPT should measure $A_{\rm sh} = 1.0$--$1.5 \pm 0.3$ by 2026.
\end{abstract}
...
ACT DR6 temperature and polarization spectra at $\ell > 1500$ show 
systematic deviations from \lcdm\ that correlate with a pre-computed 
early dark energy (EDE) template. When this template is fitted 
as a one-parameter extension, the amplitude is $A_{\rm sh} = 1.54 \pm 0.19$ 
(8.1$\sigma$), yielding $\Delta\chi^2 = -766$ relative to \lcdm.

The improvement is concentrated: 690 of 766 $\chi^2$ units come from 
ACT TT at $\ell > 1500$. BAO measurements contribute $\Delta\chi^2 = -1$. 
Pantheon+ supernovae contribute $\Delta\chi^2 = 0$. This localization to 
ACT's damping tail, with geometric probes remaining neutral, 
distinguishes the effect from parameter-space artifacts.

When the same physical EDE model ($\Lambda_{\rm EDE} = 0.145$) is applied to 
Planck + DESI with identical priors, $\Delta\chi^2 = +121$. The penalty is 
similarly localized: 108 of 121 $\chi^2$ units come from Planck high-$\ell$ 
at $\ell > 1500$---precisely where Planck's 5$'$ beam suppresses sensitivity 
relative to ACT's 1.4$'$ beam.

We characterize this asymmetry through beam convolution tests, 
phase-scrambling null checks, frequency-split analyses, and 
fixed-geometry comparisons. SPT-3G, with angular resolution 
comparable to ACT but independent systematics, provides the 
definitive test within 18 months.

\subsection{Summary of measurements}

\begin{itemize}
\item \textbf{Detection:} $A_{\rm sh} = 1.54 \pm 0.19$ (8.1$\sigma$), 
$\Delta\chi^2 = -766$ (ACT+DESI).
\item \textbf{Localization:} 90\% of $\chi^2$ improvement from ACT high-$\ell$; 
BAO and SNe neutral.
\item \textbf{Robustness:} Monte Carlo (10,000 sims, $P < 10^{-4}$); 
phase scrambling (10.5$\sigma$ coherence); frequency splits (achromatic 
at 90/150~GHz).
\item \textbf{Planck response:} $\Delta\chi^2 = +121$, with 89\% from 
beam-suppressed high-$\ell$.
\item \textbf{Derived parameters:} $H_0 = 70.7$~\kms, $\sigma_8 = 0.753$.
\end{itemize}

\subsection{The soft shoulder}
...
subsequently dilutes.

\subsection{Resolution asymmetry}
...
\end{itemize}
This resolution dependence is quantified in Section~\ref{sec:planck}.

\subsection{The role of DESI BAO}

A key finding is that ACT \emph{alone} penalizes EDE:
\begin{center}
\begin{tabular}{lcc}
\toprule
Dataset & $\Delta\chi^2$ (EDE vs \lcdm) \\
\midrule
ACT-only & $+282$ (EDE worse) \\
ACT + DESI & $-766$ (EDE better) \\
\bottomrule
\end{tabular}
\end{center}
This is not p-hacking---it reflects how spectral and geometric data combine.
Without DESI, ACT has freedom to adjust cosmological parameters ($H_0$, $\Omega_m$, 
$n_s$) to absorb the damping-tail residual within \lcdm. DESI's precise BAO 
measurements anchor the geometric parameters, forcing ACT to fit the residual 
with the EDE spectral modification. The $\Delta\chi^2 = -766$ improvement is 
localized to ACT high-$\ell$, confirming DESI breaks a geometric degeneracy 
rather than introducing a spurious preference.

\subsection{Two-regime physics}

EDE physics operates in two distinct regimes controlled by $\Lambda_{\rm EDE}$:

\emph{Regime I ($\Lambda \approx 0.16$): Spectral effects at $\ell > 2000$.}
Low $\Lambda$ produces a localized shoulder in the damping tail without 
significantly modifying the sound horizon. ACT detects this ($\Delta\chi^2 = -766$); 
Planck cannot resolve it ($\Delta\chi^2 = +121$). This paper focuses on Regime I.

\emph{Regime II ($\Lambda \approx 0.8$): Geometric effects at $\ell < 1500$.}
High $\Lambda$ shrinks the sound horizon, shifting acoustic peak positions.
Planck measures this precisely at low-$\ell$; ACT's noise at $\ell < 1500$ 
rejects strong sound-horizon modifications ($\Delta\chi^2 \approx +965$).

This two-regime structure explains why ACT and Planck respond differently 
to the same model: they are sensitive to different physical effects. Paper~I 
established the geometric ceiling ($H_0 < 71$~\kms) from Regime II; this paper 
detects the spectral signature in Regime I.
...
\subsection{Consistency with external $H_0$ measurements}

The ACT+DESI EDE fit yields $H_0 = 70.7 \pm 0.5$~\kms. This value 
is consistent with recent non-Cepheid measurements:
\begin{center}
\begin{tabular}{lcc}
\toprule
...
\subsection{Derived $H_0$ and the Hubble tension}

The ACT+DESI EDE fit yields $H_0 = 70.7 \pm 0.5$~\kms. This value is:
\begin{itemize}
...
The ACT measurement has specific observational properties:
...
\subsection{The $\Lambda = 0.16$ solution}
...
ACT DR6 combined with DESI BAO shows $\Delta\chi^2 = -766$ preference for 
EDE at $\Lambda_{\rm EDE} = 0.145 \pm 0.006$, detected at 8.1$\sigma$. The 
$\chi^2$ improvement is localized to ACT high-$\ell$ (90\%), with BAO and 
supernovae neutral. The derived parameters are $H_0 = 70.7 \pm 0.5$~\kms\ 
and $\sigma_8 = 0.753 \pm 0.012$.

Planck + DESI shows $\Delta\chi^2 = +121$ for the same model, with 89\% of 
the penalty from Planck high-$\ell$ at $\ell > 1500$. This correlation 
with beam sensitivity---ACT retains 87\% at $\ell = 3000$; Planck falls 
to 18\%---is consistent with ACT detecting structure below Planck's 
resolution limit.

Robustness tests establish:
\begin{itemize}
\item Phase-specificity: 10.5$\sigma$ coherence from phase-scrambling test.
\item Achromaticity: 90/150~GHz consistent; 220~GHz shows CIB excess.
\item Geometric independence: $\Delta\chi^2 = -1{,}557$ at fixed geometry.
\item Statistical significance: $P < 10^{-4}$ from 10,000 Monte Carlo realizations.
\end{itemize}

\subsection{Falsifiable predictions}

SPT-3G with 1.2$'$ beam and independent pipeline should measure:
\begin{itemize}
\item $A_{\rm sh} = 1.0$--$1.5 \pm 0.3$ at $\ell > 2500$ by 2026.
\item Phase matching ACT template within $\Delta\phi < 0.1$ radians.
\item Achromatic response at 95/150/220~GHz.
\end{itemize}

Simons Observatory will achieve $\sigma(A_{\rm sh}) < 0.1$ by 2027, enabling 
$>$10$\sigma$ confirmation or exclusion.

\subsection{What this measurement establishes}

The ACT/Planck asymmetry demonstrates that high-$\ell$ CMB constraints 
depend on angular resolution. At $\ell > 2000$, ACT's 1.4$'$ beam provides 
3--5$\times$ better sensitivity than Planck's 5$'$ beam.

The $H_0$ and $\sigma_8$ values derived from the ACT-preferred parameters 
lie between CMB-only values (67.4, 0.83) and late-time measurements 
(JWST TRGB: $69.9 \pm 1.2$; weak lensing $S_8 \approx 0.76$). This positioning 
is a property of the fit.

SPT-3G results expected in 2025--2026 will provide the first independent test.
