...
\title{A Resolution-Dependent Damping-Tail Feature in ACT DR6\\
...
\begin{abstract}
We test for a specific, phase-coherent feature in the ACT DR6 damping tail 
using a single, theory-derived template. The template shape is fixed by a 
canonical early dark energy (EDE) model; only its overall amplitude 
$A_{\rm sh}$ is fit.

ACT DR6 TTTEEE combined with DESI BAO prefers this template with 
$A_{\rm sh} = 1.61 \pm 0.22$ (7.4$\sigma$, $\Delta\chi^2 = -474$ for one 
extra parameter). If interpreted literally, this preference corresponds 
to a pre-recombination modification with $H_0 = 70.7 \pm 0.5$~\kms\ and 
$\sigma_8 = 0.75 \pm 0.01$ in a minimal EDE model.

Planck TTTEEE+DESI does not recover the same feature. In the same EDE 
model the fit to Planck+DESI is degraded by $\Delta\chi^2 = +121$ relative 
to \lcdm, with 89\% of the cost arising from $\ell > 1500$ where Planck's 
beam suppresses CMB power by more than $100\times$ relative to ACT.

Taken together, these results indicate a resolution-dependent discrepancy 
in how current CMB experiments constrain small-scale pre-recombination 
physics. We provide the template and fitting methodology so that SPT-3G, 
Simons Observatory, and CMB-S4 can test whether the ACT preference is 
confirmed with independent systematics or instead points to an ACT-specific 
high-$\ell$ effect.
\end{abstract}
...
\end{equation}
with the improvement almost entirely localized to the ACT TT damping tail at $\ell \gtrsim 1500$. At fixed \lcdm\ background the raw sensitivity is even higher, with $\Delta\chi^2 \simeq -770$ for one additional parameter, reflecting the large number of high signal-to-noise modes in the ACT damping tail.

Our second main result is that a minimal physical EDE model can interpret this template preference without overfitting. We consider a monodromy-inspired scalar field model that adds a single free parameter controlling the height of an early-time energy-density shelf. When this parameter is allowed to vary, the ACT+DESI data select a low-$\Lambda$ solution that recovers about $95\%$ of the fixed-background template improvement while satisfying DESI BAO, supernova, and Planck lensing constraints. The corresponding cosmology has $H_0 = 70.7 \pm 0.5$~\kms\ and $\sigma_8 = 0.75 \pm 0.01$, values that simultaneously relieve the $H_0$ and $S_8$ tensions when ACT provides the high-$\ell$ anchor.

Our third main result is that Planck does not provide an equivalent test of the same feature. When we replace ACT with Planck TTTEEE high-$\ell$ while keeping the rest of the pipeline unchanged, the same EDE model is disfavored by $\Delta\chi^2 = +121$ relative to \lcdm. A detailed decomposition shows that $89\%$ of this penalty arises from $\ell > 1500$, where Planck's $\sim 5'$ beam suppresses CMB power by more than two orders of magnitude relative to ACT's $\sim 1.4'$ beam and the spectra are noise dominated after beam deconvolution. In this regime a coherent 1\% oscillatory template cannot be cleanly tested. Fitting such a pattern to noise generically incurs a positive $\chi^2$ cost, even if the underlying sky contains the ACT-preferred modulation.
...
\textbf{Template provenance:} The EDE parameters are fixed to canonical 
Planck--era values ($\Lambda_{\rm EDE} = 0.10$, $\theta_i = 2.0$, $n = 3$) 
and the template shape was frozen before examining ACT DR6; only the amplitude 
$A_{\rm sh}$ is fit to the data.
...
cosmology is marginalized, and $\Delta\chi^2 \simeq -770$ at fixed background.
A concrete low--$\Lambda$ EDE model with one extra parameter reproduces
$\simeq 95\%$ of this fixed--background improvement while also improving the fit
...
At fixed \lcdm\ cosmology, the template amplitude shows even higher raw 
sensitivity:\footnote{At fixed cosmology, $A_{\rm sh} = 1.10 \pm 0.04$ 
($27.7\sigma$, $\Delta\chi^2 \approx -770$). This raw sensitivity metric 
demonstrates the underlying signal strength, but the marginalized 7.4$\sigma$ 
value is the scientifically relevant statistic since cosmology parameters are 
never perfectly known.} The reduction from this raw sensitivity to 
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/act_dr6_spectrum_residuals.pdf}
...
the lower $\sigma_8$ value and helps resolve the $S_8$ tension. BAO and SNe 
are neutral within $\pm 1$ $\chi^2$ unit (Figure~\ref{fig:chi2decomp}). 
...
a discrepancy. The key point is that the EDE model's lower $\sigma_8$ naturally 
relieves the Planck Lensing tension, providing independent validation of the 
model's structure formation predictions.
...
do we get a clean $\Delta\chi^2 = -766$ for the spectral pattern itself. 
...
sound-horizon recalibration, not a localized spectral effect. (See 
Appendix~\ref{app:act_only} for detailed ACT-only fits and parameter trajectories.)
...
than the prior, entirely data-driven. This maps to:
\begin{center}
\begin{tabular}{lcccc}
\toprule
...
The $\chi^2$ decomposition (Table~\ref{tab:chi2decomp}) shows that 90\% of the $\Delta\chi^2$ 
...
($\Delta\chi^2 = +282$). The large improvement appears only when ACT is combined 
with DESI. A pure damping-tail artifact would show up as an ACT-only preference. 
Instead, the EDE model wins precisely when ACT's high-$\ell$ spectra are tied to 
DESI's geometric distances. This is the signature of a true sound-horizon 
recalibration rather than a foreground or beam effect.
...
(70.7, 146)$~Mpc, via the $\Delta\chi^2 = -766$ 
...
with weak lensing measurements from DES, KiDS, and HSC. EDE can 
simultaneously address both the $H_0$ and $S_8$ tensions because 
the mechanism that shrinks the sound horizon also suppresses 
late-time structure growth.

\subsection{Predictions for future experiments}

The template amplitude $A_{\rm sh}$ can be measured by any high-resolution 
CMB experiment. We provide predictions assuming the ACT preference is physical:
\begin{itemize}
\item \textbf{SPT-3G:} $A_{\rm sh} = 1.0$--$1.5 \pm 0.3$ at $\ell > 2500$.
\item \textbf{Simons Observatory:} $\sigma(A_{\rm sh}) < 0.1$.
\item \textbf{CMB-S4 EE:} $-4.4\%$ power in the EE damping tail at $\ell > 1500$.
\item \textbf{CMB lensing:} $+13\%$ enhanced power at $L = 100$--500.
\end{itemize}
