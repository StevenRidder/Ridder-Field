...
\begin{abstract}
We detect a phase-coherent damping-tail pattern in ACT DR6 at 8.1$\sigma$ 
significance. The pattern matches a fixed template $T_\ell^{\rm soft}$ 
derived from early-time physics that predicts a percent-level reduction 
in the sound horizon $r_s$. Fitting a single amplitude $A_{\rm sh}$ to 
ACT DR6 yields $A_{\rm sh} = 1.54 \pm 0.19$ with $\Delta\chi^2 = -766$ 
relative to \lcdm, localized entirely to the damping tail ($\ell > 1500$).

The detection is robust:
\begin{itemize}
\item \textbf{Not noise:} $P < 10^{-4}$ from 10,000 Monte Carlo realizations.
\item \textbf{Not foregrounds:} Achromatic at 90 and 150~GHz; 220~GHz shows 
expected CIB excess.
\item \textbf{Phase-coherent:} 10.5$\sigma$ rejection of scrambled templates.
\item \textbf{Not DESI-manufactured:} $\Delta\chi^2 \approx -1{,}500$ with 
geometry frozen.
\end{itemize}

Planck cannot test this signal. At $\ell = 3000$, Planck's beam suppresses 
power by $>$100$\times$ relative to ACT, leaving Planck noise-dominated in 
the damping tail. The $\Delta\chi^2 = +121$ penalty Planck assigns to the 
ACT-preferred amplitude arises from fitting signal into noise---exactly 
what resolution physics predicts.

If physical, this detection implies $H_0 = 70.7 \pm 0.5$~\kms\ and 
$\sigma_8 = 0.753 \pm 0.012$ via a simple early dark energy model, 
resolving both the Hubble and $S_8$ tensions simultaneously. SPT-3G will 
confirm or refute by 2026 with independent systematics.
\end{abstract}
...
In this paper, we report the detection of exactly this signature in ACT DR6. 
Fitting a fixed, theory-motivated damping-tail template to ACT high-$\ell$ 
data yields:

\begin{itemize}
\item \textbf{Detection:} $A_{\rm sh} = 1.54 \pm 0.19$ (8.1$\sigma$), with 
$\Delta\chi^2 = -766$ relative to \lcdm.
...
signal is in the CMB spectra, not manufactured by DESI.
\end{itemize}

We subject this detection to rigorous null tests:

\begin{itemize}
\item \textbf{Monte Carlo:} $P < 10^{-4}$ from 10,000 \lcdm\ realizations.
\item \textbf{Phase scrambling:} 10.5$\sigma$ rejection of random-phase templates.
\item \textbf{Frequency splits:} 90 and 150~GHz yield consistent amplitudes; 
220~GHz shows expected CIB contamination.
\end{itemize}

Planck cannot test this signal. At $\ell = 3000$, Planck's 5$'$ beam 
suppresses power by $>$100$\times$ relative to ACT's 1.4$'$ beam, leaving 
Planck noise-dominated in the damping tail. The Planck penalty 
($\Delta\chi^2 = +121$) arises entirely from this resolution mismatch, 
not from disagreement with the physics.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Why Planck Cannot Test This Signal}
\label{sec:planck}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Planck's 5$'$ beam suppresses power by $>$100$\times$ relative to ACT at 
$\ell = 3000$, leaving Planck noise-dominated in the damping tail where 
the detection is localized. This section quantifies the expected penalty 
when forcing signal into noise.

\subsection{The predicted penalty}
...
The detection reported in this paper---$A_{\rm sh} = 1.54 \pm 0.19$ at 
8.1$\sigma$---has direct implications for cosmology if it reflects 
pre-recombination physics rather than an ACT-specific systematic.

\subsection{Resolution of the Hubble tension}

Mapping the detected amplitude onto the underlying early dark energy model 
yields $H_0 = 70.7 \pm 0.5$~\kms. This value:
\begin{center}
\begin{tabular}{lcc}
\toprule
...
\begin{itemize}
\item \textbf{Highly significant:} $A_{\rm sh} = 1.54 \pm 0.19$, 
$\Delta\chi^2 = -766$.
\item \textbf{Not noise:} $P < 10^{-4}$ from 10,000 Monte Carlo realizations.
\item \textbf{Not foregrounds:} Achromatic at 90/150~GHz.
\item \textbf{Phase-coherent:} 10.5$\sigma$ rejection of scrambled templates.
\item \textbf{Internal to ACT:} Survives with $\Delta\chi^2 \approx -1{,}500$ 
at fixed geometry.
\end{itemize}

Planck cannot test this signal due to beam suppression at $\ell > 2000$. 
The Planck penalty ($\Delta\chi^2 = +121$) is exactly what resolution 
physics predicts when fitting signal into noise.

\subsection{Physical implications}

If the detection is physical, it implies:
\begin{itemize}
\item $H_0 = 70.7 \pm 0.5$~\kms\ (resolving the Hubble tension)
\item $\sigma_8 = 0.753 \pm 0.012$ (resolving the $S_8$ tension)
\item A 1\% reduction in the sound horizon via early dark energy
\end{itemize}

\subsection{Confirmation}

SPT-3G will confirm or refute this detection by 2026 with independent 
beams, noise, and foreground controls.

\textbf{Prediction:} $A_{\rm sh} = 1.0$--$1.5 \pm 0.3$ at $\ell > 2500$.

Simons Observatory will achieve $\sigma(A_{\rm sh}) < 0.1$ by 2027, 
providing a definitive test. CMB-S4 will map the damping tail with 
unprecedented precision.

If SPT-3G confirms ACT, the case for pre-recombination new physics 
becomes compelling. If SPT-3G sees null, an ACT-specific systematic 
must be identified. 
