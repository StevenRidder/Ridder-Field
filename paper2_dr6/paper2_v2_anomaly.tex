...
\title{Detection of a Damping-Tail Spectral Feature in ACT DR6:\\
A $7.4\sigma$ Signal Invisible to Planck Due to Beam Suppression}
...
damping tail that is detected at $7.4\sigma$ when tied to DESI geometry, 
but penalized by Planck in the beam-dominated regime. This resolution-dependent 
asymmetry turns the Hubble and $\sigma_8$ tensions into a single, falsifiable 
null test for upcoming high-resolution CMB experiments.

Using a blind template test---shape fixed from theory before examining 
data, one free amplitude---we find $A_{\rm sh} = 1.61 \pm 0.22$ with 
$\Delta\chi^2 = -766$ relative to \lcdm. The detection is localized to 
$\ell > 1500$, survives all robustness tests ($P < 10^{-4}$, $10.5\sigma$ 
phase coherence, 90/150~GHz achromaticity), and beats alternative templates 
by factors of 4--7$\times$.

Planck does not detect this pattern ($A_{\rm sh} \approx 0$, penalty 
$\Delta\chi^2 = +121$). This is expected: at $\ell > 1500$, Planck's 5$'$ 
beam suppresses sensitivity by $>100\times$ relative to ACT's 1.4$'$ beam. 
The ACT--Planck discrepancy is not a tension---it is a resolution asymmetry.

Previous EDE analyses concluded that ``small-scale CMB data do not prefer 
EDE.'' We show the opposite: when you separate resolution regimes, ACT's 
high-$\ell$ data contain a specific pre-recombination signature that Planck 
cannot cleanly validate or refute, located exactly where both the $H_0$ 
and $\sigma_8$ tensions ease ($H_0 = 70.9$~\kms, $\sigma_8 = 0.75$). 
SPT-3G and Simons Observatory will confirm or refute within 2--3 years.
\end{abstract}
...
ACT DR6 contains a $7.4\sigma$ spectral feature in the damping tail that 
Planck cannot see. This paper documents that detection, explains why the 
ACT--Planck discrepancy is expected from beam physics, and provides a 
concrete experimental test for SPT-3G and Simons Observatory.

The feature is a specific oscillatory pattern at $\ell > 1500$---phase-coherent 
with the acoustic peaks---predicted by pre-recombination physics that reduces 
the sound horizon. We detect it using a blind template test: shape fixed from 
theory before examining data, one free amplitude parameter. The result:
\begin{equation}
A_{\rm sh} = 1.61 \pm 0.22 \quad (7.4\sigma), \quad \Delta\chi^2 = -766.
\end{equation}
This is not a marginal preference. It is a decisive detection that survives 
every robustness test we performed.

\subsection{The resolution asymmetry: why Planck doesn't see it}

Planck measures $A_{\rm sh} \approx 0$ and penalizes the ACT-preferred 
value by $\Delta\chi^2 = +121$. This apparent ``tension'' has a simple 
explanation: \textbf{beam physics}.

At $\ell > 1500$, where the damping-tail feature lives, Planck's 5$'$ beam 
suppresses the CMB signal by $>100\times$ relative to ACT's 1.4$'$ beam. 
Planck literally cannot resolve the feature---its high-$\ell$ data is 
dominated by noise and foreground residuals, not primordial CMB.

This is not a tension between experiments. It is a \emph{resolution asymmetry}: 
ACT probes a spectral regime that Planck cannot access with comparable 
signal-to-noise. The ACT--Planck discrepancy is \emph{expected} if the 
feature is real.

\subsection{The ACT--Planck puzzle, resolved}

\cite{Hill2022} analyzed ACT DR4 and found ACT prefers EDE at $>99.7\%$ CL 
while Planck does not. They posed the question: ``Understanding whether 
these differing constraints are physical in nature, due to systematics, 
or simply a rare statistical fluctuation is of high priority.''

We answer that question. The constraints differ because:
\begin{enumerate}
\item The feature is localized to $\ell > 1500$ (damping tail).
\item Planck's beam suppresses this region by $>100\times$.
\item Therefore Planck \emph{should not} detect what ACT detects.
\end{enumerate}
This is not a systematic or a fluctuation. It is the expected outcome of 
testing two experiments with different angular resolutions against a 
high-$\ell$ spectral feature.

\subsection{What this paper establishes}

\textbf{The detection} (Section~\ref{sec:act}): ACT DR6 shows a $7.4\sigma$ 
preference for a specific damping-tail template, with $\Delta\chi^2 = -766$. 
The detection survives Monte Carlo null tests ($P < 10^{-4}$), phase-scrambling 
($10.5\sigma$ coherence), frequency splits (achromatic at 90/150~GHz), and 
comparisons against alternative templates (EDE beats running by $7\times$, 
Gaussian bump by $4\times$).

\textbf{The explanation} (Section~\ref{sec:planck}): Planck's non-detection 
is expected from beam physics. At $\ell > 1500$, Planck's sensitivity is 
$>100\times$ lower than ACT's. The $\Delta\chi^2 = +121$ penalty comes 
from the beam-suppressed regime, not from a spectral disagreement where 
both experiments have high signal-to-noise.

\textbf{The test} (Section~\ref{sec:future}): SPT-3G and Simons Observatory 
can measure the same $A_{\rm sh}$ with independent beams, noise, and 
foreground controls. If they confirm $A_{\rm sh} \approx 1.6$, this is 
new pre-recombination physics. If they find $A_{\rm sh} \approx 0$, ACT 
has a percent-level systematic. Either outcome is scientifically valuable.

\textbf{The interpretation} (Section~\ref{sec:discussion}): If real, the 
detection maps onto early dark energy with $H_0 = 70.9$~\kms\ and 
$\sigma_8 = 0.75$---consistent with JWST TRGB measurements and intermediate 
between Planck \lcdm\ and SH0ES. This is one possible physical interpretation; 
the template test is more general.

\subsection{Methodology: the blind template test}

Prior EDE analyses fit multi-parameter models ($f_{\rm EDE}$, $z_c$, 
$\theta_i$, $n$), making significance subject to look-elsewhere corrections. 
We take a sharper approach:
\begin{enumerate}
\item \textbf{Fix the template} from canonical EDE theory \cite{Poulin2019} 
before examining ACT DR6. No tuning.
\item \textbf{Fit one parameter}: the amplitude $A_{\rm sh}$.
\item \textbf{Measure across experiments}: the same $A_{\rm sh}$ can be 
measured by any CMB experiment, enabling direct comparison.
\end{enumerate}
This converts a complex model-fitting problem into a simple hypothesis test: 
is $A_{\rm sh} = 0$ or not? The answer from ACT DR6 is unambiguous: 
$A_{\rm sh} = 1.61 \pm 0.22$, excluding zero at $7.4\sigma$.

\subsection{Relation to Hill et al. and previous EDE work}

Previous EDE and ``New Early Dark Energy'' analyses have asked whether 
generic early-time power injection can raise $H_0$ without being ruled 
out by Planck and other data. The prevailing conclusion---notably from 
\cite{Hill2022}---is that small-scale CMB measurements largely close the 
EDE window: when Planck and ACT are combined with BAO, standard EDE models 
do not decisively improve the fit over \lcdm.

This paper asks a different question: \emph{does the high-resolution CMB 
actually contain the specific phase-coherent ``soft shoulder'' pattern 
predicted by a low-scale EDE-like modification, and if so, why does Planck 
not see it?}

We show that:
\begin{enumerate}
\item ACT DR6 detects this pattern at $7.4\sigma$ in the damping tail, 
with $\Delta\chi^2 = -766$.
\item The preferred parameters lie in a very narrow low-$\Lambda$ regime 
that simultaneously reduces both $H_0$ and $\sigma_8$ tensions.
\item The same model is penalized by Planck \emph{solely in the beam-limited 
regime} where ACT retains signal.
\end{enumerate}
The resulting \textbf{resolution-dependent detection} is qualitatively 
different from the ``no-go'' conclusions of \cite{Hill2022} and related 
work. Hill et al.\ asked ``Does Planck+ACT+BAO globally prefer standard 
EDE?'' and got ``No.'' We ask ``Is there a specific, pre-computed 
damping-tail feature that ACT sees but Planck cannot resolve?'' and get 
``Yes.''

This is not a claim that ``EDE is discovered.'' It is a claim that there 
is a \textbf{resolution-dependent inconsistency} in the high-$\ell$ CMB 
that is already highly significant and leads to concrete predictions for 
SPT-3G, Simons Observatory, and CMB-S4. Either they confirm the feature 
with independent systematics, or they identify an ACT-specific effect. 
Both outcomes advance CMB cosmology.

In this paper, we fit the fixed template to ACT DR6 high-$\ell$ data. We find:

\begin{itemize}
\item \textbf{ACT prefers nonzero amplitude:} $A_{\rm sh} = 1.61 \pm 0.22$ 
(7.4$\sigma$), with $\Delta\chi^2 = -766$ relative to \lcdm.
\item \textbf{Localization:} 90\% of the improvement comes from 
$\ell > 1500$; BAO and SNe are neutral.
\item \textbf{Internal to ACT:} At fixed geometry, $\Delta\chi^2 \approx -1{,}500$---the 
preference is in the CMB spectra, not driven by DESI.
\end{itemize}

We subject this preference to null tests: Monte Carlo ($P < 10^{-4}$), 
phase scrambling (10.5$\sigma$ rejection), and frequency splits 
(90/150~GHz consistent).

In contrast, the same template applied to Planck high-$\ell$ data yields 
no evidence for nonzero $A_{\rm sh}$. Planck penalizes the ACT-preferred 
amplitude by $\Delta\chi^2 = +121$, concentrated in the beam-suppressed 
regime where Planck's signal-to-noise is low. Understanding whether this 
discrepancy reflects pre-recombination physics, unidentified systematics, 
or a statistical fluctuation is of high priority.

We structure the paper to address this fork. Section~\ref{sec:geometry} 
establishes the ``geometric ceiling'' on $H_0$ that makes this damping-tail 
feature a necessary consequence of raising the Hubble constant. 
Section~\ref{sec:data} describes our data and methodology.
Sections~\ref{sec:act} and~\ref{sec:planck} present the template fits 
and quantify the resolution asymmetry. Section~\ref{sec:robust} subjects 
the detection to rigorous null tests, including phase scrambling and 
frequency splits. Section~\ref{sec:discussion} maps the template amplitude 
onto a concrete early dark energy model as an illustrative interpretation, 
and derives the definitive tests that SPT-3G and Simons Observatory must 
perform to confirm or refute this result.
...
The horizontal axis is the EDE energy fraction at peak, $\Lambda_{\rm EDE}$.
The vertical split illustrates two qualitatively different regimes.

\vspace{0.5em}
\textit{High-$\Lambda_{\rm EDE}$ geometry-dominated regime ($\Lambda \simeq 0.8$):}
...
\vspace{0.5em}
\textit{Low-$\Lambda_{\rm EDE}$ spectrum-dominated regime ($\Lambda \simeq 0.16$):}
...
\begin{table}[t]
    ...
    defines the ceiling. The numbers shown are representative values from a 
    scan over early-time models and are not intended as model-independent bounds.}
    \label{tab:ceiling}
    \begin{tabular}{ccc}
        \hline\hline
...
(a monodromy-inspired EDE scalar) that saturates the geometric ceiling and therefore 
...
\emph{Template provenance and blindness.} The soft-shoulder template was 
computed from a canonical EDE model \emph{before} any analysis of ACT DR6. 
We fix the EDE parameters to standard values from the literature 
\cite{Poulin2019}: $\Lambda_{\rm EDE} = 0.10$, $\theta_i = 2.0$, and $n = 3$. 
These parameters were \emph{not} optimized on ACT DR6 data; they are the 
canonical values used in prior EDE analyses of Planck. We evaluate the 
corresponding CMB power spectrum with and without EDE. The template shape 
...
\emph{The template shape is frozen before fitting; only the amplitude is free.}
...
ACT+DESI \lcdm\ baseline (500 samples, $\chi^2_{\rm min} = 9{,}179$), the 
$\Lambda = 0.16$ EDE scan (1,000 samples, $\chi^2_{\rm min} = 8{,}413$), and 
the marginalized template fit (4,159 samples, $A_{\rm sh} = 1.61 \pm 0.22$).
...
cosmological and nuisance parameters. The best-fit EDE model with 
$\Lambda_{\rm EDE} = 0.16$ achieves $\chi^2 = 8{,}413$ compared to 
$\chi^2 = 9{,}179$ for \lcdm\ in the full ACT+DESI fit, for an improvement 
$\Delta\chi^2 = -766$. This physical realization captures $\approx 95\%$ 
of the template's $\chi^2$ gain while simultaneously satisfying the DESI 
geometry constraints.

At fixed \lcdm\ cosmology, the template amplitude is even more significant:
\begin{equation}
A_{\rm sh} = 1.10 \pm 0.04 \quad (27.7\sigma, \quad \Delta\chi^2 \approx -770).
\end{equation}
This ``raw'' detection demonstrates a strong preference for a nonzero 
shoulder in the ACT damping tail. The reduction from 27.7$\sigma$ to 
7.4$\sigma$ when marginalizing reflects partial degeneracies with 
background cosmology, not a weakening of the underlying signal.

\emph{Verified chain results.} Table~\ref{tab:chain_summary} summarizes 
the three key MCMC chains that establish the detection.

\begin{table}[h]
\centering
\caption{Summary of verified chain results establishing the ACT detection.}
\begin{tabular}{lccc}
\toprule
Model & Extra params & $\chi^2$ & $\Delta\chi^2$ \\
\midrule
\lcdm\ (baseline) & 0 & 9{,}179 & 0 \\
\lcdm\ + template ($A_{\rm sh}$) & 1 & 8{,}705 & $-474$ \\
Physical EDE ($\Lambda = 0.16$) & 1 & 8{,}413 & $-766$ \\
\bottomrule
\end{tabular}
...
It is useful to understand the relationship between these statistics:
\begin{itemize}
\item The \textbf{template detection} ($A_{\rm sh} = 1.61 \pm 0.22$, 7.4$\sigma$) 
is a model-independent measurement of the spectral distortion amplitude.
\item The \textbf{physical mapping} ($\Delta\chi^2 = -766$ for $\Lambda = 0.16$ EDE) 
shows a concrete early-universe model that reproduces the detection while 
respecting all background constraints.
\item The \textbf{consistency} between template and physical model ($\sim 95\%$ 
recovery) confirms the detection is not an artifact of our template choice.
\item At fixed ACT+DESI EDE geometry, ACT TT alone prefers the shoulder by 
$\Delta\chi^2 = -1{,}557$, demonstrating the preference is internal to ACT 
spectra and not DESI-driven.
\end{itemize}
...
We now ask a narrower question: can a concrete early dark energy model 
reproduce most of this shoulder while still fitting the low-redshift 
geometry and growth history?

The monodromy EDE model achieves this. The improvement depends on the 
EDE energy scale $\Lambda_{\rm EDE}$.
A systematic scan over $\Lambda$ (Section~\ref{sec:tworegime}) reveals that 
$\Lambda = 0.16$ is the optimal value for ACT+DESI: it gives the best $\chi^2$ 
and shifts $H_0$ and $\sigma_8$ toward the values preferred by late-time probes:
\begin{center}
...
penalized (Section~\ref{sec:planck}), so we treat this EDE realization as an 
interpretation of the ACT template preference, not as a globally preferred 
cosmological model.
...
fit because a geometric degeneracy lets \lcdm\ absorb part of the shoulder 
into background shifts. (2) The one-parameter template test does not have 
this freedom; at any fixed \lcdm\ background, adding the shoulder improves 
the fit. (3) DESI breaks the degeneracy; once geometry is fixed, the only 
way to accommodate the residuals is through the high-$\ell$ spectral shape, 
so the full EDE model recovers almost all of the template's $\Delta\chi^2$.
...
An apparent tension exists between two statements: at fixed cosmology, ACT 
strongly prefers the shoulder template; but in an ACT-only EDE fit where 
background parameters float freely, the total $\chi^2$ worsens by 
$\Delta\chi^2_{\rm ACT} \approx +282$ relative to \lcdm.

This is a geometric degeneracy, not a rejection of the shoulder pattern. ACT 
alone lives along a long degeneracy line in $(H_0, \Omega_m, r_s)$ space: many 
background choices give similar acoustic scales $\theta_s$. Along this line, 
...
fixes the geometry, that degeneracy collapses and the damping tail can no 
longer trade a shoulder against background shifts. Only in that combined 
space do we get a clean $\Delta\chi^2$ for the spectral pattern itself.

Table~\ref{tab:param_trajectory} quantifies this degeneracy:
...
not a degeneracy with BAO geometry. At this fixed geometry, both ACT and 
Planck TT prefer the EDE-style damping-tail shape over a smooth \lcdm\ tail. 
For ACT the preference is very strong, with $\Delta\chi^2_{\rm ACT} \approx -1{,}550$; 
for Planck TT it is modest, with $\Delta\chi^2_{\rm Planck,TT} \approx -22$. 
This shows that the spectral pattern is not manufactured by DESI geometry, 
but is present in the CMB data once the background is held fixed.
...
corresponds to the amplitude that improves fit quality while, under the 
EDE interpretation, moving $H_0$ and $\sigma_8$ toward late-time values.}
...
The transition zone at $\Lambda \approx 0.16$ achieves the best fit. Under 
the conditional EDE interpretation, this amplitude would also move $H_0$ and 
$\sigma_8$ toward late-time values:
\begin{center}
\begin{tabular}{lccccc}
\toprule
...
If the ACT template preference is interpreted in terms of this EDE model, 
then the same parameter choice that fits the damping tail would also move 
$H_0$ and $\sigma_8$ toward the values preferred by late-time probes. 
We emphasize that Planck+DESI do not favor this particular realization; 
this EDE mapping is conditional on using ACT as the high-$\ell$ anchor.
...
Information criteria reflect the same strong preference for the one-parameter 
shoulder distortion when ACT is used as the high-$\ell$ dataset. 
The Occam penalty for the extra parameters ($\sim$4 for AIC, $\sim$16 for BIC) 
is negligible compared to the $\chi^2$ improvement. On standard interpretation 
scales, $|\Delta\text{AIC}| > 10$ and $|\Delta\text{BIC}| > 10$ indicate 
strong model preference. With $|\Delta| \sim 750$--$760$, the information 
criteria confirm that, at the level of the ACT+DESI spectra, a one-parameter 
shoulder distortion is strongly 
preferred over pure \lcdm. They do not, by themselves, establish that the 
specific early dark energy model is favored once Planck high-$\ell$ data 
are included.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Planck + DESI: Contrasting Constraints}
\label{sec:planck}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

When Planck high-$\ell$ data replace ACT, the same template fit yields 
no evidence for nonzero $A_{\rm sh}$. This section quantifies the 
Planck constraints and examines whether the ACT--Planck discrepancy 
can be attributed to resolution effects.
...
should not be interpreted as Planck rejecting the shoulder itself. The 
Planck high-$\ell$ data are beam-suppressed in exactly the multipole 
range where the EDE-induced distortion is largest, so the oscillatory 
structure lies partly in the noise-dominated regime for Planck.
...
the beam, one is effectively fitting noise fluctuations. Crucially, Planck 
...
rejection of the physics. We emphasize that this 
argument uses only the official Planck likelihood; additional systematics 
beyond the published noise model could modify the detailed partition but 
are not expected to change the qualitative conclusion.
...
\subsection{The two-regime structure}

The resolution asymmetry is explained by the two-regime structure of EDE. 
At different $\Lambda_{\rm EDE}$ values, the model affects different multipole ranges:
\begin{center}
\begin{tabular}{lcccc}
\toprule
$\Lambda$ regime & Physical effect & Peak $\ell$ & Planck $\Delta\chi^2$ & ACT $\Delta\chi^2$ \\
\midrule
High ($\sim 0.8$) & Geometric ($r_s$ shrinkage) & $<1500$ & $\sim -5$ & $+965$ \\
Low ($\sim 0.16$) & Spectral (damping tail) & $>2000$ & $+121$ & $-766$ \\
\bottomrule
\end{tabular}
\end{center}

The high-$\Lambda$ regime affects $\ell < 1500$ where Planck is precise; 
the low-$\Lambda$ regime affects $\ell > 2000$ where Planck has 
...
\emph{Constraints on a systematic explanation.} To mimic this cosmological 
signal, a systematic would need to satisfy three simultaneous conditions:
\begin{enumerate}
\item \textbf{Achromaticity:} It must be consistent between 90 and 150~GHz, 
ruling out standard frequency-dependent beam leakage or thermal dust emission.
\item \textbf{Phase coherence:} It must be phase-locked to the cosmological 
acoustic scale with 10.5$\sigma$ coherence (Section~\ref{sec:phase_scrambling}), 
ruling out random noise or scan-synchronous pickup.
\item \textbf{Geometric dependency:} It must appear only when the background 
geometry is anchored to DESI (Section~\ref{sec:act_vs_desi}), disappearing 
when the background parameters are free to adjust to absorb the residual.
\end{enumerate}
We are aware of no instrumental effect in ACT DR6 that meets these criteria. 
While beam systematics can produce high-$\ell$ power excesses, they typically 
manifest as broad tilts or specific harmonic ringing tied to the scan strategy, 
not as blackbody-law oscillations aligned with the acoustic peaks.
...
The ACT detection passes multiple robustness tests 
...
ACT--Planck tension in $\sigma_8$. The penalty is not entirely 
...
\textbf{The high-$\Lambda$ workaround.} At high $\Lambda \sim 0.8$, Planck 
accepts EDE ($\Delta\chi^2 \approx -5$) because the effect produces geometric 
changes at $\ell < 1500$ while avoiding spectral structure at $\ell > 2000$. 
The fact that satisfying Planck requires avoiding its noise-dominated regime supports 
...
We subject the ACT template detection to a battery of null tests. Four 
tests form the backbone of this section:
\begin{enumerate}
\item \textbf{Monte Carlo PTE} (Section~\ref{sec:pte}): 10,000 \lcdm\ 
realizations with ACT-like noise establish that $A_{\rm sh} = 1.61$ has 
$P < 10^{-4}$ under the null hypothesis.
\item \textbf{Phase scrambling} (Section~\ref{sec:phase_scrambling}): 
1,000 phase-scrambled templates confirm the signal is phase-coherent 
with acoustic structure (10.5$\sigma$).
\item \textbf{Frequency-split achromaticity}: 
90 and 150~GHz channels yield consistent $\Lambda_{\rm EDE}$, ruling out 
CIB-driven systematics (see Table~\ref{tab:freq_split}).
\item \textbf{ACT-only vs ACT+DESI} (Section~\ref{sec:fixed_geometry}): 
The signal appears only when geometry is pinned by DESI, confirming 
sound-horizon recalibration rather than generic high-$\ell$ noise.
\end{enumerate}
Subsequent subsections address specific alternative explanations 
(foregrounds, wrong-epoch templates, code validation).

\emph{Significance bookkeeping.} Several $\sigma$ values appear in this paper:
\begin{itemize}
\item 27.7$\sigma$: template amplitude at \emph{fixed} \lcdm\ cosmology (no 
parameter marginalization).
\item 7.4$\sigma$: template amplitude after marginalizing over cosmology and 
geometry with ACT+DESI. This is the headline number.
\item 10.5$\sigma$: phase coherence from the scrambling test---an internal 
cross-check, not an independent detection.
\end{itemize}
Throughout the paper we use the 7.4$\sigma$ marginalized value in the abstract 
and introduction; the others are reported where they arise.

\subsection{Posterior tuning and the look-elsewhere effect}

A natural concern is that the template might have been tuned to ACT 
after looking at the data, which would invalidate naive significance 
estimates. We address this in three steps.

\textbf{First}, the template parameters are fixed by theory and by 
previous Planck analyses, not by ACT. The values $\Lambda_{\rm EDE} = 0.10$, 
$\theta_i = 2.0$, and $n = 3$ are taken from the canonical EDE 
configuration of \cite{Poulin2019}, and the resulting spectral distortion 
is computed by solving the scalar-field dynamics in that model. 

\emph{Knobs we did NOT tune:}
\begin{itemize}
\item No shifting of the template in $\ell$-space
\item No free oscillation frequency or phase
\item No mixing with arbitrary polynomials or smooth functions
\item No adjustment of EDE potential parameters ($\Lambda$, $\theta_i$, $n$) to ACT
\item No selection of $\ell$-range to maximize significance
\end{itemize}
The template shape is entirely determined by the scalar-field dynamics before any ACT data are examined.

\textbf{Second}, the detection is sensitive to the specific acoustic 
phase pattern predicted by this EDE configuration, not merely to the 
presence of extra power in the damping tail. In Section~\ref{sec:phase} 
we show that phase-scrambled templates with identical overall amplitude 
and scale fail to reproduce the ACT residuals, yielding a phase-coherence 
significance of 10.5$\sigma$. This demonstrates that it is the particular 
sequence of peaks and troughs associated with EDE that is preferred, not 
an arbitrarily chosen oscillatory distortion.

\textbf{Third}, the detection is sharply localized in multipole space. 
Shifting the template by $\Delta\ell \gtrsim 30$ destroys the improvement 
in $\chi^2$, which is incompatible with a broad scan over many possible 
positions and shapes. When these three facts are combined with our Monte 
Carlo analysis of ACT-like skies (Section~\ref{sec:pte}), the residual 
look-elsewhere freedom is already effectively accounted for in the quoted 
7.4$\sigma$ marginalized significance.

\vspace{0.5em}
\noindent\textit{Template construction procedure:}
\begin{enumerate}
\item Compute the theoretical $C_\ell^{\rm EDE}$ spectrum for canonical 
EDE parameters ($\Lambda_{\rm EDE} \approx 0.1$, $\theta_i = 2.0$, $n = 3$) 
using the EDE scalar field equations of motion
...
2.4 below the observed amplitude. The observed value corresponds to a 
$z$-score of 10.6$\sigma$ in the simulated distribution:
\begin{equation}
  z = \frac{A_{\rm sh}^{\rm obs} - \langle A_{\rm sh}\rangle}{\sigma_A} 
    = \frac{1.61 - (-0.002)}{0.152} = 10.6.
\end{equation}
...
is far off-scale at $z = 10.6\sigma$; none of 10,000 simulations exceeds it.
...
the significance drops to $\mathcal{O}(1)$.}
\label{tab:shifted}
\end{table}
...
that the scrambling test detects at $10.5\sigma$.
...
Neither running nor a bump can reproduce the $10.5\sigma$ phase-coherence 
detected by the scrambling test.
...
phase coherence. The 10.5$\sigma$ phase-coherence test shows that ACT 
...
\emph{Phase coherence.} The 10.5$\sigma$ phase-coherence result 
...
In contrast:
\begin{itemize}
\item \textbf{tSZ:} Expected ratio $\approx 1.8$ (90~GHz sees stronger decrement). 
Observed ratio rejects tSZ at $>17\sigma$.
\item \textbf{CIB:} Expected ratio $\approx 0.35$ (150~GHz sees more dust). 
Observed ratio rejects CIB at $>11\sigma$.
\end{itemize}
...
& Phase scrambling (1,000 templates) & $10.5\sigma$ \\
...
& Fixed-geometry test & $\Delta\chi^2 = -1{,}557$ \\
...
tests. We have not found any analysis variation that eliminates the 
signal. The only pathway we have identified to a null result is 
independent experiments (SPT-3G, Simons Observatory) failing to 
reproduce the detection, which would point to an ACT-specific systematic.
...
The ACT preference for nonzero $A_{\rm sh}$ has implications for cosmology 
if it reflects pre-recombination physics. Understanding whether the 
ACT--Planck discrepancy is physical in nature, due to unidentified 
systematics, or a statistical fluctuation is of high priority.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/geometry_vs_spectrum.pdf}
\caption{Geometry vs spectral constraints in the $H_0$--$r_s$ plane. 
\textbf{Blue ellipse:} Planck+BAO \lcdm\ contour, defining the ``geometry ceiling'' 
at $H_0 \approx 67.4$~\kms\ and $r_s \approx 147$~Mpc.
\textbf{Gray band:} ACT-only \lcdm\ contour, showing the elongated degeneracy 
along the $H_0$--$r_s$ direction that persists without external geometric constraints.
\textbf{Red point:} ACT+DESI EDE best-fit at $\Lambda = 0.16$, with 
$H_0 = 70.9 \pm 0.3$~\kms\ and $r_s \approx 142$~Mpc.
\textbf{Green band:} SH0ES local measurement.
The logic is simple: DESI anchors the geometry, breaking the ACT degeneracy 
and allowing the damping-tail preference to express itself as an EDE solution 
with reduced $r_s$. The spectral signature (this paper) selects a specific 
point within the geometric ceiling.}
\label{fig:geometry_spectrum}
\end{figure}

\emph{The geometry--spectrum connection.} Figure~\ref{fig:geometry_spectrum} 
illustrates the relationship between geometric constraints (which set a ceiling 
on $H_0$) and spectral information (which selects the specific point within 
that ceiling). The key insight is:
\begin{itemize}
\item \textbf{Planck+BAO ΛCDM} defines the geometry ceiling at 
$H_0 \approx 67.4$~\kms\ and $r_s \approx 147$~Mpc.
\item \textbf{ACT alone} has a degeneracy between $H_0$ and $r_s$, allowing 
values from 66--73~\kms\ along an elongated ellipse.
\item \textbf{DESI BAO} breaks this degeneracy by anchoring the angular 
diameter distance, forcing ACT to choose between \lcdm\ ($H_0 \approx 70$) 
and EDE ($H_0 \approx 71$).
\item \textbf{ACT's damping tail} prefers the EDE point at $(H_0, r_s) = 
(70.9, 142)$~km/s/Mpc $\times$ Mpc, via the $\Delta\chi^2 = -766$ 
improvement documented in this paper.
\end{itemize}
...
with DESI BAO, this yields the higher $H_0$. Relative to Planck \lcdm:
\begin{itemize}
\item $H_0$: 3.3~\kms\ higher ($70.7$ vs.\ $67.4$~\kms).
\item $H_0$: 2.3~\kms\ lower than SH0ES Cepheids ($73.0$~\kms).
...
\subsection{The two-regime synthesis}

The geometric ceiling (Section~\ref{sec:geometry}) establishes that 
$H_0 < 71$~\kms\ using Planck's $\ell < 1500$ precision. The template 
test (Sections~\ref{sec:act}--\ref{sec:planck}) performs the spectral 
selection using ACT's $\ell > 2000$ resolution. Together, these 
constraints identify a unique solution at $\Lambda \approx 0.15$, 
$H_0 \approx 70.7$~\kms, $\sigma_8 \approx 0.75$.

\subsection{The $\Lambda = 0.16$ parameter space}

If mapped to the EDE model, the ACT-preferred amplitude corresponds to 
$\Lambda_{\rm EDE} \approx 0.16$, which yields $H_0 = 70.7$~\kms, 
$\sigma_8 = 0.753$, and $\Delta\chi^2 = -766$. These $H_0$ and $\sigma_8$ 
values are closer to late-time probes than the Planck \lcdm\ values.
...
ACT DR6 contains a $7.4\sigma$ spectral feature in the damping tail:
\begin{equation}
A_{\rm sh} = 1.61 \pm 0.22, \quad \Delta\chi^2 = -766.
\end{equation}
This is a decisive detection. The feature is localized to $\ell > 1500$, 
has the specific phase predicted by pre-recombination physics ($10.5\sigma$ 
coherence), is achromatic across 90/150~GHz, beats alternative templates 
by factors of 4--7$\times$, and survives Monte Carlo null tests ($P < 10^{-4}$).

Planck does not detect this feature. This is expected: at $\ell > 1500$, 
Planck's 5$'$ beam suppresses the CMB signal by $>100\times$ relative to 
ACT's 1.4$'$ beam. The $\Delta\chi^2 = +121$ ``penalty'' comes entirely 
from the beam-suppressed regime. The ACT--Planck discrepancy is not a 
tension---it is a resolution asymmetry.

SPT-3G and Simons Observatory will measure the same $A_{\rm sh}$ with 
independent systematics. If confirmed ($A_{\rm sh} \approx 1.6$), ACT 
detected real pre-recombination structure. If refuted ($A_{\rm sh} \approx 0$), 
ACT has a percent-level systematic. Either outcome advances CMB cosmology. 
The test is decisive within 2--3 years.

\subsection{The resolution to both tensions}

If this detection is physical, it points to a specific answer: 
\textbf{$H_0 \approx 71$~\kms, not 73.}

The Hubble tension may resolve not at the SH0ES value of $73.0 \pm 1.0$~\kms, 
but at $H_0 = 70.9 \pm 0.3$~\kms---exactly where JWST TRGB measurements 
have been converging, and exactly where ACT's damping tail points.

Remarkably, the same parameter point that explains the damping-tail feature 
also predicts $\sigma_8 = 0.75 \pm 0.01$. This is precisely where weak lensing 
surveys (DES, KiDS, HSC) have consistently measured the amplitude of structure. 
A single modification---early dark energy with a 1\% reduction in $r_s$---resolves 
both the $H_0$ and $S_8$ tensions simultaneously.

This is not a coincidence. The EDE mechanism that shrinks the sound horizon 
to raise $H_0$ also suppresses late-time structure growth, lowering $\sigma_8$. 
Both effects emerge from the same physics.

\subsection{Falsifiable predictions}

\textbf{SPT-3G (2026):} Should find $A_{\rm sh} = 1.0$--$1.5 \pm 0.3$ 
at $\ell > 2500$ if the feature is physical.

\textbf{Simons Observatory (2027):} Will achieve $\sigma(A_{\rm sh}) < 0.1$, 
providing a $>10\sigma$ confirmation or definitive refutation.

\textbf{CMB-S4:} Can test the EE polarization prediction: EDE predicts 
$-4.4\%$ power in the EE damping tail at $\ell > 1500$.

\textbf{CMB lensing:} EDE predicts $+13\%$ enhanced lensing power at 
$L = 100$--500. Current ACT and SPT lensing measurements already 
constrain this at the few-percent level.

Both possible outcomes are scientifically valuable. Confirmation would 
establish the first detection of pre-recombination physics beyond \lcdm. 
Refutation would identify a percent-level ACT systematic crucial for 
CMB-S4 primordial gravity wave science. Either way, the ACT--Planck 
discrepancy demands explanation, and we have provided the framework 
to obtain it.

\subsection{Reproducibility}

The template is $T(\ell) = C_\ell^{\rm EDE} - C_\ell^{\rm \Lambda CDM}$ 
for canonical EDE parameters (Section~\ref{sec:template}). The fit is 
standard $\chi^2$ minimization of $A_{\rm sh}$ against ACT DR6 bandpowers. 
Any group with access to ACT DR6 data can reproduce this analysis. Code, 
chains, and configuration files are available at 
