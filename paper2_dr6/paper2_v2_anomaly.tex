...
Applied to ACT DR6, the template is strongly favored: $A_{\rm sh} = 1.54 \pm 0.19$ 
(8.1$\sigma$), with $\Delta\chi^2 = -766$ relative to pure \lcdm\ and the 
improvement localized to the damping tail ($\ell > 1500$). A fixed-geometry 
diagnostic confirms the signal is internal to ACT, not driven by distance data.

Applied to Planck, the same template is not detected. Planck prefers
$A_{\rm sh} \approx 0$ and penalizes the ACT-preferred amplitude with
$\Delta\chi^2 = +121$, of which about 90\% arises from high-$\ell$ TTTEEE
data in the beam-suppressed regime. This defines a resolution-dependent
asymmetry: ACT retains high signal-to-noise at $\ell \sim 3000$, while Planck
is noise-dominated once beam deconvolution is applied.

Monte Carlo simulations, phase-scrambled templates, and frequency-split fits
show that the shoulder detection in ACT is phase-coherent with the acoustic
peaks, statistically robust, and achromatic at 90 and 150~GHz. As a concrete
example, we map the ACT-preferred template amplitude onto a simple early dark
energy model, which yields $H_0 \simeq 70.7$~\kms\ and $\sigma_8 \simeq 0.75$
when combined with DESI BAO. We use this model only to illustrate how such 
a damping-tail preference would map onto pre-recombination physics; we do 
not claim that this model is favored when Planck high-$\ell$ data are included. 
We outline specific tests for SPT-3G, Simons Observatory, and CMB-S4 that 
can decide whether this template preference reflects pre-recombination 
physics or an ACT-specific systematic.
\end{abstract}
...
\begin{enumerate}
\item \textbf{A strong detection in ACT:} The template amplitude is preferred 
at 8.1$\sigma$ ($A_{\rm sh} = 1.54 \pm 0.19$), improving the fit by 
$\Delta\chi^2 = -766$ relative to \lcdm. This signal is internal to ACT: 
it persists with $\Delta\chi^2 \approx -1{,}500$ even when background 
geometry is frozen.

\item \textbf{A non-detection in Planck:} The same template applied to 
Planck yields $A_{\rm sh} \approx 0$ and a $\chi^2$ penalty, localized 
entirely in the beam-suppressed regime ($\ell > 1500$) where Planck 
lacks sensitivity.
\end{enumerate}
...
achieves the best fit \emph{and} would ameliorate both tensions if the 
template detection is cosmological:
\begin{center}
\begin{tabular}{lccccc}
\toprule
...
This shows that, in principle, early-time physics can ease both tensions 
simultaneously---but we emphasize that Planck+DESI do not favor this 
...
\subsection{Implications for $H_0$}
...
$H_0$ tension is solved: when Planck high-$\ell$ data replace ACT, the 
same low-$\Lambda$ EDE model is disfavored. What the data show, more 
modestly, is:
\begin{enumerate}
...
A null result from SPT-3G would identify an ACT-specific systematic effect 
important for CMB-S4 analysis. Confirmation with independent systematics 
would strongly favor a pre-recombination origin for the anomaly.
