...
\begin{abstract}
We test high-$\ell$ CMB spectra for a fixed damping-tail template
$T_\ell^{\rm soft}$ that encodes the predicted signature of a percent-level
reduction in the sound horizon $r_s$ at recombination. The template's shape
and phase are fixed from early-time physics before looking at any data, and
we introduce a single amplitude parameter $A_{\rm sh}$ that modulates this
template on top of a standard \lcdm\ spectrum.

Applied to ACT DR6, this one-parameter template improves the fit to the
high-$\ell$ spectra by $\Delta\chi^2 = -766$ relative to pure
\lcdm, with a best-fit amplitude
$A_{\rm sh} = 1.54 \pm 0.19$ (8.1$\sigma$ when cosmological parameters are
marginalized). The improvement is localized to the damping tail
($\ell \gtrsim 1500$) and persists when background geometry is held fixed,
which shows that the preference is internal to the ACT spectra rather than
driven by distance data.

Applied to Planck, the same fixed template is not required:
Planck high-$\ell$ data prefer $A_{\rm sh} \approx 0$ and penalize the
ACT-preferred amplitude by $\Delta\chi^2 = +121$, with $\sim 90\%$ of the
penalty arising from beam-suppressed multipoles at $\ell \gtrsim 1500$.
This defines a resolution-dependent asymmetry: ACT retains high
signal-to-noise in the damping tail, while Planck becomes noise dominated
once beam deconvolution is applied.

We subject the ACT result to Monte Carlo, phase-scrambling, and
frequency-split null tests, which show that the preferred template is
phase-coherent with the acoustic peaks, statistically robust, and
consistent between 90 and 150~GHz within the CMB-dominated bands.
As an illustrative mapping, we show how the ACT-preferred $A_{\rm sh}$
translates into parameters of a simple early dark energy model that sits
near the geometric ceiling for $H_0$ when combined with DESI BAO. The same 
model is disfavored when Planck high-$\ell$ data replace ACT, so we treat 
this mapping as a diagnostic example rather than as evidence for that 
specific model. Our primary result is the measurement of $A_{\rm sh}$ and 
the ACT--Planck asymmetry, which SPT-3G, Simons Observatory, and CMB-S4 
can test with independent high-resolution CMB data.
\end{abstract}
...
In this paper, we report that ACT DR6 high-$\ell$ data contain a strong, 
phase-coherent residual that matches the specific spectral signature of 
such a pre-recombination modification. When we fit a fixed, theory-motivated 
damping-tail template to the data, we find:

\begin{enumerate}
\item \textbf{ACT prefers a nonzero template amplitude:} Fitting the fixed
template to ACT DR6 high-$\ell$ spectra yields a best-fit amplitude
$A_{\rm sh} = 1.54 \pm 0.19$, which improves the total $\chi^2$ by
$\Delta\chi^2 = -766$ relative to \lcdm. The improvement is
concentrated in the damping tail, and survives at fixed background
geometry ($\Delta\chi^2 \approx -1{,}500$).

\item \textbf{Planck prefers zero amplitude and penalizes the same pattern:}
The same template applied to Planck high-$\ell$ data is consistent with
$A_{\rm sh} = 0$, and the ACT-preferred amplitude is disfavored with
$\Delta\chi^2 = +121$, with most of the penalty arising from
beam-suppressed multipoles ($\ell > 1500$) where Planck 
lacks sensitivity.
\end{enumerate}

This asymmetry defines a clean observational fork: either high-resolution 
experiments beyond ACT see the same template, or they do not. Future data 
can distinguish pre-recombination structure from ACT-specific systematics 
by repeating exactly this template test.
...
The core result of this paper is observational: ACT DR6 prefers a nonzero 
$A_{\rm sh}$ while Planck prefers $A_{\rm sh} \simeq 0$. In this section we 
explore what that measurement implies within one concrete early-time model. 
These parameter-level numbers are illustrative: the same EDE model is 
disfavored when Planck high-$\ell$ data replace ACT, so the mapping should 
not be read as a global solution.

\subsection{Implications for $H_0$ and $\sigma_8$}

If $A_{\rm sh}$ is mapped onto the EDE model, the ACT+DESI fit corresponds 
to $H_0 = 70.7 \pm 0.5$~\kms\ and $\sigma_8 = 0.753 \pm 0.012$. These values 
lie near the geometric ceiling for early-time models defined in 
Section~\ref{sec:geometry}. For reference, this $H_0$ is consistent with 
recent non-Cepheid measurements:
\begin{center}
\begin{tabular}{lcc}
\toprule
...
We have carried out a one-parameter template test for a specific
damping-tail pattern predicted by early-time modifications that shrink the
sound horizon. The template shape and phase were fixed from theory before
looking at the data, and a single amplitude $A_{\rm sh}$ was fitted on top
of \lcdm.

On ACT DR6, the template improves the fit to the high-$\ell$ spectra by
$\Delta\chi^2 = -766$ relative to \lcdm, with a best-fit amplitude
$A_{\rm sh} = 1.54 \pm 0.19$ and the improvement localized to the damping
tail. On Planck, the same template is not required and the ACT-preferred
amplitude is penalized by $\Delta\chi^2 = +121$, with most of the penalty
coming from multipoles where the beam strongly suppresses the signal. The
ACT result passes internal null tests based on Monte Carlo simulations,
phase scrambling, and frequency splits.

We have measured a specific, theory-predicted damping-tail pattern in ACT DR6 
and parameterized it with a single amplitude $A_{\rm sh}$. ACT prefers 
$A_{\rm sh} = 1.54 \pm 0.19$, while Planck prefers $A_{\rm sh} \simeq 0$ and 
penalizes the ACT-preferred value. This tension is concentrated in the 
beam-suppressed regime of the Planck spectra. A concrete early-time model can 
reproduce the ACT signal and move $H_0$ and $\sigma_8$ toward late-time probes, 
but fails when Planck high-$\ell$ data replace ACT. We therefore present the 
EDE mapping as an example, and leave it to SPT-3G, Simons Observatory, and 
CMB-S4 to decide whether the ACT damping-tail pattern is cosmological or 
experiment-specific.

\subsection{The observational test}

SPT-3G,
Simons Observatory, and CMB-S4 can repeat exactly the same template fit
with independent beams, noise, and foreground controls. A consistent
recovery of $A_{\rm sh} \neq 0$ across multiple high-resolution data sets
would strongly favor a pre-recombination origin for the damping-tail
pattern. A null result would instead point to an ACT-specific systematic
that future analyses will need to understand.
...
Taken at face value, the ACT result and the Planck result point to two broad 
classes of explanations:
\begin{enumerate}
\item \textbf{Pre-recombination physics:} A real damping-tail feature that 
only high-resolution experiments can resolve.
\item \textbf{ACT-specific systematic:} An effect that passes current 
achromaticity and phase-coherence checks but is not cosmological.
\end{enumerate}

We have structured this analysis so that both interpretations make clear, 
falsifiable predictions for upcoming data. SPT-3G (2026) and Simons Observatory 
(2027) can repeat exactly this template test with independent systematics. 
