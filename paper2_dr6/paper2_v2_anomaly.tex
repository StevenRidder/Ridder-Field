...
Applied to ACT DR6, the template is strongly favored. ACT prefers
$A_{\rm sh} = 1.54 \pm 0.19$ (8.1$\sigma$), improving the fit by
$\Delta\chi^2 = -766$ relative to pure \lcdm, with about 90\% of the gain
coming from high-$\ell$ temperature data ($\ell > 1500$). A fixed-geometry
test, in which background parameters are frozen to the ACT+DESI best-fit
values, shows that ACT spectra alone prefer the shoulder pattern by
$\Delta\chi^2 \approx -1{,}500$: the signal is internal to the damping tail,
not manufactured by low-redshift distance data.
...
when combined with DESI BAO; the same model is disfavored by Planck+DESI,
which penalizes its high-$\ell$ damping-tail structure. We outline specific
tests for SPT-3G, Simons Observatory, and CMB-S4 that can decide whether this
template preference reflects pre-recombination physics or an ACT-specific
systematic.
\end{abstract}
...
frequency splits. Finally, we derive the definitive tests that SPT-3G and 
Simons Observatory must perform to confirm or refute this result.
...
scales, $|\Delta\text{AIC}| > 10$ constitutes ``decisive evidence,'' and 
$|\Delta\text{BIC}| > 10$ constitutes ``very strong evidence.'' Our 
values of $|\Delta| \sim 750$--$760$ are orders of magnitude beyond 
these thresholds. These information criteria therefore confirm that, at the 
level of the ACT+DESI spectra, a one-parameter shoulder distortion is strongly 
