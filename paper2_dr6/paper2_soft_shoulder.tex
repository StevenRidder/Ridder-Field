...
\emph{ACT alone does not prefer EDE.} Without geometric constraints from BAO, 
the EDE model is penalized by $\Delta\chi^2 = +282$. The large improvement 
($\Delta\chi^2 = -766$) appears \emph{only} when DESI is included.

This is the signature of a genuine sound-horizon recalibration: the damping 
tail prefers a modified acoustic scale, but this preference becomes a penalty 
without geometric data to validate the new $r_s$ value. Once DESI confirms 
that the modified scale is geometrically consistent, the penalty flips to a 
large improvement.

A foreground or beam systematic would improve the ACT fit regardless of 
whether BAO data are included. Instead, the EDE model wins only when ACT's 
...
observed $A_{\rm sh} = 1.54$. The empirical probability to exceed is
\begin{equation}
...
configuration to mimic the phase-coherent acoustic structure of the template. A more 
extensive set of frequency-split and mask-split tests will be important for future 
work, but the tests we have performed so far favor a cosmological origin for the signal.
