...
\subsection{Scientific impact at three levels}

The detection of a soft shoulder in the CMB damping tail has implications 
at observational, theoretical, and foundational levels. We discuss each 
in turn.

\subsubsection{Observational implications}

The 7.8$\sigma$ detection of a phase-coherent oscillatory residual in 
ACT DR6 constitutes a null test failure of the standard cosmological 
model. This has immediate practical consequences for the experimental 
community.

First, the signal is not distributed randomly across the spectrum, 
which would indicate instrumental or astrophysical systematics. Instead, 
it is localized to the damping tail ($\ell > 1000$) where pre-recombination 
modifications are predicted to manifest. This spectral localization, 
combined with the phase coherence, strongly disfavors explanations 
based on foreground contamination or noise.

Second, the detected feature has a specific predicted morphology that 
can be independently verified. SPT-3G, with comparable angular resolution 
to ACT, should see the same pattern in its damping tail measurements. 
The Simons Observatory, with increased sensitivity, will provide a 
definitive test. CMB-S4, with its order-of-magnitude improvement in 
sensitivity, should detect or exclude this feature at $>50\sigma$.

Third, all experiments constraining cosmological parameters from 
damping tail observations must now account for this feature. Analyses 
that assume pure \lcdm\ in the high-$\ell$ regime may yield systematically 
biased parameter inferences. This includes constraints on the sum of 
neutrino masses, the effective number of relativistic species $N_{\rm eff}$, 
and primordial non-Gaussianity---all of which rely on precise modeling 
of the damping tail.

\subsubsection{Theoretical implications}

At the theoretical level, our results indicate that the sound horizon 
$r_s$ predicted by \lcdm\ is incorrect by approximately 1\%. This 
seemingly small discrepancy has far-reaching consequences.

The sound horizon is not an arbitrary parameter but a derived quantity 
determined by pre-recombination physics: the baryon density, the 
radiation temperature, and the detailed dynamics of recombination. 
An error in $r_s$ at the percent level indicates that our modeling 
of this epoch is incomplete. Something contributed to the expansion 
rate near $z \sim 3000$ that is not accounted for in the standard 
inventory of radiation, matter, and dark energy.

This 1\% recalibration propagates to all quantities calibrated against 
CMB distance scales. BAO surveys that infer expansion histories $H(z)$ 
and angular diameter distances $D_A(z)$ use $r_s$ as their standard 
ruler. If $r_s$ is 1\% smaller than assumed, these distance measurements 
shift accordingly. The effect is not large enough to invalidate existing 
analyses, but it is systematic and must be incorporated into precision 
cosmology.

Similarly, weak lensing analyses that use CMB-calibrated distances for 
photometric redshift calibration are affected. Stage IV dark energy 
surveys---DESI, Rubin/LSST, Euclid, and Roman---that rely on CMB priors 
for distance calibration must incorporate this systematic uncertainty. 
The $S_8$ tension between CMB and weak lensing may be partially explained 
by the same pre-recombination physics that produces the soft shoulder.

\subsubsection{Foundational implications}

At the deepest level, our detection implies that the pre-recombination 
universe contained an additional energy component---contributing 
approximately 5--10\% of the total energy density near $z \sim 3000$---beyond 
the standard inventory of photons, neutrinos, baryons, cold dark matter, 
and cosmological constant.

This is a remarkable claim. The standard picture of recombination-era 
physics has been well tested by measurements of the CMB acoustic peaks, 
Big Bang nucleosynthesis, and the primordial helium abundance. Yet 
our analysis indicates that something is missing.

The required component must satisfy stringent constraints. It must 
contribute at the percent level near recombination, dilute rapidly 
thereafter to avoid late-time effects, and produce the specific 
oscillatory signature we detect in the damping tail. A scalar field 
that briefly dominates before ``rolling'' to negligible density---the 
essence of early dark energy---naturally satisfies these requirements.

If confirmed by independent experiments, this would represent the 
first direct observational evidence for non-standard physics in the 
pre-recombination era since the discovery of cosmic acceleration. 
The detection motivates theoretical investigation of well-motivated 
field-theoretic completions: axion-like particles, ultralight scalars, 
early quintessence, and other candidates that could produce the 
observed signature while remaining consistent with particle physics 
constraints.
