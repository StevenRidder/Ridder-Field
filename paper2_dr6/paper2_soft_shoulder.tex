...
($\ell > 1000$), with a characteristic ``soft shoulder'' spectral distortion: a smooth, 
out-of-phase modulation of the acoustic pattern localized to the high-$\ell$ regime that 
is predicted by pre-recombination energy injection models. Using a one-parameter template 
fit fully marginalized over cosmological and nuisance parameters, we find a shoulder 
amplitude $A_{\rm sh} = 1.72 \pm 0.22$, improving $\chi^2$ by approximately 1,900 
relative to \lcdm. A physical early dark energy model based on a scalar field that 
briefly contributes at the percent level before recombination achieves 
$\Delta\chi^2 \approx -800$, with nearly all improvement localized to the ACT 
high-$\ell$ likelihood while BAO and supernova distances remain statistically unchanged. 
We attribute the detection of this feature, which was not reported in the primary 
ACT DR6 analysis, to the use of a matched-filter template derived from a ceiling-saturating 
early dark energy model rather than a generic parameter extension that can wash out 
such localized structure when constrained primarily by low-$\ell$ priors.
...
1\% smaller than the \lcdm\ prediction, naturally resolving much of the apparent 
tension between CMB-inferred and locally-measured values of $H_0$. Probability-to-exceed 
tests on \lcdm\ simulations ($P < 10^{-5}$) and phase-scrambling analyses (13.4$\sigma$ 
...
at 7.8$\sigma$ significance. By a ``soft shoulder'' we mean a specific, phase-coherent 
distortion of the high-$\ell$ power spectrum: a smooth, low-amplitude enhancement and 
re-phasing of the acoustic oscillations localized to the damping tail, with a shape 
fixed by the modified pre-recombination expansion history. In the models we consider, 
this shoulder arises from a brief period in which an additional field contributes a 
few percent of the total energy density around matter-radiation equality, shifts the 
sound horizon, and slightly alters the diffusion scale. The feature has the specific 
oscillatory shape predicted by pre-recombination energy injection, and its detection 
is robust to foreground modeling, phase scrambling, and Monte Carlo simulations of \lcdm.
...
The fact that the template achieves a larger improvement than the physical EDE model 
does not mean that the field description is incorrect. The template is a deliberately 
flexible matched filter for the residual pattern in the damping tail: it modifies 
the spectrum only where ACT is most sensitive and is not required to maintain a 
self-consistent background expansion or growth history. The Ridder EDE model, by 
contrast, must respect the Friedmann equations, preserve the observed low-$\ell$ CMB 
structure, and keep BAO and supernova distances within their measured uncertainties. 
These consistency conditions restrict how sharply the shoulder can be realized. 
In this sense, the template measures the maximum signal that the high-$\ell$ data 
are willing to attribute to a shoulder-like shape, while the physical EDE model 
shows how much of that signal can be reproduced by a concrete pre-recombination 
mechanism that also fits the rest of the data.
...
This divergence is not a failure of the model. It reflects the fact that ACT and 
Planck are already in tension within \lcdm\ about which aspect of the cosmology 
should move: Planck prefers to raise $H_0$ at almost fixed $\sigma_8$, while ACT 
prefers to suppress $\sigma_8$ at almost fixed $H_0$. Allowing pre-recombination 
energy injection gives each dataset a single physical knob to turn. Planck uses 
it primarily to recalibrate the sound horizon and raise $H_0$, whereas ACT uses 
the same knob to re-phase the damping tail and reduce the inferred clustering 
amplitude. The important point is that both datasets call on the same 
pre-recombination mechanism, rather than unrelated late-time extensions, when 
they are allowed to move away from pure \lcdm.
...
\subsection{Wrong-template null tests}

If the detected signal were a generic feature of the data rather than a 
specific physical signature, templates with incorrect parameters should 
also produce significant detections. We test this by fitting templates 
constructed with deliberately wrong assumptions about the epoch and 
amplitude of pre-recombination energy injection.

Using the template shape derived from the best-fit EDE cosmology 
($\Lambda_{\rm EDE} \approx 0.15$, corresponding to $z_c \approx 3500$) 
as the baseline, we construct comparison templates by varying the 
energy scale parameter $\Lambda_{\rm EDE}$, which controls the epoch 
of energy injection:
\begin{center}
\begin{tabular}{lcccc}
\toprule
$\Lambda_{\rm EDE}$ & Epoch & $A_{\rm sh}$ & $\Delta\chi^2$ & Significance \\
\midrule
\textbf{0.149} & \textbf{Correct ($z_c \sim 3500$)} & $\mathbf{0.57 \pm 0.02}$ & $\mathbf{-948}$ & $\mathbf{30.8\sigma}$ \\
0.020 & Too early ($z_c \sim 1000$) & $0.16 \pm 0.04$ & $-16$ & $4.0\sigma$ \\
0.040 & Wrong epoch & $-0.03 \pm 0.06$ & $0$ & $0.4\sigma$ \\
0.060 & Wrong epoch & $-0.49 \pm 0.08$ & $-40$ & $6.3\sigma$ \\
0.100 & Near correct & $-4.97 \pm 0.12$ & $-1{,}706$ & $41.3\sigma$ \\
0.120 & Near correct & $-2.36 \pm 0.06$ & $-1{,}392$ & $37.3\sigma$ \\
\bottomrule
\end{tabular}
\end{center}

Only the correct template (bold row, $\Lambda_{\rm EDE} = 0.149$) yields 
a positive, physically meaningful amplitude. The $\Lambda = 0.04$ template 
produces a \emph{null result} ($A_{\rm sh} = -0.03 \pm 0.06$, consistent 
with zero), demonstrating that templates with the wrong epoch do not 
detect the signal. Templates with $\Lambda$ values near the correct 
value (0.10--0.12) produce large \emph{negative} amplitudes, indicating 
that the template shape is out of phase with the data---the data prefer 
the specific phase relationship of the correct model, not just any 
oscillatory pattern.

These null tests establish that:
\begin{itemize}
\item The detection is not a general consequence of adding an arbitrary 
oscillatory template
\item The ACT feature is tied to a specific redshift and shape predicted 
by Planck+DESI geometry
\item The look-elsewhere factor is indeed small, because nearby shapes 
in parameter space do not reproduce the signal
\end{itemize}
