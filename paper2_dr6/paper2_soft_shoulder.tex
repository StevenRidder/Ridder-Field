...
specific physical signature, templates with incorrect phase or position 
in $\ell$-space should also produce significant detections. We test this 
by shifting the template pattern in bandpower space while keeping the 
same ACT data and covariance.

Define $t_0 = C_\ell^{\rm EDE} - C_\ell^{\rm \Lambda CDM}$ as the ``correct'' 
template derived from the best-fit EDE cosmology. We construct ``wrong'' 
templates by shifting this pattern by $\pm k$ bandpower bins: 
$t_{\rm wrong} = {\rm roll}(t_0, k)$. If ACT responds only to the specific 
phase and position of the soft shoulder, shifted templates should yield 
null results.
\begin{center}
\begin{tabular}{lccc}
\toprule
Template & Shift (bins) & $A_{\rm sh}$ & Significance \\
\midrule
\textbf{Correct (unshifted)} & \textbf{0} & $\mathbf{1.10 \pm 0.04}$ & $\mathbf{27.7\sigma}$ \\
Shifted & $+75$ & $0.00 \pm 0.03$ & $0.1\sigma$ \\
Shifted & $-30$ & $0.00 \pm 0.02$ & $1.8\sigma$ \\
Shifted & $-50$ & $-0.00 \pm 0.02$ & $2.4\sigma$ \\
Shifted & $+50$ & $-0.01 \pm 0.02$ & $5.5\sigma$ \\
\bottomrule
\end{tabular}
\end{center}

The unshifted template is detected at 27.7$\sigma$, while templates shifted 
by 30--75 bandpower bins yield amplitudes consistent with zero (0.1--2.4$\sigma$). 
This demonstrates that ACT responds to the \emph{specific phase and position} 
of the soft shoulder, not to arbitrary oscillatory patterns in the damping tail.

As a complementary test, we also vary the epoch of energy injection by 
changing $\Lambda_{\rm EDE}$ while keeping the baseline cosmology fixed. 
A template with $\Lambda_{\rm EDE} = 0.04$ (corresponding to a very different 
$z_c$) yields $A_{\rm sh} = -0.03 \pm 0.06$ (0.4$\sigma$), again consistent 
with zero. Only the epoch independently derived from Planck+DESI geometry 
produces a significant detection.
