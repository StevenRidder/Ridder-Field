...
\textbf{Figure 5: THE MONEY PLOT --- ACT DR6 TT Residuals}\\[1em]
\textbf{Panel A:} ACT DR6 TT power spectrum (black points) with \lcdm\ (blue) 
and EDE $\Lambda = 0.14$ (red) best-fits. Range: $\ell = 500$--$4000$.\\[0.5em]
\textbf{Panel B:} Residuals (data $-$ \lcdm). The oscillatory pattern at 
$\ell > 2000$ matches the EDE shoulder prediction (red curve). 
Shoulder region ($\ell = 2000$--$3500$) highlighted.\\[0.5em]
\textbf{Panel C:} Fractional residuals showing the $\sim$1\% enhancement 
in the damping tail that EDE explains.
\vspace{4cm}}}
\caption{ACT DR6 TT residuals reveal the soft shoulder. The oscillatory 
pattern at $\ell > 2000$ is fit by EDE (red) but not by \lcdm\ (horizontal 
zero in residual panels). This is the spectral feature that drives the 
$\Delta\chi^2 = -766$ improvement and the 8.1$\sigma$ template detection.}
...
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{3cm}
\textbf{Figure 6: $\Lambda_{\rm EDE}$ Posterior Distribution}\\[0.5em]
1D marginalized posterior from ACT+DESI\\[0.5em]
Peak: $\Lambda = 0.144$\\
68\% CI: $[0.139, 0.151]$\\
95\% CI: $[0.137, 0.153]$\\[0.5em]
Gray band: Prior range $[0.08, 0.25]$\\
Prior is 15$\times$ wider than posterior
\vspace{3cm}}}
\caption{Posterior distribution of $\Lambda_{\rm EDE}$ from ACT+DESI. 
The data strongly prefer $\Lambda \approx 0.14$--$0.15$, with the posterior 
width (0.012) much narrower than the prior (0.17). This demonstrates the 
constraint is data-driven, not prior-driven. The narrow peak corresponds 
to the ``sweet spot'' that simultaneously improves fit quality and reduces 
both $H_0$ and $\sigma_8$ tensions.}
...
Any frequency dependence would indicate foreground contamination.
...
\textbf{Both outcomes are scientifically important.}

If the detection is real, it opens a new window on pre-recombination physics 
and resolves two major cosmological tensions simultaneously. If it proves 
systematic, it reveals a subtle, percent-level high-$\ell$ effect that will 
be crucial for all future ground-based CMB work. Even if EDE is ultimately 
disfavored, the geometric ceiling on $H_0$, the interplay between damping 
tail and BAO, and the template methodology developed here remain valuable 
contributions to the field.

If future SPT and ACT releases fail to confirm this feature at the predicted 
amplitude ($A_{\rm sh} = 1.0$--$1.5$ at $\ell > 2500$), that will place some 
of the strongest constraints yet on percent-level pre-recombination physics. 
In that case, the main legacy of this work would be methodological, not a 
detection claim.
