...
\begin{abstract}
We test a fixed, theory-derived damping-tail template in ACT DR6 TTTEEE, 
allowing only its overall amplitude $A_{\rm sh}$ to vary. ACT DR6 combined 
with DESI BAO prefers this template at $A_{\rm sh} = 1.61 \pm 0.22$ 
($\Delta\chi^2 = -474$ for one extra parameter), with the improvement 
localized to $\ell \gtrsim 1500$. 

Planck TTTEEE+DESI, analyzed with the same template, does not recover the 
feature and instead incurs $\Delta\chi^2 = +121$, with 89\% of the cost 
arising from beam-suppressed multipoles $\ell > 1500$. 

Our primary result is the detection of a fixed, theory-derived template 
in ACT; we use a minimal early dark energy model only as one concrete 
mapping between the template amplitude and cosmological parameters. 
We interpret the ACT--Planck difference as a resolution-dependent 
discrepancy and provide the template and fitting methodology so that 
SPT-3G, Simons Observatory, and CMB-S4 can measure $A_{\rm sh}$ with 
independent systematics. When mapped onto a minimal EDE model, this amplitude corresponds to 
$H_0 \simeq 70.7$~\kms\ and $\sigma_8 \simeq 0.75$.
\end{abstract}
...
with acoustic structure (no scrambled template recovers the signal).
...
\begin{itemize}
\item \textbf{Planck+BAO \lcdm} defines the geometry ceiling at 
...
\emph{Public data products:}
\begin{itemize}
\item ACT DR6: \url{https://lambda.gsfc.nasa.gov/product/act/}
\item Planck 2018: \url{https://pla.esac.esa.int/}
\item DESI Year 1 BAO: \url{https://data.desi.lbl.gov/}
\end{itemize}

\emph{This analysis:}
\begin{itemize}
\item Modified \textsc{CLASS} code: \url{https://github.com/StevenRidder/Ridder-Field}
\item MCMC chains: Zenodo [DOI upon acceptance]
\item Analysis scripts and configuration files: included in GitHub repository
\end{itemize}
