...
The same physics predicts a characteristic ``soft shoulder'' in the CMB damping tail at the one-percent level. We construct a fixed template for this ripple pattern and fit its amplitude to ACT DR6 TT+EE data in a \emph{conditional} analysis with fixed background cosmology, finding a $6.4\sigma$ preference for a nonzero shoulder aligned with the Geometric EDE template; a fully marginalized analysis may reduce this significance. Taken together, the geometric ceiling, the $H_0$--$S_8$ convergence window, and the soft-shoulder correlation make early-time geometry a concrete and falsifiable alternative to purely late-time or systematic explanations of the current anomalies and provide a sharp target for upcoming CMB-S4 observations.
\end{abstract}
