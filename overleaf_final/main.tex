...
\begin{abstract}
We introduce a scalar field that ameliorates both the Hubble and $S_8$ tensions by modifying the pre-recombination expansion. The field reduces the sound horizon by $\sim 1\%$, raising $H_0$ to $69$--$70$~km~s$^{-1}$~Mpc$^{-1}$---the JWST/TRGB convergence window---while lowering $S_8$ to $0.82$. Pre-DESI, the model achieves $\Delta\chi^2 \approx -4$; with DESI Y1 BAO it pays $\Delta\chi^2 \approx +10$.

Fixed-$H_0$ profiles reveal a geometric ceiling: $\Delta\chi^2$ rises from $+2$ at $H_0 = 69$ to $+91$ at 72 and $+163$ at 73. Early-universe physics cannot reach SH0ES; the likelihood imposes a ceiling near $H_0 \approx 70$. We identify the mechanism: DESI flips the EDE--$\tau_{\rm reio}$ correlation (from $+0.73$ to $-0.24$), closing a degeneracy that let low-$\ell$ polarization compensate the damping-tail cost.

The field predicts a ``soft shoulder'' in the CMB damping tail. A template fit to ACT DR6 yields $A_{\rm sh} = 1.16 \pm 0.18$---a $6.4\sigma$ preference for the predicted pattern ($A_{\rm sh} = 1$) over $\Lambda$CDM ($A_{\rm sh} = 0$). This is conditional on fixed Planck+BAO cosmology; fully marginalized may yield $3$--$5\sigma$. CMB-S4 will provide a definitive test.
\end{abstract}
