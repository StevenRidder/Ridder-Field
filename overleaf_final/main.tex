...
In pre-DESI combinations of Planck, BAO, and local $H_0$ data, this ``Geometric EDE'' scenario reduces the $H_0$ tension from $\sim 4.3\sigma$ to $\sim 2.3\sigma$ and the $S_8$ tension from $\sim 2.7\sigma$ to $\sim 1.1\sigma$, with an overall improvement $\Delta\chi^2 \approx -10$ relative to $\Lambda$CDM for two additional parameters. Once DESI Y1 BAO are included, the same mechanism encounters a \emph{geometric ceiling} near $H_0 \simeq 70$~km~s$^{-1}$~Mpc$^{-1}$: values significantly above 70 are strongly disfavored by sound-horizon constraints. In this DESI era, our best-fit scalar solution sits in the emerging ``convergence window'' $H_0 \simeq 69$--$71$ with reduced $S_8$, but pays a net penalty $\Delta\chi^2 \approx +10$--$20$ relative to the global $\Lambda$CDM minimum at $H_0 \simeq 68$. Stress-test runs show that this penalty is best interpreted as a \textbf{price of admission} to the convergence window. Once DESI is included, the $\Lambda$CDM posterior for $H_0$ becomes so narrow that forcing the standard model up to $H_0 \simeq 70$ would incur a $\chi^2$ cost of $\Delta\chi^2 \gtrsim +30$--$40$ (compared to Geometric EDE's $+15$) while simultaneously worsening $S_8$ from $0.83$ to $\sim 0.84$ (compared to Geometric EDE's improvement to $0.82$).
...
The key point is that this penalty measures the \textbf{cost of entering the convergence window}, not an intrinsic rejection of the model. $\Lambda$CDM achieves a lower raw $\chi^2$ only by staying \emph{outside} that window, at $H_0 \approx 68.5$--$69$ and $S_8 \approx 0.81$--$0.84$. Once DESI is included, the $\Lambda$CDM posterior for $H_0$ becomes very narrow; our SH0ES+DESI stress tests show that even a strong local prior leaves $\Lambda$CDM pinned near $H_0 \approx 68.5$. A simple Gaussian estimate implies that forcing $\Lambda$CDM up to $H_0 \approx 70$ would require a multi-$\sigma$ displacement and would incur a $\chi^2$ cost significantly larger than the $+15$--$20$ paid by Geometric EDE at the same $H_0$---while also \emph{worsening} $S_8$. In that sense, the $\phi$-field does not lose to $\Lambda$CDM once we insist on playing in the high-$H_0$ regime; it is the \textbf{cheapest viable way to get there}.
