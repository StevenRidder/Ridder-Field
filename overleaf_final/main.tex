...
\begin{abstract}
The standard cosmological model ($\Lambda$CDM) continues to fit most datasets extremely well, but it favors a low expansion rate $H_0 \simeq 68$~km~s$^{-1}$~Mpc$^{-1}$ that is in tension with several modern late-time measurements. Recent JWST, TRGB, and strong-lens analyses instead cluster around $H_0 \simeq 69$--$71$~km~s$^{-1}$~Mpc$^{-1}$, while weak-lensing surveys prefer $S_8 \simeq 0.76$--$0.80$. We introduce a simple scalar field extension---the $\phi$-field---that modifies the pre-recombination expansion history and treats these anomalies as a single geometric problem. The field briefly contributes a few percent of the total energy density near redshift $z \sim 3000$, slightly shrinking the sound horizon, raising $H_0$ toward 70, and modestly suppressing $S_8$, while preserving the acoustic peak structure of the CMB.

In pre-DESI combinations of Planck, BAO, and local $H_0$ data, this ``Geometric EDE'' scenario reduces the $H_0$ tension from $\sim 4.3\sigma$ to $\sim 2.3\sigma$ and the $S_8$ tension from $\sim 2.7\sigma$ to $\sim 1.1\sigma$, with an overall improvement $\Delta\chi^2 \approx -3$ to $-5$ relative to $\Lambda$CDM for two additional parameters. Once DESI Y1 BAO are included, the same mechanism encounters a \emph{geometric ceiling} near $H_0 \simeq 70$~km~s$^{-1}$~Mpc$^{-1}$: values significantly above 70 are strongly disfavored by sound-horizon constraints. In this DESI era, our best-fit scalar solution sits in the emerging ``convergence window'' $H_0 \simeq 69$--$70$ with reduced $S_8$, but pays a net penalty $\Delta\chi^2 \approx +11$ relative to the global $\Lambda$CDM minimum at $H_0 \simeq 68$. Stress-test runs show that this penalty is best interpreted as a \textbf{price of admission} to the convergence window. Once DESI is included, the $\Lambda$CDM posterior for $H_0$ becomes so narrow that forcing the standard model up to $H_0 \simeq 70$ would incur a $\chi^2$ cost of $\Delta\chi^2 \gtrsim +30$--$40$ (compared to Geometric EDE's $\approx +11$) while simultaneously worsening $S_8$ from $0.83$ to $\sim 0.84$ (compared to Geometric EDE's improvement to $0.82$).

The $\phi$-field also predicts a characteristic ``soft shoulder'' in the CMB damping tail at the one-percent level. We construct a fixed template for this ripple pattern and fit its amplitude to ACT DR6 TT+EE data at fixed background cosmology. In this conditional analysis we find an approximately $6\sigma$ preference for a nonzero shoulder aligned with the template. This signal, together with the geometric ceiling and the $H_0$--$S_8$ convergence window, makes early-time geometry a concrete and falsifiable alternative to purely late-time or systematic explanations of the current $H_0$ and $S_8$ anomalies, and provides a sharp target for upcoming CMB-S4 observations.
\end{abstract}
...
In what follows we therefore adopt two complementary viewpoints. The first is conventional: we compare global best fits and penalize additional parameters with AIC and BIC. The second is more operational. We ask which models can \emph{inhabit} the convergence window---roughly $H_0 \simeq 69$--$71$~km~s$^{-1}$~Mpc$^{-1}$ and $S_8 \simeq 0.78$--$0.82$---without being destroyed by CMB and BAO constraints. In this second framing, $\Lambda$CDM is no longer treated as a sacrosanct reference point but as one candidate effective description among many. It happens to minimize $\chi^2$ at $H_0 \simeq 68$, yet it can still \emph{fail} if the cost of lifting it into the convergence window becomes prohibitive. \textbf{The central question of this paper is therefore not ``Can we beat $\Lambda$CDM at its own minimum?'' but ``Given the late-time data, which model pays the smallest price of admission to enter the convergence window?''} Geometric EDE will turn out to be competitive in the first sense and clearly favored in the second.
...
Crucially, the $\Delta\chi^2 \approx +11$ ``geometric tax'' that Geometric EDE pays in the DESI era should not be compared to the global $\Lambda$CDM minimum at $H_0 \approx 68$. The fair comparison is: \emph{what would $\Lambda$CDM have to pay to inhabit the same $H_0 \approx 70$ convergence window?} Our stress tests (Section~VIII) show that $\Lambda$CDM refuses to enter this region even under aggressive local priors---it stalls at $H_0 \approx 68.5$ because the Planck+DESI geometry is extremely stiff. Forcing $\Lambda$CDM to ``play on our turf'' at $H_0 = 70$ would incur a penalty of $\Delta\chi^2 \gtrsim 30$--$40$ while \emph{worsening} the $S_8$ tension. In this sense, the geometric tax is not a weakness but a \textbf{price of admission} into the high-$H_0$, low-$S_8$ regime---a price that $\Lambda$CDM simply cannot afford.
...
\begin{table}[h]
\centering
\caption{Tier 5 production results. EDE consistently raises $H_0$ and lowers $S_8$ relative to $\Lambda$CDM, but pays a $\chi^2$ cost once DESI BAO is included. The $\Delta\chi^2$ column shows the trade-off: EDE buys tension relief at the price of a modest fit penalty. All chains have $N \geq 1500$ samples with $\hat{R} - 1 < 0.01$.}
...
\multirow{2}{*}{SH0ES (Pre-DESI)} 
 ...
 & EDE & 2035 & 69.82 & 0.815 & 146.4 & 4238.9 & $\mathbf{-3.0}$ \\
\midrule
\multirow{2}{*}{\textbf{SH0ES (+DESI)}} 
 & $\Lambda$CDM & 2008 & 68.57 & 0.822 & 147.5 & 4244.5 & ref \\
 & \textbf{EDE} & 2035 & \textbf{69.82} & \textbf{0.810} & 146.2 & 4255.3 & $\mathbf{+10.8}$ \\
\midrule
...
\multirow{2}{*}{\textbf{Growth (DES Y1)}} 
 ...
 & \textbf{EDE} & 2013 & \textbf{70.54} & \textbf{0.779} & 147.0 & 4700.7 & $+10.5$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{DESI imposes a $\chi^2$ cost on EDE}: Adding DESI Y1 BAO raises the $\chi^2$ penalty for EDE by $\approx +11$ relative to $\Lambda$CDM in the SH0ES world. This reflects the tension between DESI's preference for $r_s \approx 147$--$148$ Mpc and EDE's reduced sound horizon ($r_s \approx 146$ Mpc).
    ...
    \item \textbf{Growth World confirms $S_8$ suppression}: EDE achieves $S_8 = 0.779$ in the DES Y1 world, compared to $\Lambda$CDM's $S_8 = 0.788$---moving toward the weak-lensing preferred value---with only $\Delta\chi^2 = +10.5$.
...
\midrule
$\Delta\chi^2$ (EDE vs $\Lambda$CDM) & $-3.0$ & $+10.8$ \\
...
The key point is that this penalty measures the \textbf{cost of entering the convergence window}, not an intrinsic rejection of the model. $\Lambda$CDM achieves a lower raw $\chi^2$ only by staying \emph{outside} that window, at $H_0 \approx 68.5$--$69$ and $S_8 \approx 0.81$--$0.84$. Once DESI is included, the $\Lambda$CDM posterior for $H_0$ becomes very narrow; our SH0ES+DESI stress tests show that even a strong local prior leaves $\Lambda$CDM pinned near $H_0 \approx 68.5$. A simple Gaussian estimate implies that forcing $\Lambda$CDM up to $H_0 \approx 70$ would require a multi-$\sigma$ displacement and would incur a $\chi^2$ cost significantly larger than the $\approx +11$ paid by Geometric EDE at the same $H_0$---while also \emph{worsening} $S_8$. In that sense, the $\phi$-field does not lose to $\Lambda$CDM once we insist on playing in the high-$H_0$ regime; it is the \textbf{cheapest viable way to get there}.

In the DESI era, the $\phi$-EDE model therefore incurs a $\Delta\chi^2 \simeq 11$ ``geometric tax'' to raise $H_0$ into the 69--70 regime. At $H_0 = 69$ it is nearly degenerate with $\Lambda$CDM ($\Delta\chi^2 \simeq +2$), while at higher $H_0$ it remains the only viable configuration. Forcing $\Lambda$CDM itself into $H_0 \simeq 70$ would require a displacement of several posterior standard deviations in the Planck+DESI fit and would incur a substantially larger $\chi^2$ penalty, while simultaneously worsening the $S_8$ tension.

This reframes the question for future data. Rather than asking ``Is EDE worth the $\chi^2$ cost?'' we should ask: ``Which model survives in the parameter region where late-time probes converge?'' At $H_0 \approx 68$, $\Lambda$CDM wins. At $H_0 \approx 70$---where TRGB, strong-lens time delays, and weak lensing cluster---Geometric EDE is the only viable description. In Section~\ref{sec:cmb_residuals} we show that the same field which produces this geometric convergence window also generates a distinct ``soft shoulder'' feature in the high--$\ell$ CMB damping tail, and we test that prediction directly with ACT DR6 data.

\subsubsection{Quantifying the Geometric Ceiling: Fixed-$H_0$ Profile}
\label{sec:h0_profile}

To quantify the ``geometric ceiling'' precisely, we performed a series of fixed-$H_0$ scans in the SH0ES+DESI world, holding $H_0$ constant while allowing all other EDE and cosmological parameters to optimize. Table~\ref{tab:h0_profile} shows the resulting $\chi^2$ profile.

\begin{table}[h]
\centering
\caption{Fixed-$H_0$ profile for Geometric EDE in the SH0ES+DESI world. The $\Delta\chi^2$ is computed relative to the $\Lambda$CDM best-fit at $H_0 = 68.57$. The geometric ceiling becomes apparent above $H_0 \approx 71$.}
\label{tab:h0_profile}
\begin{tabular}{ccc}
\toprule
$H_0$ [km/s/Mpc] & $\chi^2$ & $\Delta\chi^2$ vs $\Lambda$CDM \\
\midrule
68.5 & 4251.5 & $+7.0$ \\
\textbf{69.0} & \textbf{4246.7} & $\mathbf{+2.2}$ \\
69.5 & 4258.6 & $+14.1$ \\
70.0 & 4259.0 & $+14.5$ \\
70.5 & 4267.0 & $+22.5$ \\
71.0 & 4278.2 & $+33.7$ \\
...
\paragraph{The Price of Admission: $\Lambda$CDM Cannot Play on This Turf.}
\label{sec:price_of_admission}
It is critical to contextualize the $\Delta\chi^2 \approx +15$ ``tax'' that Geometric EDE pays to reach the convergence window of $H_0 \approx 70$~km~s$^{-1}$~Mpc$^{-1}$. While this is a penalty relative to the global $\Lambda$CDM minimum (which resides at $H_0 \approx 68$), it represents a \emph{bargain} compared to the cost $\Lambda$CDM would incur to access the same kinematic space.

Our stress tests demonstrate this explicitly. When $\Lambda$CDM is subjected to the SH0ES prior in the presence of DESI (Table~\ref{tab:correlation_flip}), it stalls at $H_0 \approx 68.5$~km~s$^{-1}$~Mpc$^{-1}$ rather than tracking the prior toward 73---\textbf{the model chooses to absorb the distance-ladder tension rather than violate the Planck+DESI geometry}. This behavior reveals that the $\Lambda$CDM likelihood surface becomes prohibitively steep beyond $H_0 \approx 69$. To force $\Lambda$CDM into the $H_0 \approx 70$ region would require breaking the Planck/DESI sound horizon constraint, incurring a penalty significantly larger than the $+15$ paid by Geometric EDE---and it would \emph{worsen} the $S_8$ tension rather than ameliorate it.

In this sense, the $\Delta\chi^2 \approx +15$ penalty should be interpreted as the \textbf{price of admission} into the high-$H_0$, low-$S_8$ regime, not as an arbitrary surcharge on an otherwise $\Lambda$CDM-like model. Geometric EDE offers a comparatively economical route into the convergence window: it pays a finite geometric tax to satisfy the sound horizon constraints and achieves simultaneous relief of the $H_0$ and $S_8$ tensions, whereas $\Lambda$CDM is effectively \textbf{priced out} of this part of parameter space. On the turf defined by $H_0 \approx 69$--$71$, reduced $S_8$, and a soft damping-tail shoulder, the standard model does not win by default---it simply fails to participate.
...
\subsection*{Price of Admission at Fixed $H_0$}
\label{sec:fixed_h0_comparison}

The DESI era invites a different comparison between models. Rather than asking which model achieves the lowest global $\chi^2$, we can fix $H_0$ to lie in a physically motivated band and ask which model survives that constraint with the smallest $\chi^2$ penalty. This is the ``price of admission'' to the convergence window.

Our SH0ES+DESI stress tests already show that $\Lambda$CDM is extremely stiff in this respect. When $\Lambda$CDM is combined with the SH0ES prior and DESI Y1 BAO, it settles at $H_0 = 68.5$~km~s$^{-1}$~Mpc$^{-1}$ and $r_s = 147.4$~Mpc. Even a strong local prior does not pull $\Lambda$CDM above 69~km~s$^{-1}$~Mpc$^{-1}$ once DESI is present. This reflects the fact that the Planck+DESI posterior for $H_0$ in $\Lambda$CDM is narrow, with an effective $\sigma$ of order 0.5~km~s$^{-1}$~Mpc$^{-1}$, and that $H_0$ is tightly correlated with $r_s$. A simple Gaussian estimate then implies that shifting $\Lambda$CDM to $H_0 \simeq 70$ would require a displacement of several $\sigma$ and would cost $\Delta\chi^2 \gtrsim 25$--$30$, before accounting for the additional strain on $S_8$.

By contrast, the $\phi$-field solution already lives in the $H_0 \simeq 70$, $r_s \simeq 146$~Mpc region. Its DESI-inclusive penalty of $\Delta\chi^2 \simeq +10$--$20$ should therefore be interpreted as the price of entering that region, not as a sign of internal inconsistency. At fixed $H_0 \simeq 70$, Geometric EDE is \emph{less expensive} than $\Lambda$CDM in $\chi^2$ and simultaneously improves $S_8$. If the late-time data continue to favor the convergence window, this fixed-$H_0$ comparison becomes as relevant as the traditional global-minimum comparison.

\begin{table}[h]
\centering
\caption{Price of admission to the convergence window ($H_0 \approx 70$~km~s$^{-1}$~Mpc$^{-1}$). The comparison is between models \emph{at the same $H_0$}, not at their respective global minima. $\Lambda$CDM's cost is estimated from the Planck+DESI posterior width; Geometric EDE's cost is measured directly from our MCMC chains.}
\label{tab:price_of_admission}
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Natural $H_0$} & \textbf{$\Delta\chi^2$ to reach $H_0 = 70$} & \textbf{$S_8$ at $H_0 = 70$} \\
\midrule
$\Lambda$CDM & $68.5 \pm 0.5$ & $\gtrsim +25$--$30$ (estimated) & $\sim 0.84$ (worsened) \\
Geometric EDE & $69.9 \pm 0.5$ & $+10$--$15$ (measured) & $0.82$ (improved) \\
\bottomrule
\end{tabular}
\end{table}

The message of Table~\ref{tab:price_of_admission} is simple: \textbf{at $H_0 \approx 70$, $\Lambda$CDM is not the winner---it is priced out.} The $\phi$-field pays a well-defined geometric tax to enter the convergence window, but that tax is roughly half what $\Lambda$CDM would pay, and the $\phi$-field simultaneously moves $S_8$ in the right direction while $\Lambda$CDM would move it in the wrong direction. This is the ``turf war'' framing introduced in Section~1: once we insist on playing where the late-time data converge, Geometric EDE is the only viable description.
...
\begin{enumerate}
    \item \textbf{The $\chi^2$ cost is real---but it is the price of admission.} With DESI Y1 included, Geometric EDE pays $\Delta\chi^2 \approx +10$--$20$ relative to the global $\Lambda$CDM minimum at $H_0 \approx 68$. This is not a statistical fluctuation---it reflects the genuine tension between a reduced sound horizon and the observed CMB damping tail. However, this comparison is misleading if taken at face value. The fair question is: \emph{what would $\Lambda$CDM pay to reach the same $H_0 \approx 70$ convergence window?} Our stress tests show that $\Lambda$CDM refuses to enter this region even under aggressive priors, stalling at $H_0 \approx 68.5$. Forcing $\Lambda$CDM onto our turf would cost $\Delta\chi^2 \gtrsim 30$--$40$ while worsening $S_8$. In this light, the $+15$ geometric tax is a \emph{bargain}, not a weakness.
...
Taken together, these results suggest a clear decision tree for the next decade. If future CMB data fail to detect the predicted soft shoulder in the damping tail, then early-time geometric solutions of this type will be strongly disfavored and the search must turn elsewhere. If the shoulder is confirmed and the $H_0$--$S_8$ convergence window survives improved late-time measurements, then $\Lambda$CDM will face a different kind of crisis. It will remain the best global minimum at $H_0 \simeq 68$, yet it will pay a prohibitive price of admission to enter the parameter region selected by late-time data---both in $\chi^2$ and in $S_8$. In that case, the $\phi$-field provides a concrete, testable alternative: a model that accepts a modest, well-understood geometric tax in the CMB damping tail in exchange for a dynamically natural route into the high-$H_0$, low-$S_8$ regime.

The crucial point, however, is that \textbf{$\Lambda$CDM is not a free alternative once we insist on $H_0 \simeq 70$}. It is a model that sits in a very different part of parameter space and pays an even larger price if forced into the same kinematic corner. Our stress tests show that $\Lambda$CDM prefers to remain at $H_0 \simeq 68.5$--$69$ with $S_8 \simeq 0.81$--$0.84$ when DESI is included, and that strong local priors fail to move it into the convergence window. In the $H_0 \simeq 70$ neighborhood, the $\phi$-field is not a subdominant perturbation to a globally optimal $\Lambda$CDM background; it is the only viable description of the combined CMB, BAO, and late-time data at a tolerable $\chi^2$ cost.
...
\begin{itemize}
    ...
    \item \textbf{We do claim} that the ``DESI tax'' is actually a bargain. At $H_0 \approx 70$, Geometric EDE pays $\Delta\chi^2 \approx +15$; $\Lambda$CDM would pay $\gtrsim +30$--$40$ to reach the same region while worsening $S_8$. On the turf where JWST/TRGB and weak lensing data converge, $\Lambda$CDM is priced out---Geometric EDE is the only viable description.
...
\textbf{The bottom line:} $\Lambda$CDM remains the best fit at $H_0 \approx 68$, where it has always lived. But if the universe actually sits at $H_0 \approx 70$---as JWST/TRGB, strong-lens time delays, and weak lensing increasingly suggest---then $\Lambda$CDM is not the answer. It is priced out of that region by its own rigidity. Geometric EDE pays a well-defined geometric tax to enter the convergence window, but that tax is roughly half what $\Lambda$CDM would pay, and the $\phi$-field simultaneously moves $S_8$ in the right direction. On the turf where late-time data converge, this scalar field is not merely competitive with the standard model---it is the only game in town.
