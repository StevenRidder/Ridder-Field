...
1. `phase3/scripts/deploy_v2_to_vm.sh` (single VM)
   - Usage: `./deploy_v2_to_vm.sh ridderadmin@<IP>`
...
rsync -avz --exclude='*.o' --exclude='*.a' \
    phase2/class/ ridderadmin@172.174.34.125:~/Ridder-Field/phase2/class/

rsync -avz phase3/ridder_v2_tier3_production.yaml \
    ridderadmin@172.174.34.125:~/Ridder-Field/phase3/
...
Repeat Step 1, but:
- Replace IP: `172.174.34.125` → `172.191.4.60`
...
ssh ridderadmin@172.174.34.125 "cd ~/Ridder-Field/phase3 && [status commands]"

echo ""
echo "=== US EAST VM (TIER 4) ==="
ssh ridderadmin@172.191.4.60 "cd ~/Ridder-Field/phase3 && [status commands]"
```
...
1. **Download chains:**
   ```bash
   rsync -avz ridderadmin@172.174.34.125:~/Ridder-Field/phase3/chains/ridder_v2_tier3_prod* ./results/tier3/
   rsync -avz ridderadmin@172.191.4.60:~/Ridder-Field/phase3/chains/ridder_v2_tier4_prod* ./results/tier4/
   ```
...
# Launch (Australia)
ssh ridderadmin@172.174.34.125 "cd ~/Ridder-Field/phase3 && export CLASS_DATA_PATH=/classy && nohup python3 -m cobaya.run ridder_v2_tier3_production.yaml > tier3_prod.log 2>&1 &"

# Launch (US East)
ssh ridderadmin@172.191.4.60 "cd ~/Ridder-Field/phase3 && export CLASS_DATA_PATH=/classy && nohup python3 -m cobaya.run ridder_v2_tier4_production.yaml > tier4_prod.log 2>&1 &"

# Monitor
ssh ridderadmin@172.174.34.125 "tail -f ~/Ridder-Field/phase3/tier3_prod.log"
ssh ridderadmin@172.191.4.60 "tail -f ~/Ridder-Field/phase3/tier4_prod.log"
```
