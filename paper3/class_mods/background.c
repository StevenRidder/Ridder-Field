...
int background_functions(
  ...
  if (pba->has_cdm == _TRUE_) {
    ...
    /* NEW: Late-time DM-DE coupling (xi_late) - only active at z < 10 */
    if (pba->xi_late != 0.0) {
      double z_late = 1.0/a - 1.0;
      if (z_late < 10.0) {
        double Omega_m_frac = pba->Omega0_cdm + pba->Omega0_b;
        double Omega_DE_frac = 1.0 - Omega_m_frac;
        double f_DE = Omega_DE_frac * pow(a, 3.0) / (Omega_DE_frac * pow(a, 3.0) + Omega_m_frac);
    ...
    rho_tot += rho_cdm_eff;
    p_tot += 0.;
    rho_m += rho_cdm_eff;  /* Use effective for matter content */
  }
  /* idm */
...
int background_w_fld(
  ...
  case CLP:
    /* Ultra-low-z phantom transition */
    double z_here = 1.0/a - 1.0;
    if (pba->z_w_transition > 0 && z_here > pba->z_w_transition) {
      *w_fld = -1.0;  /* Standard cosmological constant above transition */
    } else {
  ...
  if (pba->has_ridder == _TRUE_) {
    ...
    if (pba->ridder_fluid_mode == _FALSE_) {
      ...
      if (pba->ridder_freeze_phi == _TRUE_) {
        ...
        /* V3 EDE: Let natural Hubble friction handle field freezing (like v2)
         * The explicit time-window freeze was causing issues by releasing 
         * the field when H was already too small to provide damping.
         */
        {
