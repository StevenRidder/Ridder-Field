...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /* RIDDER FIELD: Read parameters early to ensure has_ridder flag is set */
  ...
  class_read_double("xi_late",pba->xi_late);
  ...
  if (pba->Omega0_fld != 0.) {
    ...
    if (pba->fluid_equation_of_state == CLP) {
      ...
      class_read_double("z_w_transition",pba->z_w_transition);
  ...
  /* Validation */
  if (pba->has_ridder == _TRUE_) {
    /* Gauge restriction: Ridder field currently only supports Newtonian gauge */
    if (ppt->gauge != newtonian) {
...
int input_default_params(struct background *pba,
  ...
  pba->fluid_equation_of_state = CLP;
  pba->w0_fld = -1.;
  pba->z_w_transition = 0.0;  /* Default: no transition (standard CPL) */
  pba->cs2_fld = 1.;
  /** 9.a.2.1) 'CLP' case */
  pba->wa_fld = 0.;
  /** 9.a.2.2) 'EDE' case */
  ...
  /** 9.c) Ridder field (RC-X* model) */
  /** 9.c.1) Ridder field parameters */
  pba->Lambda_EDE_ridder = 0.0;        /**< EDE energy scale [eV] */
  ...
  pba->xi_late = 0.0;               /**< Late-time DM-DE coupling (default off) */
