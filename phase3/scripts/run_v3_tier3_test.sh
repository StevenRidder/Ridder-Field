...
rm -f $CHAINS_DIR/v3_tier3_test*.info.yaml
rm -f $CHAINS_DIR/v3_tier3_test*.lock
...
for i in {1..4}; do
    ...
    cd $WORK_DIR
    COBAYA_USE_FILE_LOCKING=False nohup cobaya-run config.yaml --force > chain${i}.log 2>&1 &
    PID=$!
